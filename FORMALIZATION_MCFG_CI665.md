...
Current confirmed CI point: Lean CI #665, commit `1ef3ea8`, pushed by `growupkuriyama-hub`.
...
`ConcreteCanonicalLearnerWorkingGrammarIdentification.lean` is now confirmed passed at Lean CI #665.  The verified chain listed above contains 178 CI-confirmed files.
...
current strongest class endpoint:
  correctedConcreteWorkingGrammarLearner_class_conclusion_package
current strongest selected-stage endpoint:
...
Current quantitative endpoint for the retained source finite rule object:

```lean
(correctedConcreteWorkingGrammarLearner hα obs f K).sourceRuleCount ≤
  (4 * (sampleLengthBudget K + f + 1)) ^
    (64 *
...
rule-count bound for the actual source finite hypothesis: complete, but deliberately coarse
actual WorkingMCFG compilation and reverse equivalence: complete on the exact domain
WorkingMCFG-valued uniform positive-data class theorem: complete under Nonempty α, StartRootedNormal, and substitutability
qualitative positive-learning theorem for the stated class: about 98--99%
compiled grammar total-size/description-size analysis: about 65--75%
executable polynomial learner implementation: about 60--70%
whole paper including observation design and boundary/non-identifiability results: about 65--70%
```

The principal remaining work is no longer existence of a concrete learner or
existence of an actual grammar-valued hypothesis.  It is executable coding,
replacement of brute-force bounded-word enumeration by sample factorization,
a bound for the compiled grammar including its cut saturation, preservation or
reconstruction of paper-side exact-working conditions if that is required of
output hypotheses, optional start normalization, and the negative/boundary
half of the paper.
...
The main import chain remains deliberately linear.  The directed-system file is a verified side branch from finite-object monotonicity; the actual-grammar compiler proceeds through the obstruction, cut-saturation, construction, equivalence, and identification chain.
...
CI #665 completes the qualitative concrete positive-learning route all the way
to an actual finite `WorkingMCFG`-valued learner on its exact compilation
domain.  The remaining limitations are now quantitative, executable,
normal-form, and boundary issues.

### 5.1 Executable coding versus `noncomputable` finite definitions

The finite rule sets, cut saturation, and compiled grammars are mathematically
finite, but several definitions use classical choice, function-valued Finsets,
and noncomputable filters/images.  Extracted executable code and verified
decidable encodings are still absent.

### 5.2 Polynomial-time construction

The present enumerator still generates all bounded words over the sample
alphabet.  The new rule-count bound honestly measures this brute-force object;
it does not show polynomial time.  A polynomial construction should enumerate
actual sample factorizations, substrings, component splits, and hole placements.

### 5.3 Total compiled-grammar size

The verified explicit bound is for the retained source finite-rule object.
The compiled grammar additionally contains:

```text
one start rule per sample word
one terminal seed rule
one constant rule per control tuple
one lifted rule per corrected binary rule
one rule per saturated cut pair.
```

A bound for `grammarRuleCount`, encoded description length, and the potentially
quadratic cut-pair family remains to be proved.

### 5.4 Paper-side exact-working conditions of output hypotheses

The compiled grammar is an exact language realization in the lightweight
`WorkingMCFG` syntax.  Its constant and cut rules intentionally ignore dummy
children, so the compiler does not claim that the output grammar itself
satisfies the paper's nondeleting/exact-once conditions.  A publication must
state whether hypotheses may use this broader syntax or provide a stricter
language-preserving compiler.
...
requires `Nonempty α`, and this is formally necessary.  The class theorem for
actual grammar outputs therefore carries this hypothesis.  The finite-rule
learner itself does not need it.

### 5.7 Quantitative learning bounds

Still open or not sharp:

```text
characteristic-sample cardinality
compiled grammar rule count and description length
construction time
stabilization-stage bounds
mind-change bounds
...
learner.  A final public import should identify the CI #665 endpoints as the
canonical route and move superseded interfaces to legacy/internal namespaces.

## 6. Immediate next files

The current CI #665 endpoint is:

```lean
ConcreteCanonicalLearnerWorkingGrammarIdentification.lean
```

The next file should be quantitative rather than another semantic facade.

### 6.1 `ConcreteCanonicalLearnerWorkingGrammarSize.lean`

Recommended next.

Prove exact formulas or upper bounds for:

```text
|controlCodes|
|cutPairs| ≤ |controlCodes|²
startRules.length
terminalRules.length
binaryRules.length
grammarRuleCount
```

and connect the result to the existing paper-facing source-rule bound.  A
natural first estimate is:

```text
control codes ≤ |K| + 2·unitRules + 3·binaryRules
compiled grammar rules
  ≤ |K| + 1 + controlCodes + binaryRules + controlCodes².
```
...
construction-time, hypothesis-description-size, and possibly mind-change
bounds.

### 6.5 Strict output-grammar normalization

If the paper requires learned hypotheses themselves to satisfy exact-working
conditions, construct a stricter compiler or clearly formalize the broader
hypothesis syntax accepted by the identification theorem.
...
### Stage H: Compiled grammar size and executable implementation

Status: open and now the principal positive-side task.

Targets:

```text
control/cut cardinality bounds
compiled grammar rule-count and description-size bounds
list-based decidable codes
sample-factorization enumeration
polynomial construction for fixed f and M
mind-change/stage bounds
```
...
strict output compiler preserving desired exact-working conditions
or explicit use of broader WorkingMCFG hypotheses
```
...
explicit source finite-hypothesis rule-count bound
...
```text
classical selection of characteristic samples and coverage stages
...
compiled grammar language exactness without a claim of nondeleting/exact-once output rules
```
...
compiled grammar total rule-count and description-size bound
construction-time and mind-change bounds
strict compiler preserving paper-side exact-working conditions, if required
...
```bash
lake build LeanCfgProject.MCFG.ConcreteCanonicalLearnerWorkingGrammarIdentification
```
...
Current strongest finite-rule endpoint:
...
Current strongest actual-grammar learner endpoints:
...
⇒ actual dependent finite hypothesis object
⇒ actual finite WorkingMCFG output with exactly the same language
⇒ exact reconstruction on every positive finite superset
...
CI #665 completes the qualitative actual-grammar-valued learning theorem.  The
next genuine mathematical gap is to bound the compiler overhead: finite control
codes, saturated cut pairs, and the total number/description size of rules in
the generated `WorkingMCFG`.  This will connect the already verified source
finite-rule bound to the actual hypothesis object returned by the final
learner.
