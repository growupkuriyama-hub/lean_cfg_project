...
Current confirmed CI point: Lean CI #738, commit `6ef91db`, pushed by `growupkuriyama-hub`.
...
`ConcreteCanonicalLearnerWorkingGrammarObservationSelectionParetoRankSelector.lean`
is now confirmed passed at Lean CI #738.  The verified chain listed above
contains 249 CI-confirmed files.
...
cost comparison, perturbation, normalization, and Pareto-envelope theory: complete semantically
...
observation-selection decision/optimization complexity: about 35--45%
...
observation-design mathematical framework: about 95--98% semantically
whole intended paper formalization: about 88--92%
```
...
witness, replace semantic target-membership filters by a decidable encoded
selection problem, and prove the intended decision/optimization complexity
bounds, including NP-hardness or NP-completeness if valid.  On the boundary
side: prove no-advice non-identifiability and the proposed copy/member-kernel
exclusions, and decide whether strict output/start normalization is required by
the final paper statement.
