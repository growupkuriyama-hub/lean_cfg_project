...
Current confirmed CI point: Lean CI #624, commit `c60dbd3`, pushed by `growupkuriyama-hub`.
...
`CharacteristicSampleExactOnceExposingTransportRoute.lean` is now confirmed passed.
...
unrestricted universal splicing constructor: proved false in general and no longer needed by the corrected route
exact-once integration: connected to characteristic samples, positive-superset exactness, prefix exactness, and Gold identification
minimal paper route: builder + fanout + promise + exact-once, without commonTransport
exposing-transport route: witness positivity + concrete exact-once splicing now reaches the paper conclusion package
finite typed presentation layer: soundness, monotonicity, and completeness interface are in place
trimmed-presentation layer: skeleton, base representatives, pre-core, and sample data are connected to the corrected exact-once route
current strongest corrected endpoint: trimmed_exposing_transport_exact_working_conclusion_package
concrete semantic exposing transport: still assumed through TrimmedPresentationExposingTransportData
concrete canonical learner enumeration: still not yet formalized
actual construction of G̃₀ / CS(G̃₀): still not concretely constructed from G and h
```
...
Current strongest corrected route, expanded:
...
```text
logical theorem plumbing and reachable-model reasoning: 90--95%
exact-once named-context splicing obligation: completed
conditional positive-learning theorem through exposing transport: about 85--90%
concrete end-to-end theorem from G and h to a learned grammar: about 65--70%
fully enumerated canonical learner and equivalence proof: about 50--55%
whole paper including complexity and boundary/non-identifiability results: about 45--50%
```

These percentages are qualitative.  The remaining work is concentrated in fewer
but still substantial constructions, especially concrete G^h/G̃₀, semantic
transport from the target promise, and concrete learner enumeration.
...
Current corrected exact-once endpoint:

```lean
trimmed_exposing_transport_exact_working_conclusion_package
```

Current corrected meaning:
...
The earlier all-pieces and context-equation endpoints are retained only as
historical decomposition milestones.
...
actual construction of the trimmed output-type presentation / G̃₀ as a concrete finite presentation
proof that the constructed G^h / G̃₀ presentation satisfies PresentationCompleteFor
actual construction of CS(G̃₀) from the concrete trimmed presentation
proof of the semantic transport obligations now isolated as ExposingContextTransport / AnchorCommonContextTransport / SameContextTransport variants
...
The latest CI-confirmed route is:
...
This route constructs rule-witness positivity from exposing-context transport
and uses the concrete exact-once splicing constructor internally.


## 5. What remains as explicit assumptions or skeleton fields?

The CI #621--#624 phase eliminated the main splicing construction obligation for exact-once working grammars.  The remaining assumptions are now concentrated in the concrete grammar/presentation and semantic-transport layers rather than in filling-identity bookkeeping.

### 5.1 Concrete exposing-context transport data

Still explicit.

The latest route assumes:

```lean
TrimmedPresentationExposingTransportData D
```

This contains finite base-nonterminal coverage, rule arity selectors, a `TrimmedPresentationExposingContextTransport D`, and positivity of the distinguished start word.  The next semantic target is to derive these objects from a concrete trimmed presentation and the fixed-observation substitutability promise.

### 5.2 Concrete `TrimmedOutputTypePresentation`

Still explicit.

Need to construct the actual finite trimmed output-type presentation from the refined grammar:

```text
G
⇒ G^h
⇒ reachable/productive trimmed presentation G̃₀
⇒ TrimmedOutputTypePresentation G obs
```

### 5.3 `PresentationCompleteFor` for the actual constructed presentation

The interface is verified, but the actual constructed `G^h / G̃₀` completeness proof is still missing:

```text
G.StringLanguage ⊆ PresentationStringLanguage P.
```

### 5.4 Concrete `TrimmedPresentationPreCoreData`

Still explicit.

Need to choose base representatives, prove terminal/binary/start type equalities, and connect typed representatives to base nonterminals.

### 5.5 Concrete `GrammarRuleBuilder` / rule coverage construction from the actual trimmed presentation

The grammar-rule-builder interfaces are verified, but the actual construction from the concrete finite trimmed presentation remains to be performed.

### 5.6 Semantic transport witnesses

Still explicit and now the main local semantic obligation.

The main target has narrowed to constructing one of the following, preferably the common-context route:

```lean
TrimmedPresentationAnchorCommonContextTransport D
```

Alternative stronger/weaker routes are also available:

```lean
TrimmedPresentationExposingContextTransport D
TrimmedPresentationSameContextTransport D
```

### 5.7 Concrete `CS(G̃₀)`

The finite-builder/component/rule enumeration infrastructure is verified, but the actual concrete characteristic sample from `G̃₀` remains to be built.

### 5.8 Concrete canonical learner equivalence

Still not done.

Need to relate the fully enumerated canonical learner from a finite sample to the currently verified reachable learner language:
...
or prove both inclusions.

### 5.9 Polynomial and boundary results

Still separate from the current Lean chain:

```text
fixed-parameter polynomial construction bound
no-advice non-identifiability
copy-language / member-kernel exclusion
```
...
The current CI #624 endpoint is:

```lean
CharacteristicSampleExactOnceExposingTransportRoute.lean
```

The exact-once named-context splicing problem is no longer the next task.  The
highest-value next phase is to construct the semantic exposing-transport data
rather than add another paper-facing wrapper.

### 6.1 `CharacteristicSampleExposingTransportFromSubstitutability.lean`
...
from concrete facts already stored by the trimmed presentation and the target
promise.

Expected proof pattern:

```text
D.expose A accepts D.anchor A
+
x has the same componentwise observation type as D.anchor A
+
a suitable common accepting context for anchor and x
+
FixedNamedTupleSubstitutable
⇒ D.expose A accepts x.
```
...
The substitutability promise alone does not create a shared context.  The file
must identify exactly which trimmed-presentation invariant proves that
`D.expose A` accepts the candidate tuple, or strengthen the pre-core data with a
genuine typed-nonterminal membership/derivation witness.  Do not hide this issue
in a new transport record.

### 6.2 `ConcreteOutputTypeRefinementPresentation.lean`

Parallel major target.

Goal:

Construct the finite output-type presentation `G^h` from the finite grammar and
finite monoid observation, rather than assuming an arbitrary
`OutputTypeRefinementPresentation`.

Required results:

```text
finite typed nonterminals and typed rules
ordinary derivations lift into the presentation
presentation derivations erase
PresentationCompleteFor for the constructed presentation
```

### 6.3 `ConcreteTrimmedOutputTypePresentation.lean`

After the full refinement presentation.

Goal:

Construct reachable/productive trimming and prove:

```lean
PresentationStringLanguage G̃₀ = G.StringLanguage.
```

This should also provide the present typed nonterminals and successful
occurrence witnesses needed for anchors and exposing contexts.

### 6.4 `CharacteristicSampleFromConcreteTrimmedPresentation.lean`

Goal:

Construct the actual finite `CS(G̃₀)` and the data required by
`TrimmedPresentationExposingTransportData`.

### 6.5 Concrete learner enumeration

Only after the concrete presentation/sample route is stable:
...
This stage is the foundational verified core; later exact-once paper-facing routes now sit above it.
...
```text
concrete construction of the finite sample CS(G̃₀) from a concrete trimmed presentation
concrete proof of exposing/common-context transport facts
construction of the trimmed presentation itself from G and h
```
...
transport construction diagrams and route choices
split core/global/target assumption layers
flat construction route facade
constructive learning consequence package
paper-facing constructive statement
preferred anchor-common all-pieces checklist
splicing decomposition to context functions and namedFill equations
formal counterexample to unrestricted splicing
concrete exact-once left/right context construction
exact-once filling equations and constructor
exact-once integration through characteristic samples and Gold identification
exposing-transport exact-once paper route
```

### Verified but intentionally abstract

```text
legacy unrestricted named-context splicing route (conditional only; premise is not generally inhabitable)
...
existence of a flat constructive route
paper constructive route assumption
preferred all-pieces existence
```
...
concrete flat anchor-common/exposing/same-context construction route
equivalence between reachable learner and fully enumerated canonical learner
polynomial bound
...
```bash
lake build LeanCfgProject.MCFG.CharacteristicSampleExactOnceExposingTransportRoute
```
...
Current strongest corrected paper-facing endpoint:
...
Most important completed correction:

```text
the unrestricted NamedContextSplicingConstructor is false in general;
exact-once named-context splicing is now concretely constructed and integrated.
```

Recommended next file:

```text
CharacteristicSampleExposingTransportFromSubstitutability.lean
```

Reason:

The splicing obligation is solved.  The next genuine mathematical gap in the
current strongest route is the construction of
`TrimmedPresentationExposingTransportData D`, especially the exposing-context
transport witness, from concrete trimmed-presentation semantics and the
fixed-observation substitutability promise.
...
The complete end-to-end theorem still requires an actual finite `G^h` and
trimmed `G̃₀`, not merely the verified presentation interfaces.
