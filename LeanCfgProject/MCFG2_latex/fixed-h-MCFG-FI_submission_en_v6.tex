...
Fix a fan-out bound \(f\) and an explicit finite monoid morphism \(h\).  For
every finite sample \(K\), the corresponding canonical hypothesis grammar can
be constructed in time \(\|K\|_+^{O(f)}\), including the time required to
write the output grammar.

Thus, for fixed \(f\) and \(h\), hypothesis construction is polynomial in the
sample size \(\|K\|_+\).  This is only a bound on constructing a hypothesis
from the sample at hand; it does not assert that a characteristic sample
sufficient for exact identification has size polynomial in the target
presentation.
\end{theorem}
...
what the fixed observation morphism captures in familiar block-synchronization
languages.  A fixed regular envelope supplies only coarse information such as
the order of the letter blocks.  Equalities between block lengths are not
encoded by the envelope; they are recovered by the learner from the
sentence-context distributions of tuples observed in positive examples.  We
also verify the semantic promise for these examples, so that they really belong
to the relevant fixed-observation fibers.  For related letter-count separations
and calibrations in MCFLs, see \cite{LehnerLindorfer2020}.
...
The preceding argument can be generalized.  For example, the language
\(\{a^n\#_2b^m\#_3c^n\#_4d^m\mid n,m\ge1\}\) is represented in the following
definition by \(\Lambda:=\{(1,3),(2,4)\}\).
...
\subsection{Sufficiency of concrete contexts}

\begin{proposition}[Concrete-context sufficiency]
\label{prop:concrete-context-sufficiency}
Let \(G\) be a reduced working MCFG, and let \(\widetilde G_0\) be its trimmed
output-type refinement.  For each surviving typed nonterminal \(X\), fix an
anchor tuple \(\omega(X)\in L_X\) and a concrete exposing context \(\chi(X)\)
such that \(\chi(X)[\tuple u]\in L(G)\) for every \(\tuple u\in L_X\).  If
\(\widetilde S\to X\) is a typed start rule, choose
\(\chi(X)=\blank_1\).

Let \(S_{\mathrm{ctx}}\subseteq L(G)\) consist of the following words:
\begin{enumerate}[label=(\roman*),leftmargin=*]
\item the anchor exposure \(\chi(X)[\omega(X)]\) for every surviving typed
      nonterminal \(X\);
\item the terminal exposure \(\chi(X)[(a)]\) for every surviving typed terminal
      rule \(X\to(a)\);
\item the rule exposure \(\chi(X)[\tuple z_R]\) for every surviving typed
      binary rule \(R\colon X\to\rho(Y,Z)\), where
      \(\tuple z_R:=\rho(\omega(Y),\omega(Z))\).
\end{enumerate}
Thus \(S_{\mathrm{ctx}}=\CS(\widetilde G_0)\).

For every finite sample \(K\) satisfying
\(S_{\mathrm{ctx}}\subseteq K\subseteq L(G)\), the canonical hypothesis
\(\widehat G_0(K)\) simulates every rule of \(\widetilde G_0\) under the map
\(\iota(X):=[\omega(X)]\), in the following precise sense.
\begin{enumerate}[label=(\roman*),leftmargin=*]
\item If \(\widetilde S\to X\) is a typed start rule, then
      \(\widehat S\to\iota(X)\) is a start rule of \(\widehat G_0(K)\).
\item If \(X\to(a)\) is a typed terminal rule, then
      \(\iota(X)\Rightarrow[(a)]\Rightarrow(a)\) in \(\widehat G_0(K)\).
\item If \(R\colon X\to\rho(Y,Z)\) is a typed binary rule, then
      \[
...
Consequently, the simulation requires only the componentwise \(h\)-types
computed from observed tuples, one exposed anchor for each surviving typed
nonterminal, and one exposed filled example for each surviving typed rule.  No
additional sentence-level interface marker has to be stored in a learner
nonterminal.
\end{proposition}

\begin{proof}
The set \(S_{\mathrm{ctx}}\) is finite because \(\widetilde G_0\) has only
finitely many surviving typed nonterminals and typed rules.  Moreover, every
word listed above belongs to \(L(G)\).  Indeed,
\(\omega(X)\in L_X\), and the defining property of \(\chi(X)\) gives
\(\chi(X)[\omega(X)]\in L(G)\).  If \(X\to(a)\) is a typed terminal rule,
then \((a)\in L_X\), so \(\chi(X)[(a)]\in L(G)\).  If
\(R\colon X\to\rho(Y,Z)\) is a typed binary rule, then
\(\omega(Y)\in L_Y\) and \(\omega(Z)\in L_Z\), whence
\(\tuple z_R=\rho(\omega(Y),\omega(Z))\in L_X\) and
\(\chi(X)[\tuple z_R]\in L(G)\).  Thus
\(S_{\mathrm{ctx}}\subseteq L(G)\).

Fix a finite \(K\) with
\(S_{\mathrm{ctx}}\subseteq K\subseteq L(G)\).  We consider the three rule
forms separately.

\medskip
\noindent
\textbf{Start rules.}
Let \(\widetilde S\to X\) be a typed start rule.  Then \(X\) has fan-out one
and \(\chi(X)=\blank_1\).  Write \(\omega(X)=(w_X)\).  Since
\(w_X=\chi(X)[\omega(X)]\in K\),
Definition~\ref{def:learner}(i) adds the start rule
\(\widehat S\to[(w_X)]=[\omega(X)]=\iota(X)\).

\medskip
\noindent
\textbf{Terminal rules.}
Let \(X\to(a)\) be a typed terminal rule.  Both
\(\chi(X)[\omega(X)]\) and \(\chi(X)[(a)]\) belong to \(K\), so
\(\omega(X)\) and \((a)\) are observed in the same concrete context
\(\chi(X)\).  Write \(X=A_{\mathbf p}\).  By
Proposition~\ref{prop:output-invariants},
\(h^{(\mu(A))}(\omega(X))=\mathbf p\).  The definition of a typed terminal
rule gives \(\mu(A)=1\) and \(\mathbf p=(h(a))\), hence
\(h^{(1)}(\omega(X))=h^{(1)}((a))\).  Therefore
Definition~\ref{def:learner}(iv) adds the unit rule
\([\omega(X)]\to[(a)]\), while Definition~\ref{def:learner}(ii) adds
\([(a)]\to(a)\).  Thus
\(\iota(X)\Rightarrow[(a)]\Rightarrow(a)\).

\medskip
\noindent
\textbf{Binary rules.}
Let \(R\colon X\to\rho(Y,Z)\) be a typed binary rule and put
\(\tuple z_R:=\rho(\omega(Y),\omega(Z))\).  The two words
\(\chi(X)[\omega(X)]\) and \(\chi(X)[\tuple z_R]\) belong to \(K\), so
\(\omega(X)\) and \(\tuple z_R\) are observed in the same concrete context
\(\chi(X)\).  If \(X=A_{\mathbf p}\), then
Proposition~\ref{prop:output-invariants} gives
\(h^{(\mu(A))}(\omega(X))=\mathbf p\).  Because \(R\) is a typed rule with
left-hand side \(X\), the definition of the output-type refinement and the
same proposition give \(h^{(\mu(A))}(\tuple z_R)=\mathbf p\).  Hence
Definition~\ref{def:learner}(iv) adds the unit rule
\([\omega(X)]\to[\tuple z_R]\).

Consider the parent occurrence \((\chi(X),\tuple z_R)\) in the word
\(\chi(X)[\tuple z_R]\).  The child tuples \(\omega(Y)\) and \(\omega(Z)\)
are observed in \(K\) through their own anchor exposures
\(\chi(Y)[\omega(Y)]\) and \(\chi(Z)[\omega(Z)]\); these observations need
not occur in the parent sample word.  In the equality
\(\tuple z_R=\rho(\omega(Y),\omega(Z))\), the template of \(\rho\) partitions
each parent component into terminal gaps and variable intervals carrying the
components of \(\omega(Y)\) and \(\omega(Z)\).  Linearity and nondeletion make
each variable occur exactly once over the whole parent tuple.  If a child
component is empty, the variable order in the template supplies the local tie
order at the common cut.

The parent occurrence, the two child observations, and this ordered
segmentation therefore form a binary witness in the sense of
Definition~\ref{def:binary-witness}.  By
Lemma~\ref{lem:unique-induced-template}, its induced template is exactly the
original template \(\rho\).  Definition~\ref{def:learner}(iii) consequently
adds the binary rule
\([\tuple z_R]\to\rho([\omega(Y)],[\omega(Z)])\).  Combining it with the unit
rule obtained above yields
\(\iota(X)\Rightarrow[\tuple z_R]\Rightarrow
\rho(\iota(Y),\iota(Z))\).

Thus every surviving start, terminal, and binary rule of \(\widetilde G_0\) is
simulated under \(X\mapsto\iota(X)\).  The context \(\chi(X)\) is used only as
positive evidence that two observed tuples share an accepting context, thereby
justifying the relevant unit rule.  Learner nonterminals remain indexed solely
by observed tuple values \([\tuple x]\); neither \(\chi(X)\) nor any other
sentence-level interface information has to be stored in their names.
\end{proof}
...
Let \(G\) be a witnessing working presentation of the target language, and let
\(\widetilde G_0\) be its trimmed output-type refinement.  Use the anchor tuples
\(\omega(X)\) and exposing contexts \(\chi(X)\) fixed in
Proposition~\ref{prop:concrete-context-sufficiency}.

Let \(\widetilde V_0\) be the set of surviving typed nonterminals of
\(\widetilde G_0\), and let \(\widetilde P_{\mathrm{ter}}\) and
\(\widetilde P_{\mathrm{bin}}\) be its sets of surviving typed terminal and
binary rules, respectively.  Define the exposure-word set by
\[
\begin{aligned}
...
\end{aligned}
\]
The set is nonempty because \(\widetilde G_0\) has at least one surviving
typed nonterminal.  The \emph{exposure size} of \(G\) relative to \(h\) is
\(B_{\mathrm{exp}}(G,h):=
\max_{w\in\mathcal E_{\mathrm{exp}}(G,h)}\max\{1,|w|\}\).
The notation suppresses the dependence on the fixed choices of \(\omega(X)\)
and \(\chi(X)\).
\end{definition}

\begin{proposition}[Characteristic-sample size under an exposure bound]
\label{prop:cs-exposure-bound}
Put \(N_{\mathrm{NT}}:=|\widetilde V_0|\) and
\(N_{\mathrm{rule}}:=|\widetilde P_{\mathrm{ter}}|+
|\widetilde P_{\mathrm{bin}}|\), the number of surviving typed non-start rules.
Then
\[
...
\]
Consequently, if \(N_{\mathrm{NT}}\), \(N_{\mathrm{rule}}\), and
\(B_{\mathrm{exp}}(G,h)\) are all polynomially bounded in the size \(|G|\) of
the chosen witnessing presentation on a subclass, then every target in that
subclass has a characteristic sample of size polynomial in \(|G|\).
\end{proposition}

\begin{proof}
By Definition~\ref{def:cs}, the characteristic sample contains at most one
anchor exposure for each \(X\in\widetilde V_0\), at most one terminal exposure
for each rule in \(\widetilde P_{\mathrm{ter}}\), and at most one binary-rule
exposure for each rule in \(\widetilde P_{\mathrm{bin}}\).  Counting before
duplicate words are removed gives at most
\(N_{\mathrm{NT}}+N_{\mathrm{rule}}\) words; taking a set can only decrease
this number.

Every \(w\in\CS(\widetilde G_0)\) satisfies
\(\max\{1,|w|\}\le B_{\mathrm{exp}}(G,h)\).  Therefore
\[
  \|\CS(\widetilde G_0)\|_+
...
Finally, suppose that polynomials \(p_1,p_2,p_3\) satisfy
\(N_{\mathrm{NT}}\le p_1(|G|)\),
\(N_{\mathrm{rule}}\le p_2(|G|)\), and
\(B_{\mathrm{exp}}(G,h)\le p_3(|G|)\) throughout the subclass.  Then
\(\|\CS(\widetilde G_0)\|_+\le
(p_1(|G|)+p_2(|G|))p_3(|G|)\), which is polynomial in \(|G|\).
\end{proof}
