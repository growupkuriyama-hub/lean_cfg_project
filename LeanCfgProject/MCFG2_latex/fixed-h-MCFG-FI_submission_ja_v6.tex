...
\label{thm:poly}
ファンアウト上界 \(f\) と明示的有限モノイド準同型 \(h\) を固定する。
このとき、任意の有限正例標本 \(K\) から正準仮説文法
\(\widehat{G}_0(K)\) を、出力文法を書き出す時間も含めて
\(\lVert K\rVert_+^{O(f)}\) 時間で構成できる。

したがって、各固定 \(f\) および \(h\) に対して、この構成時間は
\(\lVert K\rVert_+\) の多項式である。ただし、その多項式の次数は
\(f\) に依存してよいので、この評価は \(f\) に関して一様な
多項式時間上界を主張するものではない。
\end{theorem}

\begin{proof}
\(n:=\lVert K\rVert_+\) とおく。以下では、\(f\)、有限モノイド \(M\)、
その積表、および各文字に対する \(h\)-値は固定されているものとする。
したがって、モノイド演算一回の計算時間は定数として扱える。

まず、標本中のタプル出現を列挙する計算量を評価する。
固定された \(1\le d\le f\) に対して、一つの標本語
\(w\in K\) におけるアリティ \(d\) のタプル出現は、各スロットの
二つの端点と、スロットの左から右への順序によって指定される。
空スロットが同じ切断位置に置かれる場合には、その切断位置における
局所順序も指定する。

語 \(w\) の長さを \(m\) とすると、切断位置は \(m+1\) 個である。
したがって、固定された \(d\) に対する候補数は
\((m+1)^{O(d)}\) 個である。すべての \(w\in K\) と
\(1\le d\le f\) について和を取っても、標本中のすべての
タプル出現と、そこから得られるすべての観測タプル値は
\(n^{O(f)}\) 個以下である。

各 \(w\in K\) に対する開始規則
\(\widehat{S}\to[(w)]\) と、観測された各文字タプル
\((a)\) に対する終端規則 \([(a)]\to(a)\) は、
列挙された観測タプルから直接構成できる。

次に単位規則を考える。同じアリティ \(d\le f\) の観測タプル
\(\tuple{x},\tuple{y}\) の各順序対について、まず
\(h^{(d)}(\tuple{x})=h^{(d)}(\tuple{y})\) を検査する。
さらに、列挙済みの具体的出現を用いて、ある一つのアリティ \(d\) の
文コンテクスト \(E\) が存在し、
\(E[\tuple{x}]\in K\) と \(E[\tuple{y}]\in K\) がともに
成り立つかを検査する。両条件が成り立つ場合に限り、単位規則
\([\tuple{x}]\to[\tuple{y}]\) を加える。

観測タプル値と具体的出現の総数はいずれも \(n^{O(f)}\) であるため、
それらの有限個の組を全探索しても、計算量は \(n^{O(f)}\) の範囲に
収まる。ここで指数中の定数は変化しうるが、依然として \(f\) の
定数倍である。

最後に二分証人を列挙する。二分証人の候補は、親タプル出現
\((E_P,\tuple{z})\)、アリティがそれぞれ高々 \(f\) の二つの
観測子タプル \(\tuple{x},\tuple{y}\)、およびそれらの成分を
親タプル内に配置する順序付き分割によって指定される。

子タプルの成分数の合計は高々 \(2f\) である。したがって、
親タプルの各成分を、終端語からなる間隙と、子成分に対応する
高々 \(2f\) 個の変数区間とに分割する方法は、高々 \(O(f)\) 個の
切断位置、変数ラベル、および同一切断位置における局所順序によって
指定できる。\(f\) は固定されているので、すべての候補分割の総数は
\(n^{O(f)}\) 個である。

各候補について、定義~\ref{def:binary-witness} の条件、すなわち、
各変数がちょうど一度使われていること、指定された各区間の文字列が
対応する子タプル成分に一致すること、および親成分全体が指定された
終端間隙と変数区間から復元されることを、文字列の直接比較によって
検査する。条件を満たす候補ごとに、誘導テンプレートを用いた規則
\([\tuple{z}]\to\rho([\tuple{x}],[\tuple{y}])\) を一つ出力する。

以上より、正準学習器のすべての非終端記号と規則を列挙する時間は、
出力される文法の大きさを含めて \(n^{O(f)}\) で抑えられる。

なお、単位規則は定義~\ref{def:learner} における仮説文法の一部である。
単位規則を含まない等価な表示が必要な場合には、構成済み文法の
非終端記号間の単位閉包を計算し、単位経路の先にある終端規則および
二分規則をその始点へ複製した後、単位規則を除去すればよい。
非終端記号数と規則数はすでに \(n^{O(f)}\) であるため、この処理も、
固定された \(f,h\) の下では \(n^{O(f)}\) 時間で実行できる。
\end{proof}
...
\(G\) を対象言語の証人となる簡約された作業形表示とし、
\(\widetilde{G}_0\) を \(G\) のトリムされた出力型精密化とする。
また、定義~\ref{def:cs} で固定したアンカータプル
\(\omega(X)\) と露出文脈 \(\chi(X)\) に基づく特徴標本を
\(\CS(\widetilde{G}_0)\) とする。

\(G\) の \(h\) に関する\emph{露出サイズ}を
\[
  B_{\mathrm{exp}}(G,h)
  :=
  \max_{w\in\CS(\widetilde{G}_0)}
  \max\{1,\lvert w\rvert\}
\]
と定める。

すなわち、\(B_{\mathrm{exp}}(G,h)\) は、選択された特徴標本に
含まれる語の正例サイズの最大値である。\(G\) は簡約されており、
各型付き非終端記号にアンカーが選ばれているため、
\(\CS(\widetilde{G}_0)\neq\emptyset\) であり、上の最大値は
定義される。

この記法では、あらかじめ固定したアンカータプル
\(\omega(X)\) と露出文脈 \(\chi(X)\) への依存を省略している。
\end{definition}

\begin{proposition}[露出上界の下での特徴標本サイズ]
\label{prop:cs-exposure-bound}
\(\widetilde{V}_0\) を、\(\widetilde{G}_0\) に残る型付き
非終端記号の集合とする。また、
\(\widetilde{P}_{\mathrm{ter}}\) と
\(\widetilde{P}_{\mathrm{bin}}\) を、それぞれ残っている
型付き終端規則と型付き二分規則の集合とする。

\(N_{\mathrm{NT}}:=\lvert\widetilde{V}_0\rvert\) および
\(N_{\mathrm{rule}}
:=\lvert\widetilde{P}_{\mathrm{ter}}\rvert
 +\lvert\widetilde{P}_{\mathrm{bin}}\rvert\)
とおく。このとき、
\[
  \lvert\CS(\widetilde{G}_0)\rvert
  \le N_{\mathrm{NT}}+N_{\mathrm{rule}},
  \qquad
  \lVert\CS(\widetilde{G}_0)\rVert_+
  \le
...
したがって、ある部分クラス上で \(N_{\mathrm{NT}}\)、
\(N_{\mathrm{rule}}\)、および \(B_{\mathrm{exp}}(G,h)\) が
いずれも、選ばれた証人表示の大きさ \(\lvert G\rvert\) の
多項式で上から抑えられるならば、その部分クラスの各対象は、
\(\lvert G\rvert\) に関して多項式サイズの特徴標本をもつ。
\end{proposition}

\begin{proof}
定義~\ref{def:cs} によれば、特徴標本
\(\CS(\widetilde{G}_0)\) は、次の語を集合として集めたものである。

各 \(X\in\widetilde{V}_0\) に対して、アンカー露出語
\(\chi(X)[\omega(X)]\) を一つ選ぶ。また、各
\(X\to(a)\in\widetilde{P}_{\mathrm{ter}}\) に対して終端露出語
\(\chi(X)[(a)]\) を一つ選び、各
\(X\to\rho(Y,Z)\in\widetilde{P}_{\mathrm{bin}}\) に対して
二分規則露出語
\(\chi(X)[\rho(\omega(Y),\omega(Z))]\) を一つ選ぶ。

したがって、重複を区別して数えた場合でも、選ばれる語の総数は
\(N_{\mathrm{NT}}+N_{\mathrm{rule}}\) 以下である。
\(\CS(\widetilde{G}_0)\) は集合として定義されているため、
異なる記号や規則から同じ語が選ばれた場合には、その重複は一つの
要素にまとめられる。ゆえに
\(\lvert\CS(\widetilde{G}_0)\rvert
\le N_{\mathrm{NT}}+N_{\mathrm{rule}}\)
を得る。

定義~\ref{def:exposure-size} により、すべての
\(w\in\CS(\widetilde{G}_0)\) について
\(\max\{1,\lvert w\rvert\}\le B_{\mathrm{exp}}(G,h)\) である。
したがって、
\[
\begin{aligned}
  \lVert\CS(\widetilde{G}_0)\rVert_+
  &=
  \sum_{w\in\CS(\widetilde{G}_0)}
  \max\{1,\lvert w\rvert\}\\
  &\le
  \lvert\CS(\widetilde{G}_0)\rvert
  B_{\mathrm{exp}}(G,h)\\
...
最後に、ある多項式 \(p_1,p_2,p_3\) が存在し、対象とする部分クラスの
すべての証人表示 \(G\) について
\(N_{\mathrm{NT}}\le p_1(\lvert G\rvert)\)、
\(N_{\mathrm{rule}}\le p_2(\lvert G\rvert)\)、および
\(B_{\mathrm{exp}}(G,h)\le p_3(\lvert G\rvert)\)
が成り立つと仮定する。このとき、
\[
  \lVert\CS(\widetilde{G}_0)\rVert_+
  \le
  \bigl(
    p_1(\lvert G\rvert)+p_2(\lvert G\rvert)
  \bigr)
  p_3(\lvert G\rvert)
\]
である。右辺は \(\lvert G\rvert\) の多項式なので、この部分クラスは、
選ばれた証人表示に関して多項式サイズの特徴標本をもつ。
\end{proof}

完全出力型精密化に現れる型付き非終端記号の個数は高々
\(\sum_{A\in V\setminus\{S\}}\lvert M\rvert^{\mu(A)}\) である。
また、もとの二分規則 \(A\to\rho(B,C)\) 一つから生じる型付き
二分規則は高々
\(\lvert M\rvert^{\mu(B)+\mu(C)}\) 個である。

したがって、\(f\) と \(h\) を固定すれば、各非終端記号と各規則から
生じる型付きコピーの個数には、\(G\) に依存しない定数上界がある。
そのため、トリムされた出力型精密化についても
\(N_{\mathrm{NT}}+N_{\mathrm{rule}}=O(\lvert G\rvert)\)
である。ここで定数は、固定された \(f\)、\(M\)、および \(h\) に
依存してよい。

これに対して、特徴標本に選ばれる露出語の長さは、型付き記号や規則の
個数だけからは制御できない。アンカーを生成する導出や、それを
開始記号まで露出する導出が非常に長い場合には、
\(B_{\mathrm{exp}}(G,h)\) も大きくなりうる。したがって、
対象クラス全体に対する多項式データ上界を得るうえで本質的に追加の
制御を必要とする量は、特徴標本中の最大語長、すなわち
\(B_{\mathrm{exp}}(G,h)\) である。

命題~\ref{prop:cs-exposure-bound} は、クラス
\(\Cmcf{f}{h}\) 全体に対して \(B_{\mathrm{exp}}(G,h)\) が
\(\lvert G\rvert\) の多項式で抑えられることを主張するものではない。
本論文で無条件に得られるのは、有限特徴標本による厳密再構成と、
任意の与えられた有限標本からの仮説の多項式時間構成である。
多項式時間かつ多項式データでの同定が従うのは、これに加えて
\(B_{\mathrm{exp}}(G,h)\) に多項式上界をもつ部分クラスに限られる。
...
本論文の同定定理は、対象言語が意味論的な
((f,h))-タプル代入可能性を満たすことを仮定する。
本付録では、この仮定が、与えられた文法表示から一般に判定できる
構文論的付帯条件ではないことを示す。

有限アルファベット (\Sigma)、整数 (f\ge 1)、および
明示的有限モノイド準同型 (h\colon\Sigma^*\to M) を固定する。
標準的な実効符号化の下で、
(\mathsf{Sub}^{\mathrm{CFG}}_{f,h}) を、(\Sigma) 上の CFG (G) の符号
(\langle G\rangle) であって、(L(G)) が
((f,h))-タプル代入可能であるもの全体の集合とする。

\begin{theorem}[固定有限観測に対する非決定可能性]
\label{thm:promise-undecidable}
(\Sigma) を (\lvert\Sigma\rvert\ge 2) を満たす固定有限アルファベットとし、
(f\ge 1) を固定する。また、(h\colon\Sigma^*\to M) を
有限モノイド (M) への任意の固定された明示的準同型とする。
このとき、(\mathsf{Sub}^{\mathrm{CFG}}_{f,h}) は
co-r.e.-完全である。

したがって、この集合は決定不能であり、標準的な実効符号化の下では
(\Pi^0_1)-完全である。
\end{theorem}

\begin{proof}
証明は、固定アルファベット上の CFG 普遍性問題からの多対一帰着と、
代入可能性の失敗が有限証人によって半決定可能であることからなる。

\paragraph{第1段階：固定アルファベット上の CFG 普遍性。}
標準的な実効符号化の下で、CFG の普遍性問題は
(\Pi^0_1)-完全である
\cite{DomaratzkiOkhotinShallit2007}。
ここで、入力文法のアルファベットは入力に応じて変化してよい。

この完全性は、濃度が少なくとも二である任意の固定アルファベット
(\Sigma) 上でも成り立つ。
実際、相異なる文字 (a,b\in\Sigma) を固定する。
任意の有限アルファベット (\Gamma) に対し、
(2^m\ge\lvert\Gamma\rvert) を満たす (m\ge 1) を選び、
単射 (c\colon\Gamma\to{a,b}^m) を構成する。
これをモノイド準同型 (c\colon\Gamma^*\to\Sigma^*) へ拡張する。

符号語全体の集合を (C:=c(\Gamma)^*) とする。
すべての符号語が同じ長さ (m) をもつため、(C) は正則であり、
(c\colon\Gamma^*\to C) は単射である。

(\Gamma) 上の CFG (G) から、(\Sigma) 上の言語
(U_G:=(\Sigma^*\setminus C)\cup c(L(G))) を生成する CFG を
実効的に構成できる。しかも、
[
U_G=\Sigma^*
...
右から左は明らかである。
逆に (U_G=\Sigma^*) と仮定し、(w\in\Gamma^*) をとる。
(c(w)\in C) なので、(c(w)\in U_G) であるためには
(c(w)\in c(L(G))) でなければならない。
(c) の単射性から (w\in L(G)) を得る。
したがって、固定された非単項アルファベット上の CFG 普遍性は
(\Pi^0_1)-困難である。

一方、語 (w\in\Sigma^*) を列挙し、CFG 所属問題によって
(w\notin L(G)) を検査すれば、非普遍性は半決定できる。
よって、固定アルファベット上の普遍性問題は (\Pi^0_1) に属し、
したがって (\Pi^0_1)-完全である。
単項アルファベットの場合については
\cite{Hopcroft1969} を参照されたい。

\paragraph{第2段階：同じ (h)-値をもつ二つの接頭辞。}
(M) は有限であり、(\lvert\Sigma\rvert\ge 2) なので、
(\lvert\Sigma\rvert^N>\lvert M\rvert) を満たす (N\ge 1) を選べる。
鳩の巣原理により、相異なる語 (X,Y\in\Sigma^N) で
(h(X)=h(Y)) を満たすものが存在する。

(X) と (Y) は同じ長さをもつ相異なる語なので、
一方が他方の接頭辞になることはない。
したがって (X\Sigma^*\cap Y\Sigma^*=\emptyset) である。
以下では、この (X) と (Y) を入力文法に依存しない固定語として用いる。

\paragraph{第3段階：入力文法からの言語構成。}
非空語 (\beta\in\Sigma^+) を固定する。
(\Sigma) 上の CFG (G_0) に対して、
(L'_{G_0}:=(\Sigma^*\setminus\beta\Sigma^*)\cup\beta L(G_0))
と定める。

この言語は文脈自由であり、(G_0) からその文法を実効的に構成できる。
また、(L(G_0)=\Sigma^*) ならば (L'_{G_0}=\Sigma^*) である。
さらに、(z\notin L(G_0)) ならば
(\beta z\notin L'*{G_0}) であり、(\eps\in L'*{G_0}) である。

(H_0:=h(\Sigma^*)) および (S:=h(Y)H_0) とおき、
(K_{\mathrm{pos}}:=h^{-1}(S)) と定める。
これは実効的に構成可能な正則言語である。最後に、
[
K(G_0)
:=
\bigl(K_{\mathrm{pos}}\setminus Y\Sigma^*\bigr)
\cup YL'_{G_0}.
\tag{2}
]

第1項は正則であり、第2項は文脈自由なので、(K(G_0)) は
文脈自由である。さらに、(G_0) から (K(G_0)) を生成する CFG を
実効的に構成できる。

\paragraph{第4段階：(G_0) が普遍的である場合。}
(L(G_0)=\Sigma^*) と仮定する。
このとき (L'_{G_0}=\Sigma^*) である。

また、任意の (w\in\Sigma^*) に対して
(h(Yw)=h(Y)h(w)\in h(Y)H_0=S) なので、
(Y\Sigma^*\subseteq K_{\mathrm{pos}}) である。
したがって、式~\textup{(2)} から
(K(G_0)=K_{\mathrm{pos}}=h^{-1}(S)) を得る。

任意の (d\ge 1)、タプル
(\tuple{x},\tuple{y}\in(\Sigma^*)^d)、およびアリティ (d) の
文コンテクスト (E) をとり、
(h^{(d)}(\tuple{x})=h^{(d)}(\tuple{y})) と仮定する。

モノイド準同型性により
(h(E[\tuple{x}])=h(E[\tuple{y}])) である。
(K(G_0)=h^{-1}(S)) なので、
(E[\tuple{x}]\in K(G_0)) と
(E[\tuple{y}]\in K(G_0)) は同値である。

これはすべての文コンテクスト (E) について成り立つから、
(\D_{K(G_0)}^{(d)}(\tuple{x})
=\D_{K(G_0)}^{(d)}(\tuple{y})) である。
よって (K(G_0)) は、すべての (f\ge 1) に対して
((f,h))-タプル代入可能である。

\paragraph{第5段階：(G_0) が普遍的でない場合。}
(L(G_0)\ne\Sigma^*) と仮定し、
(z\in\Sigma^*\setminus L(G_0)) をとる。
(u:=X)、(v:=Y)、(E:=\blank_1)、および
(F:=\blank_1\beta z) とおく。
このとき (h(u)=h(v)) である。

まず、(E[u]=X\in K(G_0)) である。
実際、(h(X)=h(Y)\in h(Y)H_0=S) なので
(X\in K_{\mathrm{pos}}) であり、さらに
(X\notin Y\Sigma^*) である。

また、(\eps\in L'*{G_0}) なので
(E[v]=Y=Y\eps\in YL'*{G_0}\subseteq K(G_0)) である。
したがって、(u) と (v) は受理文コンテクスト (E) を共有する。

次に、(F[u]=X\beta z\in K(G_0)) である。
実際、(h(X\beta z)=h(Y\beta z)\in h(Y)H_0=S) なので
(X\beta z\in K_{\mathrm{pos}}) であり、
(X\beta z\notin Y\Sigma^*) である。

一方、(F[v]=Y\beta z\notin K(G_0)) である。
この語は (Y\Sigma^*) に属するので式~\textup{(2)} の第1項から除かれる。
また、(z\notin L(G_0)) から
(\beta z\notin L'_{G_0}) であるため、第2項にも属さない。

以上により、
[
h(u)=h(v),\qquad
E[u],E[v],F[u]\in K(G_0),\qquad
F[v]\notin K(G_0).
\tag{3}
]

したがって、(u) と (v) は同じ (h)-型をもち、
受理文コンテクストを一つ共有するが、それらの文コンテクスト分布は異なる。
よって (K(G_0)) は ((1,h))-タプル代入可能ではない。

アリティ一の反例は、任意の固定 (f\ge 1) に対する
((f,h))-タプル代入可能性も否定する。

\paragraph{第6段階：帰着の結論。}
第4段階と第5段階により、任意の固定 (f\ge 1) について
[
L(G_0)=\Sigma^*
\quad\Longleftrightarrow\quad
K(G_0)\text{ は }(f,h)\text{-タプル代入可能である}.
\tag{4}
]

しかも、(G_0) から (K(G_0)) の CFG への変換は実効的である。
したがって、固定アルファベット上の CFG 普遍性問題から
(\mathsf{Sub}^{\mathrm{CFG}}*{f,h}) への多対一帰着が存在する。
よって (\mathsf{Sub}^{\mathrm{CFG}}*{f,h}) は
(\Pi^0_1)-困難である。

\paragraph{第7段階：co-r.e. への所属。}
CFG (G) の言語 (L(G)) が
((f,h))-タプル代入可能でないことは、ある
(1\le d\le f)、タプル
(\tuple{x},\tuple{y}\in(\Sigma^*)^d)、およびアリティ (d) の
文コンテクスト (E,F) が存在して、
[
...
となることと同値である。
分布の差が逆向きに現れる場合には、
(\tuple{x}) と (\tuple{y}) を交換すればよい。

有限タプルと有限文コンテクストは実効的に列挙でき、
CFG の所属問題は決定可能である。
したがって、候補
(d,\tuple{x},\tuple{y},E,F) を順に列挙して
式~\textup{(5)} を検査すれば、代入可能性が失敗している場合には、
いつか有限証人を発見できる。

よって (\mathsf{Sub}^{\mathrm{CFG}}*{f,h}) の補集合は r.e. であり、
(\mathsf{Sub}^{\mathrm{CFG}}*{f,h}) は co-r.e. に属する。
すでに示した (\Pi^0_1)-困難性と合わせると、
(\mathsf{Sub}^{\mathrm{CFG}}_{f,h}) は co-r.e.-完全、
すなわち (\Pi^0_1)-完全である。
\end{proof}

\begin{remark}[仮定が意味論的である理由]
\label{rem:promise-undecidable}
定理~\ref{thm:promise-undecidable} は、
注意~\ref{rem:semantic-class} で述べた意味論的非実効性を
計算論的に精密化するものである。

正準学習器 (K\mapsto\widehat{G}_0(K)) 自体は、
任意の有限正例標本 (K) に対して全域的かつ計算可能に定義されている。
また、その仮説は観測済みの正例を生成するという意味で整合的である。

決定不能なのは学習器の実行ではなく、与えられた CFG 表示 (G) から、
その生成言語 (L(G)) が厳密再構成定理の意味論的仮定を満たすかを
事前に判定する問題である。

代入可能性の失敗には、式~\textup{(5)} の形の有限証人が存在する。
したがって、失敗は半決定可能である。
これに対して、すべてのタプルとすべての文コンテクストについて
反例が存在しないことを、一般に有限時間で確認する方法は存在しない。

したがって、本論文の厳密再構成定理は、未知の対象言語が実際に
((f,h))-タプル代入可能であるならば、正準学習器が、
対象に応じた有限特徴標本を含む十分な正例標本から対象言語を
厳密に再構成する、という条件付き定理として理解されなければならない。

本定理は、一般の CFG 表示を入力とする意味論的判定問題について述べている。
本付録では、入力文法を本論文の厳密な二分・線形・非消去・
開始記号分離型の作業形に制限した場合にも、同じ完全性分類が成り立つとは
別途主張しない。

そのような制限付き入力問題へ移すには、意味論的性質と表示上の制限を
同時に保存する追加の実効的帰着が必要である。
\end{remark}
