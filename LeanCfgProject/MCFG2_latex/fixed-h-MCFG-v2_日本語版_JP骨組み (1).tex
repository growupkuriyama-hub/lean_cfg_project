...
% 見出し（\sectionや\subsection）前後の余白を極限まで削る設定
\titlespacing*{\section}{0pt}{1ex plus 0.2ex minus 0.2ex}{0.5ex plus 0.1ex}
\titlespacing*{\subsection}{0pt}{0.5ex plus 0.2ex minus 0.2ex}{0.2ex plus 0.1ex}
...
\title{\vspace{-1.5cm}固定有限モノイド観測の下での作業形 MCFG の正例データ学習}
\author{栗山 貴之}
\date{\vspace{-5ex}}
...
\begin{abstract}
本論文では、ファンアウトが有界な作業形多重文脈自由文法の意味論的部分クラスに対して、
助言相対的な正例データ学習定理を証明する。ここで助言とは、学習開始前に与えられ、
テキストから推論されることのない、固定された明示的有限モノイド準同型
\(h:\Sigma^*\to M\) である。

簡約された二分・線形・非消去の作業形 MCFG 表示に対して、名前付き文コンテクストによって
露出されるタプルに関する分布的代入可能性の有限観測版として、
\((f,h)\)-タプル代入可能性を定義する。任意の固定ファンアウト上界 \(f\) と、
任意の固定された明示的有限準同型 \(h\) に対して、標準的な集合駆動学習器を構成する。
対象言語がこのような作業形表示をもち、意味論的な \((f,h)\)-代入可能性の仮定を満たすならば、
この学習器は表示相対的な有限特徴標本をもち、正例データから対象言語を厳密に同定する。
固定された \((f,h)\) の下で、有限標本 \(K\) から構成される仮説は、
時間 \(\|K\|_+^{O(f)}\) で構成可能である。

固定観測は本質的である。有界な有限観測族は一つの直積準同型へコンパイルできる一方、
すべての有限観測準同型にわたる非有界和集合は、正例データから同定できない。
さらに、コピー言語があらゆる有限観測ファイバーから排除されることを含む、
有限観測による排除基準を与える。したがって本結果は MCFL 全体を学習するものではない。
むしろ、弱文脈依存言語の内部に、固定観測かつ助言相対的に学習可能な頑健な領域を切り出す。
\end{abstract}
...
\cite{Gold1967}。正例データからの学習可能性を回復する有効な方法の一つは、
分布上の制約を課すことである。すなわち、観測された部分文字列、あるいはより一般には
観測されたタプルは、それらを受理するコンテクストが十分に制御された仕方で一致するとき、
安全に置換できると考える。この見方は、Clark と Eyraud の代入可能文脈自由言語
\cite{ClarkEyraud2007}、Yoshinaka による弱文脈依存言語の多次元代入可能性に関する結果
\cite{Yoshinaka2008,Yoshinaka2009,Yoshinaka2011}、さらに並列文脈自由文法および
多重文脈自由文法の分布学習に関する後続研究
\cite{ClarkYoshinaka2014,ClarkYoshinaka2016} の基礎にある。
これに関連する正例データ学習可能性は、制限された範疇文法にも現れる
\cite{Kanazawa1998}。MCFG および関連形式体系を動機づける弱文脈依存文法の背景については、
\cite{Shieber1985,VijayShankerWeirJoshi1987,Weir1988,Kallmeyer2010} を参照されたい。

本論文は、この考え方の型付き版を、ファンアウト有界な作業形多重文脈自由文法（MCFG）に
対して研究する。本結果は、文脈自由言語に対する固定有限モノイド型付き分布学習
...
である。したがって学習器は助言相対的である。学習器は正例から対象言語を学習するが、
観測インターフェース自体はあらかじめ固定されている。すべての結果は、ファンアウトが有界で、
簡約された二分・線形・非消去の作業形 MCFG 表示をもつ言語について述べる。
任意の MCFG に対する正規形定理を用いることも、主張することもない。

\paragraph{何を学習し、何を学習しないのか。}
学習器は、対象言語の正例と固定有限準同型 \(h\) を受け取る。対象文法、導出オラクル、
負例、所属質問、あるいは決定可能な入力としての意味論的仮定は受け取らない。
定理が述べるのは、未知の対象が固定ファイバー \(\Cmcf{f}{h}\) に属するならば、
有限個の正例標本だけで標準的集合駆動学習器が厳密な対象言語へ安定する、ということである。
正例データから \(h\) 自体を発見できるとは述べない。また、任意の MCFG が正例データから
同定可能であるとも述べない。
...
少なくとも一つの受理文コンテクストを共有するときに限られる。意味論的仮定は、そのとき
すべての受理コンテクストが一致することを要求する。これは対象言語に対する意味論的仮定であり、
入力文法に対する決定可能な構文論的仮定ではない。

本論文の主要な貢献は次のとおりである。
\begin{enumerate}[label=(\roman*),leftmargin=*]
\item 簡約された二分・線形・非消去の作業形 MCFG 表示をもち、意味論的な
\((f,h)\)-タプル代入可能性を満たす言語に対して、固定観測ファイバー
\(\Cmcf{f}{h}\) を定義する。

\item 各固定 \(f\) と固定された明示的有限準同型 \(h\) に対して、標準的正例データ学習器を
構成し、表示相対的な有限特徴標本からの厳密再構成を証明する。

\item 有限観測が真正の助言であることを証明する。有界な観測族は一つの直積準同型へ
コンパイルできるが、すべての有限準同型にわたる非有界和集合は正例データから同定できない。

\item コピー言語を含む有限観測による排除基準と例を与え、固定観測ファイバーが MCFL 階層全体の
言い換えではなく、MCFL の真の助言相対的部分領域をなすことを示す。
\end{enumerate}

以下の表は、主要命題がどこで用いられるかを要約する。
\begin{center}
\begin{tabular}{|p{0.26\textwidth}|p{0.64\textwidth}|}
\hline
命題 & 役割 \\
\hline
定義~\ref{def:tuple-substitutable} および~\ref{def:class-cmcf} & 固定観測ファイバー \(\Cmcf{f}{h}\) と、その意味論的対象仮定。 \\
\hline
系~\ref{cor:gold} & 標準的学習器が、各固定ファイバー \(\Cmcf{f}{h}\) を正例データから同定する。 \\
\hline
定理~\ref{thm:poly} & 固定 \((f,h)\) に対して、有限標本 \(K\) から得られる仮説が時間 \(\|K\|_+^{O(f)}\) で構成可能である。 \\
\hline
定理~\ref{thm:no-advice-nonidentifiability} および~\ref{thm:bounded-union-identifiable} & 観測準同型が真正の助言であること。有界サイズの族は一つの直積準同型へコンパイルできるが、非有界な無助言和集合は同定不能である。 \\
\hline
定理~\ref{thm:member-kernel-exclusion} および命題~\ref{prop:copy-language-outside-every-finite-observation} & 有限観測ファイバーは標準的 MCFL 全体を尽くさない。 \\
\hline
\end{tabular}
\end{center}

コピー言語の例は、境界を示す指標としてとりわけ有用である。これは、固定観測条件が単なる
MCFL 階層の別表示ではないことを示す。実際、標準的なファンアウト 2 の MCFL でさえ、
無限個の要素分布をもち、その結果としてあらゆる有限観測仮定に失敗しうる。

第~\ref{sec:prelim} 節では学習論的および文法論的定義を与える。
第~\ref{sec:running-examples} 節では標準的な同期例を記す。
第~\ref{sec:learner} 節と第~\ref{sec:exact} 節では学習器を定義し、厳密同定を証明する。
第~\ref{sec:poly-time} 節では固定パラメータ多項式構成上界を述べる。
第~\ref{sec:fixed-observation} 節では、有界／非有界の観測助言境界と要素核による排除を証明する。
第~\ref{sec:comparison} 節では、本結果を型なし分布条件と比較する。
...
二分規則の意味論は同時代入である。
\(\tuple u\in(\Sigma^*)^{d_B}\) および \(\tuple v\in(\Sigma^*)^{d_C}\) のとき、
\(\rho(\tuple u,\tuple v)\) は、テンプレートタプル中の各 \(x^i\) を \(u_i\) に、
各 \(y^j\) を \(v_j\) に置き換えて得られる。
非終端記号 \(A\) のタプル言語を \(L_A(G)\) と書き、\(L(G):=L_S(G)\) とする。
すべての非終端記号が \(S\) から到達可能で、かつ生成的であるとき、文法は\emph{簡約されている}
という。
...
空のタプル成分が標本語の内部で観測されるとき、それを、局所的な同順位順序を固定した切断位置で
表現する。これは具体的出現を列挙するための記帳にすぎない。

\begin{definition}[タプル分布]
\label{def:tuple-distribution}
\(L\subseteq\Sigma^*\) および \(\tuple x\in(\Sigma^*)^d\) に対して、
\[
\D_L^{(d)}(\tuple x):=
...
\begin{remark}[意味論的対象クラス]
\label{rem:semantic-class}
\(\Cmcf{f}{h}\) への所属は意味論的条件であり、入力文法について実効的に認識可能な仮定ではない。
証人は、作業形 MCFG 表示と、その言語が \((f,h)\)-タプル代入可能性を満たすという事実からなる。
後者が表示から決定可能であるとは主張しない。失敗は半決定可能である。実際、
\(h^{(d)}(\tuple x)=h^{(d)}(\tuple y)\)、
\(E[\tuple x],E[\tuple y],F[\tuple x]\in L(G)\)、
\(F[\tuple y]\notin L(G)\) を満たすタプル \(\tuple x,\tuple y\) と文コンテクスト \(E,F\) を
列挙できる。MCFG の所属問題は決定可能なので、そのような有限の失敗証拠は検証できる。
この意味論的な非実効性は、極限同定にとって問題にならない。定理が主張するのは、
仮定を満たす各対象が、固定学習器に対して有限特徴標本をもつことである。
仮定が失敗する場合、厳密再構成定理と同定定理は正しさを何も主張しない。
それでも学習器は任意の有限標本上で定義されており、集合駆動整合性
\(K\subseteq L(\widehat G_0(K))\) は仮定と無関係に成立する。
\((f,h)\)-タプル代入可能性なしに失敗しうるのは、観測標本を越えた健全な外挿である。
\end{remark}
...
\begin{lemma}[ゾーン準同型]
\label{lem:zone-morphism-basic-v10}
...
\label{prop:equality-block-substitutable-v10}
任意の \(m\ge2\) に対して、言語
\[
...
\]
ここで \(n,n'\ge1\) である。同じ外側コンテクストが供給する終端記号部分を比較すると、
各 \(1\le j\le m\) に対して
\[
  \sum_i |y_i|_{a_j}-\sum_i |x_i|_{a_j}=n'-n .             \tag{1}
\]
を得る。次に \(F\) を \(\tuple x\) の任意の受理コンテクストとし、
\(F[\tuple x]=a_1^N\cdots a_m^N\)（\(N\ge1\)）とする。
成分ごとの \(h_m\)-型が一致するので、語全体を充填した結果も同じ \(h_m\)-値をもつ。
補題~\ref{lem:zone-morphism-basic-v10} により \(F[\tuple y]\in R_m\) である。
式 (1) により、\(F[\tuple y]\) におけるすべての文字数は
\(N+(n'-n)\) に等しい。この共通値は 0 にはなりえない。もし 0 なら
\(F[\tuple y]=\eps\) であり、
\(h_m(F[\tuple x])=h_m(F[\tuple y])=h_m(\eps)\) となって、
補題~\ref{lem:zone-morphism-basic-v10} に矛盾する。
したがって \(F[\tuple y]\in E_m^+\) である。逆向きも対称的に従うため、
二つのタプル分布は等しい。
\end{proof}

\begin{lemma}[ブロック等式包絡]
\label{lem:block-equality-envelopes-v10}
\(R\) を、\(a^+\#b^+\#c^+\#d^+\) や \(a^+b^+c^+d^+\) のように、
固定された順序付き可視ブロック列上の正則ブロック包絡とし、\(h_R\) をその標準 DFA の
遷移準同型とする。可視ブロック長変数の間の等式からなる任意の有限集合 \(\Lambda\) に対して、
言語
\[
  L_\Lambda=\{w\in R\mid
  \text{\(w\) の可視ブロック長が \(\Lambda\) のすべての方程式を満たす}\}
\]
は、任意のアリティ \(r\ge1\) について \((r,h_R)\)-タプル代入可能である。
\end{lemma}

\begin{proof}
成分ごとに同じ \(h_R\)-型をもつタプル \(\tuple x\) と \(\tuple y\) が、
受理コンテクスト \(E\) を共有し、両方の充填結果が \(L_\Lambda\) に属すると仮定する。
各可視ブロック文字 \(\sigma\) について、\(E\) の外側終端記号による寄与は二つの充填で同じである。
したがって、\(\Lambda\) 中の各等式 \(|\sigma|=|\tau|\) から
\[
  \sum_i |y_i|_\sigma-\sum_i |x_i|_\sigma
  =
  \sum_i |y_i|_\tau-\sum_i |x_i|_\tau .                  \tag{2}
\]
を得る。次に、\(F\) を \(\tuple x\) の任意の受理コンテクストとする。
成分ごとの \(h_R\)-型が等しいので
\(h_R(F[\tuple x])=h_R(F[\tuple y])\) であり、したがって \(F[\tuple y]\in R\) である。
\(\Lambda\) の各方程式に (2) を適用すると、\(\tuple x\) を \(\tuple y\) に置き換えた後も、
必要なブロック長等式はすべて保たれる。ゆえに \(F[\tuple y]\in L_\Lambda\) である。
対称性により、タプル分布の等しさが従う。
\end{proof}
...
意味論的仮定は、命題~\ref{prop:equality-block-substitutable-v10} において
...
意味論的仮定は、補題~\ref{lem:block-equality-envelopes-v10} において
\(R=P\) および
\(\Lambda=\{|a|=|b|,\ |b|=|c|,\ |c|=|d|\}\) とすれば従う。
\end{proof}
...
意味論的仮定は、補題~\ref{lem:block-equality-envelopes-v10} において
\(R=Q\) および \(\Lambda=\{|a|=|c|,\ |b|=|d|\}\) とすれば従う。
\end{proof}

% ================================================================
% ここまで日本語化済み（原稿冒頭から Examples 節末まで、省略なし）。
% 以下は次回以降に順次日本語化する原文部分。
% ================================================================
\section{正準学習器}
\label{sec:learner}

学習器が用いるのは、固定されたパラメータ \(f\) と \(h\)、および現在の有限な
正例標本だけである。対象を証言する表示は、有限な特徴標本が存在することを
示す証明の中でのみ用いられる。完全な文インターフェース型付けとは異なり、
以下の精密化が保存するのは成分ごとの出力 \(h\)-型だけである。再構成に必要な
具体的な露出文脈は、特徴標本によって与えられる。

\subsection{出力型精密化}
\label{sec:refinement}

再構成の証明では、証言文法の有限精密化を用いる。この精密化が記録するのは、
導出された各タプルの出力 \(h\)-型だけである。これは意図的に、完全な
文インターフェース精密化より弱い。完全な精密化であれば、周囲の文の中へ
タプル成分が配置される順序や、境界区間の \(h\)-値も記録することになる。
ここでこの軽量な精密化だけで十分なのは、学習器の単位規則が、抽象的な
インターフェース適合性ではなく、標本中で実際に共有される文脈によって
保護されているからである。証明では、必要な周辺情報を非終端記号の型そのもの
に保存する代わりに、露出文脈 \(\chi(X)\) を介して有限特徴標本中に保存する。

\begin{definition}[テンプレート評価]
\label{def:output-map}
\leavevmode\par\noindent
\(\mu(B)=d_B\)、\(\mu(C)=d_C\) である二分規則
\(\rho\colon A\to(\alpha_1,\ldots,\alpha_e)(B,C)\) をとる。
\(\mathbf q\in M^{d_B}\)、\(\mathbf r\in M^{d_C}\) に対し、テンプレート語
\(\beta\) の評価 \(\ev_\rho(\beta;\mathbf q,\mathbf r)\in M\) を、各終端記号
\(a\) を \(h(a)\) に、各 \(x^i\) を \(q_i\) に、各 \(y^j\) を \(r_j\) に
置き換え、出現順に積をとることによって定義する。さらに
\[
\begin{aligned}
\out_\rho(\mathbf q,\mathbf r)
:={}&\bigl(\ev_\rho(\alpha_1;\mathbf q,\mathbf r),\ldots,\\
&\ev_\rho(\alpha_e;\mathbf q,\mathbf r)\bigr)\in M^e
\end{aligned}
\]
と定義する。
\end{definition}
...
\(G^h\) の任意の非終端記号 \(A_{\mathbf p}\) について、次が成り立つ。
\begin{enumerate}[label=(\roman*),leftmargin=*]
\item \(G^h\) において \(A_{\mathbf p}\Rightarrow^*\tuple u\) ならば、
\(h^{(\mu(A))}(\tuple u)=\mathbf p\) であり、型添字を消去すると \(A\) からの
正しい \(G\)-導出が得られる。
\item 根が開始記号でない非終端記号である任意の \(G\)-導出木は、各節点に、
その節点で導出されるタプルの成分ごとの \(h\)-型を付すことによって、一意に
\(G^h\)-導出木へ持ち上がる。
\item \(L(G^h)=L(G)\) であり、\(\widetilde G_0\) へのトリミングは文字列言語を
保存する。
\end{enumerate}
\end{proposition}

\begin{proof}
(i) は導出に関する帰納法による。終端規則の場合は直ちに従う。二分規則の場合、
帰納法の仮定から子の出力型 \(\mathbf q\) と \(\mathbf r\) が得られ、親の
出力型は \(h\) の準同型性により、ちょうど
\(\out_\rho(\mathbf q,\mathbf r)\) である。添字を消去すれば、基礎となる
\(G\) の規則が得られる。(ii) では、各節点に、その節点で導出されるタプルの
\(h\)-型を付す。すると定義~\ref{def:output-refinement} により、各内部節点に
必要な型付き規則がちょうど存在する。(iii) の二つの包含は (i) と (ii) から
従い、トリミングで除かれるのは、いかなる成功導出にも現れない対象だけである。
\end{proof}

\(\widetilde G_0\) に残る型付き非終端記号 \(X=A_{\mathbf p}\) に対し、その
タプル言語を \(L_X\) と書く。タプルおよび具体的な文コンテクストの上に、長さ辞書式順序
を精密化する全有効順序を一つ固定する。\(L_X\) における最小のタプルを
\(\omega(X)\) とする。

\begin{lemma}[具体的な露出文脈]
\label{lem:exposing-contexts}
\(\widetilde G_0\) に残る任意の型付き非終端記号
\(X=A_{\mathbf p}\) に対し、すべての \(\tuple u\in L_X\) について
\(E_X[\tuple u]\in L(G)\) を満たす、具体的なアリティ \(\mu(A)\) の文コンテクスト
\(E_X\) が存在する。\(X\) が型付き開始規則 \(\widetilde S\to X\) の子として
現れるなら、単項文脈 \(\blank_1\) を選ぶことができる。
\end{lemma}

\begin{proof}
\(X\) はトリミング後にも残っているので、\(\widetilde G_0\) のある成功導出木の
ある節点に現れる。すべての型添字を消去し、同じ節点を、成功した \(G\)-導出中の
\(A\) の出現とみなす。その導出の残りの部分は、この出現で導出されたタプルを
囲む具体的な文コンテクスト \(E_X\) を定める。\(\tuple u\in L_X\) なら、その節点に
\(A\) から \(\tuple u\) への導出を代入しても、基礎文法 \(G\) には依然として
正しい成功導出が存在する。したがって \(E_X[\tuple u]\in L(G)\) である。
\(X\) が開始記号から直接到達される場合、外側の文脈は \(\blank_1\) である。
\end{proof}
...
開始規則の子 \(X\) に対しては、慣約として \(\chi(X)=\blank_1\) を選ぶ。
\end{definition}

\subsection{特徴標本}
\label{subsec:learner-grammar}

\begin{definition}[証言精密化の表示相対的特徴標本]
\label{def:cs}
...
表示相対的標本 \(\CS(\widetilde G_0)\) は、選ばれた証言表示 \(G\) と、その
...
\(w\in K\) におけるアリティ \(d\) のタプル出現とは、\(E\) が具体的な
アリティ \(d\) の文コンテクスト、\(\tuple x\in(\Sigma^*)^d\)、かつ
\(E[\tuple x]=w\) であるような対 \((E,\tuple x)\) をいう。同値な言い方を
すれば、\(w\) の中で名前付きスロットを \(d\) 個選ぶ。正の長さをもつスロットは
区間であり、空スロットは切断位置である。同じ切断位置を複数の空スロットが共有する
場合には、局所的な同順位順序を付す。
\end{definition}

\begin{example}[空スロットと局所的同順位順序]
...
同順位順序は、\(\blank_2<\blank_3\) か \(\blank_3<\blank_2\) かを記録する。
...
\(K\) における二分証人は、親タプル出現 \((E,\tuple z)\)、ただし
\(\tuple z=(z_1,\ldots,z_e)\)、二つの観測タプル
\(\tuple x=(x_1,\ldots,x_{d_B})\) と
\(\tuple y=(y_1,\ldots,y_{d_C})\)、および各親成分 \(z_i\) の分割からなる。
その分割では、終端記号からなる隙間と、
\(\{x^1,\ldots,x^{d_B},y^1,\ldots,y^{d_C}\}\) の互いに異なる変数で
ラベル付けされた変数区間とに分け、各変数を全体でちょうど一度用いる。また、
\(x^j\) でラベル付けされた区間はちょうど \(x_j\)、\(y^k\) でラベル付けされた
区間はちょうど \(y_k\) でなければならない。空の子成分は、
定義~\ref{def:tuple-occurrence} の同順位順序規約を付した長さゼロの変数区間として
表す。この分割は、\(\tuple z=\rho(\tuple x,\tuple y)\) を満たす具体的な
二分・線形・非消去テンプレート \(\rho\) を定める。
\end{definition}

\begin{lemma}[誘導される二分テンプレートの一意性]
\label{lem:unique-induced-template}
具体的な親タプル出現と、その成分内にある具体的な子出現を固定し、長さゼロの
子成分には定義~\ref{def:tuple-occurrence} の局所的同順位順序を付す。このとき、
定義~\ref{def:binary-witness} の順序付き分割は、線形・非消去の二分テンプレート
\(\rho\) を一意に定め、親タプルはちょうど \(\rho(\tuple x,\tuple y)\) である。
\end{lemma}

\begin{proof}
各親成分を左から右へ読む。終端記号からなる隙間は終端文字列として記録し、印を付けた
各子区間は対応する変数として記録する。複数の長さゼロの区間が同じ切断位置を共有する
場合でも、局所的同順位順序によってこの読み取りは全順序になる。各子成分はちょうど
一度だけ印を付けられるので、得られる成分テンプレートのタプルでは各変数がちょうど
一度現れる。記録された子文字列を代入すると、構成上もとの親成分が再構成されるので、
誘導テンプレートは一意である。
\end{proof}
...
(ii) について、\(\tuple z_R:=\rho(\omega(Y),\omega(Z))\) とおく。特徴標本は
\(\chi(X)[\omega(X)]\) と \(\chi(X)[\tuple z_R]\) を含み、両者は同じ具体的文コンテクスト
\(\chi(X)\) を共有する。両タプルは \(L_X\) に属するので、
命題~\ref{prop:output-invariants} により
\(h(\omega(X))=h(\tuple z_R)\) が成分ごとに成り立つ。よって学習器は単位規則
\([\omega(X)]\to[\tuple z_R]\) をもつ。同じ標本語
\(\chi(X)[\tuple z_R]\) は、親出現 \((\chi(X),\tuple z_R)\)、子タプル
\(\omega(Y)\)、\(\omega(Z)\)、テンプレート \(\rho\) をもつ二分証人を与える。
特徴標本は \(\chi(Y)[\omega(Y)]\) と \(\chi(Z)[\omega(Z)]\) を含むので、
子タプルは観測されている。したがって学習器は二分規則
\([\tuple z_R]\to\rho([\omega(Y)],[\omega(Z)])\) をもつ。
...
\(\widetilde G_0\) の成功導出へ持ち上がる。そのような導出中の各型付き非終端記号
\(X\) を \(\eta(X)=[\omega(X)]\) に置き換える。補題~\ref{lem:rule-simulation}
により、型付き導出の各終端ステップ、二分ステップ、開始ステップは、学習器文法中の
有限導出区間によってシミュレートされる。したがって、\(L(G)\) のすべての語は
...
\begin{proof}
各対象 \(L\in\Cmcf{f}{h}\) に対し、証言文法 \(G\) と、その有限特徴標本
\(\CS(\widetilde G_0)\) を選ぶ。定理~\ref{thm:exact-reconstruction} に
補題~\ref{lem:sufficient-to-gold} を適用する。
\end{proof}

\begin{remark}[有界規則ランク]
\label{rem:bounded-rule-rank}
二分規則という規約は、記法を読みやすく保つためだけに用いられている。固定された各
規則ランク上界 \(r\) に対し、同じ構成と証明は、規則ランクが高々 \(r\) である
線形・非消去の作業形表示へ拡張できる。二分証人規則を、\(k\le r\) である
\(k\)-項証人付き規則 \(A\to\rho(B_1,\ldots,B_k)\) に置き換える。具体的な
親タプルを、終端記号からなる隙間と、\(k\) 個の子タプルの成分とに分割する。
線形性により、誘導テンプレートの一意性は同様に成り立つ。充填恒等式と合成の証明は、
残りの \(k-1\) 個の子タプルを周囲の文コンテクストの一部として固定し、子を一つずつ置換する
ことによって適用する。特徴標本には、型付きの各 \(k\)-項規則について、対応する
充填済み規則証人を含める。したがって固定された \(f,h,r\) に対し、学習器はクラスの
ランク \(r\) 版を同定し、明示的構成は、指数が \(f\) と \(r\) に依存する標本サイズ
の多項式時間で実行できる。
\end{remark}
...
二分証人規則については、親出現、アリティが高々 \(f\) の二つの子出現、および親成分を
終端記号の間隙と高々 \(2f\) 個の変数区間へ分割する方法を列挙する。\(f\) は固定されて
いるので、端点、切断位置、同順位順序の選び方の総数は \(n^{O(f)}\) である。候補となる
...
次の定理は、本論文を通じて観測準同型を固定している理由を説明する。学習器に、正例データ
から適切な有限合同を発見することは要求しない。すべての有限観測準同型にわたる和集合へ
クラスを拡大すると、ファンアウト一の場合ですでに有限告知標本が存在しなくなる。

系~\ref{cor:gold} において、固定準同型 \(h\) は学習問題の一部である。本節では、これが
単なる表示上の便宜ではないことを示す小さな不可能性結果を記す。準同型が与えられず、
すべての有限観測準同型にわたる和集合を取ると、正例データからの同定は失敗する。
...
以下では、Angluin による正例データ同定可能性の有限告知標本による特徴づけ
\cite{Angluin1980} の必要条件にあたる、初等的な告知標本障害を用いる。あるクラスが
言語列 \(L_1\subsetneq L_2\subsetneq\cdots\) と
\(L_\infty=\bigcup_k L_k\) をともに含むならば、\(L_\infty\) はその部分クラス内で
有限告知標本をもたない。実際、\(L_\infty\) の任意の有限部分集合は、すでにある真の
部分言語 \(L_k\) に含まれる。したがって、この超有限部分クラスを含む任意のクラスは、
正例データから同定できない。
...
\begin{proof}
同定不可能な部分クラスを示せば十分である。補題~\ref{lem:limit-ab-substitutable} により
\(L_*\in\Cmcf{f}{h_*}\) である。補題~\ref{lem:finite-approximants-substitutable} により、各 \(L_k\) は
ある明示的有限準同型 \(h_k\) に対して \(L_k\in\Cmcf{f}{h_k}\) である。したがって
\[
  \mathcal S:=\{L_*\}\cup\{L_k\mid k\ge1\}\subseteq\Cmcf{f}{\bullet}.
\]
さらに、\(L_1\subsetneq L_2\subsetneq\cdots\) かつ \(\bigcup_k L_k=L_*\) である。
任意の有限集合 \(T\subseteq L_*\) は、ある真の近似 \(L_k\subsetneq L_*\) に含まれるため、
\(\mathcal S\) 内で \(L_*\) の告知標本にはなりえない。有限告知標本障害により、
\(\mathcal S\) は正例データから同定できない。したがって \(\mathcal S\) を含むクラスはすべて
同定不可能であり、とくに \(\Cmcf{f}{\bullet}\) は同定不可能である。
\end{proof}
...
この包含は真に狭くなりうる。\(L_3=\{a^n b^n c^n\mid n\ge1\}\) とし、\(h_R\) を
...
\(\Cmcf{f}{h}\) は、正準的なタプル列挙学習器により正例データから極限同定可能である。この構成は
助言相対的である。有限観測そのものは学習されず、意味論的代入可能性は対象言語の仮定として
置かれる。それでもこの助言は有界直積の下で安定である一方、すべての有限観測にわたる非有界和集合は
正例データから同定できない。

得られた全体像は、意図的に中間的なものである。固定有限観測は、文脈自由文法に対する分布学習を
弱文脈依存の設定へ拡張し、三ブロック一致、並列一致、交差直列一致などの標準的同期例を含む。
同時に、メンバー核障害は、傾き和言語やコピー言語をすべての有限観測ファイバーから分離する。
したがって固定観測は、すべての MCFL を別の形で表示したものではなく、真に助言相対的な
学習可能領域を与える。
...
\((f,h)\)-タプル代入可能性についても同様である。さらに、肯定例の集合は
\(\Pi_1\)-完全である。
\end{theorem}

\begin{proof}
固定アルファベット \(\Sigma\) 上の文脈自由文法の普遍性問題から帰着する。この問題は
\(|\Sigma|\ge2\) のとき \(\Pi_1\)-完全である。\(M\) は有限なので、
\(|\Sigma|^N>|M|\) を満たす整数 \(N\) を選ぶ。長さ \(N\) の語の中には、
...
検査することで半決定可能である。したがって肯定例の集合は \(\Pi_1\)-完全である。
\end{proof}
