...
実効的に列挙できる。また、MCFG の所属問題は決定可能なので、
...
コンテクスト \(F\) による充填では、\(F\) がもともと含んでいる
終端記号部分は固定されており、\(\tuple x\) と \(\tuple y\) の
各成分は、それぞれ同じ位置に代入される。
さらに \(h_m\) はモノイド準同型なので、連結された語の
\(h_m\)-値は、各部分の \(h_m\)-値の積として計算できる。
したがって、各成分の \(h_m\)-値が一致することから
\[
...
\(R_m\) への所属は \(h_m\)-値だけで決まる。
したがって、\(F[\tuple x]\) と同じ \(h_m\)-値をもつ
\(F[\tuple y]\) も \(R_m\) に属する。すなわち、
\[
  F[\tuple y]\in R_m .
\]
...
の固定順序ブロック包絡とする。
また、\(\mathcal A_R\) を \(R\) を認識する完全 DFA とし、
\[
