...
固定された \(f\) と固定された明示的 \(h\) に対し、有限標本 \(K\) に対応する正準仮説は、
出力サイズを含めて時間 \(\|K\|_+^{O(f)}\) で構成可能である。これは手元の標本からの構成上界で
あり、それだけで対象クラス全体に対する多項式データ定理を意味するものではない。
\end{theorem}
...
\[
  ...
  \qquad (1\le \ell\le e)
\]
を規則 \(\rho\) の\emph{テンプレート成分}と呼び、そのタプル
\[
  \boldsymbol{\alpha}_\rho
  :=
  (\alpha_1,\ldots,\alpha_e)
  \in(\Gamma_\rho^*)^e
\]
...
タプル
\[
  \tuple u=(u_1,\ldots,u_{d_B})
  \in(\Sigma^*)^{d_B},
  \qquad
  \tuple v=(v_1,\ldots,v_{d_C})
  \in(\Sigma^*)^{d_C}
\]
に対して、写像
\[
  \sigma_{\tuple u,\tuple v}\colon
  \Gamma_\rho\to\Sigma^*
\]
を
\[
  \sigma_{\tuple u,\tuple v}(a):=a
  \qquad (a\in\Sigma),
\]
\[
  \sigma_{\tuple u,\tuple v}(x^i):=u_i
  \qquad (1\le i\le d_B),
\]
\[
  \sigma_{\tuple u,\tuple v}(y^j):=v_j
  \qquad (1\le j\le d_C)
\]
によって定める。
...
\[
  \widehat{\sigma}_{\tuple u,\tuple v}\colon
  \Gamma_\rho^*\to\Sigma^*
\]
とする。規則 \(\rho\) を
\(\tuple u,\tuple v\) に適用した結果を
\[
  \rho(\tuple u,\tuple v)
  :=
  \bigl(
...
\[
  L_A(G)\subseteq(\Sigma^*)^{\mu(A)}
\]
は、次の条件を満たす最小の族として定義する。
\begin{enumerate}[label=(\roman*),leftmargin=*]
\item 終端規則 \(A\to(a)\) が存在するならば、
      \[
        (a)\in L_A(G).
      \]
...
\[
  L(G):=
  \{w\in\Sigma^*\mid (w)\in L_S(G)\}
\]
と定義する。開始記号のファンアウトは \(\mu(S)=1\) なので、
必要に応じて \(L_S(G)\) と \(L(G)\) を同一視する。

非終端記号 \(A\) が\emph{生成的}であるとは、
\[
  L_A(G)\ne\emptyset
\]
であることをいう。また、非終端記号依存グラフにおいて、
開始記号 \(S\) から \(A\) への有向路が存在するとき、
\(A\) は \(S\) から\emph{到達可能}であるという。
ここで、開始規則 \(S\to A\) に対して辺 \(S\to A\) を置き、
二分規則
\[
  A\to(\alpha_1,\ldots,\alpha_e)(B,C)
\]
に対して辺 \(A\to B\) および \(A\to C\) を置く。

すべての非終端記号が \(S\) から到達可能であり、かつ生成的であるとき、
文法 \(G\) は\emph{簡約されている}という。
...
言語 \(L\) がクラス \(\Cmcf{f}{h}\) に属するためには、次の二条件が
満たされなければならない。
\begin{enumerate}[label=(\roman*),leftmargin=*]
\item \(L\) が、ファンアウト高々 \(f\) の簡約された作業形 MCFG
      \(G\) によって \(L=L(G)\) と表示されること。
\item 言語 \(L(G)\) が \((f,h)\)-タプル代入可能であること。
\end{enumerate}

条件 (i) は具体的な文法表示に関する条件である。
これに対して条件 (ii) は、文法の規則の形ではなく、
生成される言語全体の文コンテクスト分布に関する条件である。
この意味で、\(\Cmcf{f}{h}\) は\emph{意味論的対象クラス}である。

本論文では、与えられた作業形 MCFG \(G\) から、
\(L(G)\) が \((f,h)\)-タプル代入可能であるかどうかを
常に決定できるとは主張しない。
ただし、代入可能性が\emph{失敗すること}は半決定可能である。
実際、失敗は有限個の語と文コンテクストによって証明できる。

具体的には、ある \(1\le d\le f\)、タプル$  \tuple x,\tuple y\in(\Sigma^*)^d$
およびアリティ \(d\) の文コンテクスト \(E,F\) が存在して、
\[
...
ここで、コンテクスト \(E\) は、\(\tuple x\) と \(\tuple y\) が
少なくとも一つの受理文コンテクストを共有することを示す。
一方、コンテクスト \(F\) は、
\[
  F\in
  \D_{L(G)}^{(d)}(\tuple x)
...
\[
  ...
  \D_{L(G)}^{(d)}(\tuple y),
\]
となり、定義~\ref{def:tuple-substitutable} の条件に反する。

有限のタプルと文コンテクストは、長さの短いものから順に
実効的に列挙できる。また、MCFG の所属問題は形式文法により多項式時間で決定可能なので、
各候補について上の所属・非所属条件を有限時間で検査できる。
したがって、失敗証拠が存在する場合には、探索によっていつか
それを発見できる。ただし、失敗証拠が存在しない場合に探索が
停止するとは限らないため、これは決定手続ではなく半決定手続である。

この意味論的な非実効性は、本論文の極限同定定理には支障を与えない。
学習器には、対象文法 \(G\) も、対象が
\((f,h)\)-タプル代入可能であるという証明も入力されない。
定理が主張するのは、未知の対象言語が実際に
\(\Cmcf{f}{h}\) に属しているならば、その対象に応じた有限特徴標本が
存在し、その特徴標本を含む十分な正例データを受け取った後には、
固定された学習器の仮説言語が対象言語と一致する、ということである。

対象言語が \((f,h)\)-タプル代入可能でない場合には、
本論文の厳密再構成定理および極限同定定理は適用できない。
したがって、学習器が有限正例標本 \(K\) から構成する仮説言語$  L(\widehat G_0(K))$
が、未知の対象言語と一致するとは保証されない。

ただし、学習器自体は、意味論的仮定が成り立つかどうかに
かかわらず、任意の有限正例標本 \(K\) に対して定義されている。
さらに、構成された仮説文法 \(\widehat G_0(K)\) は、
標本中のすべての語を生成する。したがって、$  K\subseteq L(\widehat G_0(K))$
が常に成り立つ。

つまり、学習器が観測済みの正例を仮説言語から取りこぼすことはない。
問題になりうるのは、標本 \(K\) にまだ現れていない語である。
具体的には、$  L(\widehat G_0(K))\setminus K$
に属する語の中に、実際の対象言語には属さない語が含まれる可能性がある。

したがって、\((f,h)\)-タプル代入可能性が保証するのは、
観測済みの正例を再現できることではなく、有限標本から未観測の語へ
仮説を拡張するときに、余分な語を生成しないという健全性である。
\end{remark}
...
以下ではたとえば$  \{a^n\#_2b^m\#_3c^n\#_4d^m\mid n\ge1,\, m\ge1\}$のような言語を一般化して取り扱う。
...
\begin{enumerate}[label=(\roman*),leftmargin=*]
\item 任意の \(A\in V\setminus\{S\}\)、
      \(\mathbf p\in M^{\mu(A)}\)、および
      \(\tuple u\in(\Sigma^*)^{\mu(A)}\) に対して、
      \[
        A_{\mathbf p}\Rightarrow_{G^h}^*\tuple u
      \]
      ならば、
      \[
        h^{(\mu(A))}(\tuple u)=\mathbf p.
      \]
      さらに、この \(G^h\)-導出木からすべての型添字を消去すると、
...
実際、二つの写像
\[
  h\circ\widehat{\sigma}_{\tuple u,\tuple v},
  \qquad
  \widehat{\eta}_{\mathbf q,\mathbf r}
  \colon
  \Gamma_\rho^*\to M
\]
はいずれもモノイド準同型である。さらに、任意の生成元について、
終端記号 \(a\in\Sigma\) ならば
\[
  h\bigl(\widehat{\sigma}_{\tuple u,\tuple v}(a)\bigr)
  =
  h(a)
  =
  \eta_{\mathbf q,\mathbf r}(a),
\]
変数 \(x^i\) ならば
\[
  h\bigl(\widehat{\sigma}_{\tuple u,\tuple v}(x^i)\bigr)
  =
  h(u_i)
  =
  q_i
  =
  \eta_{\mathbf q,\mathbf r}(x^i),
\]
変数 \(y^j\) ならば
\[
  h\bigl(\widehat{\sigma}_{\tuple u,\tuple v}(y^j)\bigr)
  =
  h(v_j)
  =
  r_j
...
二つの子部分木がそれぞれ
\[
  B_{\mathbf q}\Rightarrow_{G^h}^*\tuple u,
  \qquad
...
\[
  \rho\colon
  A_\nu\to
  (\alpha_1,\ldots,\alpha_e)(B,C)
\]
が用いられているとする。二つの子節点で導出されるタプルを
それぞれ \(\tuple u,\tuple v\) とし、その型を
\[
  \mathbf q:=h^{(\mu(B))}(\tuple u),
  \qquad
  \mathbf r:=h^{(\mu(C))}(\tuple v)
\]
とする。式~\textup{(1)} により、親節点で導出されるタプルの型は
\[
  \mathbf p_\nu
  =
  \out_\rho(\mathbf q,\mathbf r)
\]
である。したがって、\(G^h\) の定義により
\[
  (A_\nu)_{\mathbf p_\nu}
  \to
  \rho(B_{\mathbf q},C_{\mathbf r})
...
\(w\in L(G^h)\) とする。このとき、ある \(A\in V\setminus\{S\}\) と
\(p\in M\) が存在して、
\[
  \widetilde S\to A_{(p)}
\]
を根の規則とし、
\[
  A_{(p)}\Rightarrow_{G^h}^*(w)
\]
となる成功導出木が存在する。(i) により、その部分木から型添字を
消去すると
\[
  A\Rightarrow_G^*(w)
\]
が得られる。また、\(\widetilde S\to A_{(p)}\) は、もとの開始規則
\[
  S\to A
\]
から導入された規則である。したがって、
...
\(w\in L(G)\) とする。開始記号分離の仮定により、その成功導出木の
根では、ある開始規則
\[
  S\to A
\]
...
(ii) により、この部分木は
\[
  A_{(h(w))}\Rightarrow_{G^h}^*(w)
\]
という \(G^h\)-導出木へ一意に持ち上がる。
また、\(G^h\) には開始規則
\[
  \widetilde S\to A_{(h(w))}
\]
が存在する。したがって、
\[
  \widetilde S\Rightarrow_{G^h}^*(w),
\]
すなわち \(w\in L(G^h)\) である。以上より、
\[
  L(G^h)=L(G).
\]
...
したがって、同じ木 \(T\) は \(\widetilde G_0\) における成功導出木でも
あり、
\[
  w\in L(\widetilde G_0)
\]
である。よって、
\[
  L(G^h)\subseteq L(\widetilde G_0).
\]
以上から、
\[
  L(\widetilde G_0)=L(G^h)=L(G)
...
\(G\) を証人となる作業形表示とし、\(\widetilde G_0\) をそのトリムされた出力型精密化とする。
残っている各型付き非終端記号 \(X\) について、アンカータプル \(\omega(X)\) と、\(X\) から
導出されるすべてのタプルを受理する具体的露出文脈 \(\chi(X)\) を選ぶ。このとき、次のデータは、
正準学習器が \(\widetilde G_0\) のすべての規則をシミュレートするために十分である。
\begin{enumerate}[label=(\roman*),leftmargin=*]
\item 残っている各型付き記号の成分ごとの出力 \(h\)-型、
\item 残っている各型付き記号について一つの露出されたアンカー、
\item 残っている各型付き規則について一つの露出された充填例。
\end{enumerate}
学習器の非終端記号に、これ以外の文レベルのインターフェース標識を付す必要はない。
\end{proposition}

\begin{proof}
出力型により、型付き親アンカーと対応する充填済み規則タプルは、成分ごとに同じ \(h\)-値をもつ。
それらを露出する二つの標本語は同じ具体的文脈 \(\chi(X)\) を用いるため、学習器は両観測タプル値を
保護された単位規則で結ぶことができる。充填済み二分規則語は、親タプル内にある二つの子アンカーの
実際の変数区間を記録する。アンカー自体は、それぞれ自身の露出語を通じて観測されていてよい。
こうして得られる二分証人は、基礎となる線形テンプレートを再構成する。終端規則と開始規則は、
対応する露出された終端語とアンカー語によって処理される。規則ごとの形式的シミュレーションは
補題~\ref{lem:rule-simulation} で与える。
\end{proof}
...
有限標本 \(K\) における二分証人は、次のデータからなる。
\begin{enumerate}[label=(\roman*),leftmargin=*]
\item \(K\) のある語中の親タプル出現 \((E,\tuple z)\)。ここで
      \(\tuple z=(z_1,\ldots,z_e)\) とする。
\item 二つのタプル値
      \(\tuple x=(x_1,\ldots,x_{d_B})\) と
      \(\tuple y=(y_1,\ldots,y_{d_C})\)。各々は \(K\) のどこかで観測されているものとする。
      それらを証明する観測は、親出現と同じ標本語中に現れる必要はない。
\item 各親成分 \(z_i\) を、終端記号からなる隙間と、
      \(\{x^1,\ldots,x^{d_B},y^1,\ldots,y^{d_C}\}\) の互いに異なる変数でラベル付けされた
      変数区間とへ分ける順序付き分割。
\item 各変数は親タプル全体でちょうど一度用いられ、\(x^j\) でラベル付けされた区間の文字列値は
      ちょうど \(x_j\)、\(y^k\) でラベル付けされた区間の文字列値はちょうど \(y_k\) である。
\item 長さゼロの子成分は、指定された切断位置にある長さゼロの区間として表す。同じ切断位置を
      複数の長さゼロ区間が用いるとき、それらの局所順序も証人データの一部とする。
\end{enumerate}
各親成分について終端記号からなる隙間と変数ラベルを左から右へ読めば、
\(\tuple z=\rho(\tuple x,\tuple y)\) を満たす二分・線形・非消去テンプレート \(\rho\) が得られる。
\end{definition}
...
親タプル出現と、ラベル付き区間が指定された子成分文字列値をもつ順序付き分割を固定する。
長さゼロの区間に対する局所順序も含めれば、この分割は二分・線形・非消去テンプレート \(\rho\) を
一意に定め、親タプルはちょうど \(\rho(\tuple x,\tuple y)\) である。
\end{lemma}

\begin{proof}
各親成分を左から右へ読む。終端記号からなる隙間は終端文字列として記録し、印を付けた
各子区間は対応する変数として記録する。複数の長さゼロの区間が同じ切断位置を共有する
場合でも、局所順序によってこの読み取りは全順序になる。各子成分はちょうど
...
\(G\) を対象の証人表示とし、\(\widetilde G_0\) をそのトリムされた出力型精密化とする。
\(B_{\mathrm{exp}}(G,h)\) を、\(\widetilde G_0\) に残る型付き非終端記号、終端規則、二分規則に
対して用いる特徴標本語
\[
  \chi(X)[\omega(X)],\qquad
  \chi(X)[(a)],\qquad
...
\(N_{\mathrm{NT}}\) を \(\widetilde G_0\) に残る型付き非終端記号の個数、
\(N_{\mathrm{rule}}\) を残る開始規則以外の型付き規則の個数とする。このとき
\[
  |\CS(\widetilde G_0)|
...
\[
  ...
  B_{\mathrm{exp}}(G,h)
\]
である。したがって、選ばれた証人表示のサイズに関して \(N_{\mathrm{NT}}\)、
\(N_{\mathrm{rule}}\)、\(B_{\mathrm{exp}}(G,h)\) が多項式で抑えられる任意の部分クラスは、
多項式サイズの特徴標本をもつ。
\end{proposition}

\begin{proof}
定義~\ref{def:cs} は、残っている各型付き非終端記号について高々一つのアンカー語を、残っている
開始規則以外の各型付き規則について高々一つの充填語を加える。重複する語は個数を減らすだけである。
各語の正例サイズは高々 \(B_{\mathrm{exp}}(G,h)\) なので、二つの上界が得られる。
\end{proof}
