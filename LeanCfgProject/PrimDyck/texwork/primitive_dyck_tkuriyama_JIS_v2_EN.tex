...
integer it represents in base $m$.  Adjoining $0$ for convenience in additive
representations, set
\[
 \NpD=\{0\}\cup\{\val{w}{2}:w\in\DP\}.
\]
This convention does not affect the conclusions of the theorems below.
...
$C_n=\frac1{n+1}\binom{2n}{n}$\footnote{The grammar
...
\href{https://github.com/growupkuriyama-hub/lean_cfg_project/blob/3525cfb/LeanCfgProject/PrimDyck/A395858.py}{\path{A395858.py} at the fixed repository snapshot \texttt{3525cfb}}.
...
numeral, with $[\varepsilon]_4=0$.  For a positive integer $n$, let
$\operatorname{bin}(n)$ and $\operatorname{quad}(n)$ denote its standard binary
and base-$4$ expansions, respectively.
...
\begin{proof}
Suppose first that $p\in\Pset$.  Then
\[
 v:=4p\in\NpD,\qquad v\geq12.
\]
Write the binary expansion of $v$ as
\[
 b_1b_2\cdots b_{2L}\qquad(b_i\in\{0,1\}),
\]
and define its height function by
\[
 h(t):=\sum_{i=1}^{t}(2b_i-1)
 \qquad(0\leq t\leq2L).
\]
...
Because the primitive word has length at least four, its first pair is $11$
and its last pair is $00$: otherwise it would return to height $0$ after its
first two bits or before its last two bits.  Thus
\[
 d_1=3,\qquad d_L=0.
\]
...
\]
Dividing $v=4p$ by $4$ deletes the final base-$4$ digit $0$.  Therefore the
base-$4$ expansion of $p$ is $3w$, where
\[
...
\[
 ...
 \geq1-1=0.
\]
Moreover, by \eqref{eq:height-sigma},
\[
...
\]
Append a final base-$4$ digit $0$ and expand the digits of $3w0$ by the
block map
\[
 \phi(3)=11,\qquad \phi(2)=10,\qquad
 \phi(1)=01,\qquad \phi(0)=00.
\]
Let $B=\phi(3w0)$ and let $h$ be its binary height function.  At the pair
boundaries,
\[
 h(2)=2
\]
...
\[
 ...
 =0.
\]
It remains to check positions inside the two-bit blocks.  The blocks
corresponding to $3$ and $2$ begin with $1$, so the height first increases.
A block corresponding to $1$ is $01$; its initial height is at least $2$, so
after the first bit its height is still at least $1$.  If a digit $0$ occurs
inside $w$, then the Motzkin prefix condition implies that the weight before
that digit is at least $1$.  Hence the binary height before its block is at
least $4$, and after the first $0$ it is at least $3$.  Finally, the appended
terminal block $00$ begins at height $2$, passes through height $1$, and ends
at height $0$.

Consequently $B$ stays strictly above height $0$ before its final bit and
returns to height $0$ at the end.  It is therefore a primitive Dyck word.  Its
base-$4$ digits are $3w0$, so it represents $4p$.  Also $p\geq3$, and hence
$4p\geq12$.  Thus $4p\in\NpD$ and $p\in\Pset$.
\end{proof}
...
\]
The digits $1$ and $2$ both have $\sigma$-weight $0$, so appending either of
them preserves the two-coloured Motzkin condition.  The claim follows from
Lemma~\ref{lem:base4}.
\end{proof}
