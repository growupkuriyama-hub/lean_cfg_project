...
できる。この方法でどの整数の全体集合が、加法的に
どれほど豊かであるのか。本論文では、この問いの自然な変種を扱う。

\emph{Dyck語}とは、$1$ と $0$ を同数含み、かつ任意の接頭辞において
$1$ の個数が $0$ の個数以上である語 $w\in\{1,0\}^*$ のことである\footnote{ここに記号「$\,cdot\,$」は文字列の連結を表す。}。
\footnote{Dyck言語は正規ではないが、文脈自由言語である。}
例えば、Dyck語 $101100$ の原始Dyck語への一意な分解は
$101100=10\cdot1100$ である。空でないDyck語が\emph{原始的}であるとは、
...
原始Dyck語全体の言語を $\DP$ と書く。空でない接頭辞に対する条件で$\DP$を定義するので、空語 $\lambda$ は $\DP$ に
含めない。長さ順、同じ長さでは辞書式順に並べると、その最初の18語は
\[
\begin{gathered}
...
\[
11 10 01 01 00_{(2)}=11\cdot 10\cdot 01\cdot 01\cdot 00_{(2)}=32110_{(4)}
\]
のように表記する。言い換えれば形式$u_{(2)}=v_{(4)}$はある文字列$u\in\{0,1\}^*$と文字列$v\in\{0,1,2,3\}^*$を、対応
...
の連結によって同一視している。 この表記$*_{(2|4)}$はアルファベット上のメタ記号であり適宜省略して表記する。


基数 $m$ の数字からなる語 $w$ に対し、それが基数 $m$ で表す整数を
$\val{w}{m}$ と書く。本論文で扱う加法表示の便宜のために $0$ を加え、
\[
 \NpD=\{0\}\cup\{\val{w}{2}\in\mathbb{Z}:w\in\DP\}
\]
...
以下で$\Pset$の基数4構造を明らかにする。基数4の各数字に重みを
\[
  \sigma(3)=1,\qquad \sigma(0)=-1,\qquad
  \sigma(1)=\sigma(2)=0
\]
によって割り当てる。空列$\lambda$の重みづけは$\sigma(\lambda)=0$とする。語 $w=c_0\cdots c_{k-1}\in\{0,1,2,3\}^{k}$ が
\emph{2色Motzkin語}である\footnote{重み $0$ の二つの数字 $1$ と $2$ を、異なる
色とみなしている。}とは、任意の接頭辞の総重みが非負であり、かつ $w$ 全体の
総重みが $0$ であることをいう。$\mathsf{Motz}$ は空語 $\lambda$ を含む。文字列$w$を左から読んで重み付けするとき一般的には何回か$0$になりうる。
2色Motzkin語全体の集合を $\mathsf{Motz}$ と書く。$\DP$ とは異なり、また、
$w$ を基数4の数として読んだ値を $[w]_4$
...
\begin{proof}
この命題を直感的に済ませるのではなくフォーマルに示すことに価値があると考える。まず $p\in\Pset$ と仮定する。$\Pset$ の定義より、
\[
...
\[
 ...
 \qquad b_i\in\{0,1\},
\]
は$|B|=2L\geq4$ かつ$B\in\DP$である。その高さ関数は
\[
 h_B(t):=\sum_{i=1}^{t}(2b_i-1)
 \qquad(0\leq t\leq2L)
\]
で計算できる。条件$B\in\DP$を形式的にかけば、 
...
\]
から、第 $j$ ブロックによる高さの変化は比喩的に言えば$\Delta{h}({2j})=2\Delta\sigma_{j}$, 正確には
\[
...
次に、最初と最後のブロックを決定する。$B\in\DP$なので $b_1=1$ である。
もし $b_2=0$ ならば $h_B(2)=0$ となり、$2<2L$ および
\eqref{eq:primitive-height} に反する。したがって $b_2=1$ であり、
\[
 q_1=b_1b_2=3_{(4)}
\]
...
\eqref{eq:primitive-height} に反する。したがって $b_{2L-1}=0$ であり、
\[
 q_L=b_{N-1}b_{N}=0_{(4)}
\]
に限る。
...
\end{equation}
を得る。$4p$ を $4$ で割ることは、その4進展開の末尾の数字 $0_{(4)}=q_{L}$ を削除することに
対応するので、$q_1=3_{(4)}$とあわせれば、
\[
...
以下では$u\in\mathsf{Motz}$を示す。$u$ の長さ $r$ の接頭辞を考え、$0\leq r\leq L-2$ とする。$r=0$ の場合、その
...
\]
したがって、\eqref{eq:prefix-height}および\eqref{eq:total-weight-zero}により$u\in\mathsf{Motz}$ であり、必要性が示された。
...
\[
 ...
 w=q_2q_3\cdots q_m
\]
と書く。条件$w\in\mathsf{Motz}$を形式的に書けば、
...
\end{equation}
である。$m=1$ の場合、いずれの和も空和と解釈し、接頭辞条件は空虚に
成り立つものとする。
...
\[
 B:=\phi( \operatorname{quad}(p)q_{m+1})=\phi(q_1q_2\cdots q_mq_{m+1})
   =:b_1b_2\cdots  b_{2m+1}b_{2m+2}
\]
とする。$b_{2m+1}b_{2m+2}=00_{(2)}$なので、
\begin{equation}\label{eq:block-value}
 [B]_2=[b_1b_2\cdots  b_{2m+1}b_{2m+2}]_2=4p
\end{equation}
である。$B$ の高さ関数は
\[
 h_B(t):=\sum_{i=1}^{t}(2b_i-1)
...
\]
で計算できるのであった。先ほどと同じブロックごとの計算により、
...
まず、偶数位置の高さを調べる。$1\leq j\leq m$ に対して、$\sigma(q_1)=\sigma(3)=1$および\eqref{eq:reverse-motzkin-prefix}により、
\[
 \sum_{i=1}^{j}\sigma(q_i)
...
=1+\sum_{i=2}^{j}\sigma(q_i)
 \geq1
\]
である。ただし $j=1$ の場合、第2項は空和 $0$ とする。\eqref{eq:reverse-height-sigma}より、
...
\]
最後に $q_j=0_{(4)}$すなわち$00_{(2)}$の場合を考える。\eqref{eq:reverse-motzkin-prefix} より、
\[
...
\]
ゆえに、\eqref{eq:reverse-height-sigma} において$j:=j-1$を代入すれば、
\[
 h_B(2j-2)
   =2\left(\sum_{i=1}^{j-1}\sigma(q_i)\right) =2\left(\sigma(q_1)+\sum_{i=2}^{j-1}\sigma(q_i)\right)=2\left(1+\sum_{i=2}^{j-1}\sigma(q_i)\right)
 \geq4,
\]
したがって
\[
...
付け加えた終端数字 $q_{m+1}=0$ については、
  \eqref{eq:reverse-height-sigma} と\eqref{eq:reverse-motzkin-total}より、
\[
...
以上により、$1\leq t<2m+2$の全ての場合で
$ h_B(t)>0$を確認できた。一方、\eqref{eq:terminal-height} より $h_B(2m+2)=0$ である。
したがって $B\in\DP$ である。

さらに、$\operatorname{quad}(p)$の先頭数字は $3$ なので $p\geq3$、したがって
$4p\geq12$ である。\eqref{eq:block-value}である$[B]_2=4p$ と $B\in\DP$ を合わせると、$p\in\Pset$ である。これで十分性も示された。
\end{proof}
...
\]
である。$\sigma(1_{(4)})=\sigma(2_{(4)})=0$なので、どちらを末尾に
付け加えても $\mathsf{Motz}$ への所属は保たれる。したがって、補題~\ref{lem:base4}
から主張が従う。
\end{proof}
...
\emph{下界。}
$\sigma(0)=-1$ なので、最初の数字は $0$ ではあり得ない。$c_0\in\{1,2\}$ の場合は$\sigma(c_0)=0$なので、接尾辞 $w'=c_1\cdots c_{\ell-1}$ も$\mathsf{Motz}$に属するので、語長$\ell-1$の帰納的仮定により、
\[
...
\emph{上界。}
$c_0\in\{1,2\}$ の場合、$\sigma(c_0)=0$より$w'$ も$\mathsf{Motz}$に属するので、語長$\ell-1$の帰納的仮定により、
\begin{align*}
  [w]_4
...
次に $c_0=3$ とし、$w$を左から読んで総重み$\sigma$が始めて$0$ となる空でない接頭辞を$u$とする。$r=|u|$とすれば $2\leq r\leq\ell$であり、残りの接尾辞を$v$で表記すると、
\[
  u=c_1\cdots c_{r-2},
  \qquad
  c_{r-1}=0,
...
\]
であり、$|u|=r-2$かつ$|v|=\ell-r$であり$u,\, v\mathsf{Motz}$である。$c_0=3$の場合を今考えているので$w=3u0v$ である。位取り記数法と帰納法の仮定から、
\begin{align*}
  [w]_4
...
$\Pset$ の元の基数4展開がちょうど $d$ 桁であるとき、その元は\emph{世代 $d$} に属するという。補題~\ref{lem:base4} により、ある $w\in\mathsf{Motz}$ が存在して$ \operatorname{quad}(p)=3w$となるので、世代 $d$ の元の値は $[3w]_4=3\cdot4^{d-1}+[w]_4$ である。ここで $|w|=d-1$ である。補題\ref{lem:extremal}により直ちに以下の系が得られる。
...
\begin{proof}
区間の主張は$[3w]_4=3\cdot4^{d-1}+[w]_4$に\eqref{lem:extremal}において$\ell=d-1$とした$\frac{4^{d-1}-1}{3}\leq[w]_4\leq4^{d-1}-2^{d-1}$を代入することで得られる。端点が達成されることは、補題~\ref{lem:extremal} の極値語$w_{\max,\ell}$から従う。(a) は、次の世代が $g_{d+1}>3\cdot4^d$ から始まることによる。最後に、
$  3g_{k+1}
...
第~\ref{sec:base4} 節で記述した基数4の族の内部には、原始Dyck語を貫く単純な正規パターンがある。例えば、$868\in\NpD$ の2進展開は
\[
...
\]
と仮定する。大きい加数の個数を ${N_{\rm b}}$ とする。${N_{\rm b}}\leq2$ なら、残る位置を可能な限り大きい小さい加数で埋めても、
\begin{align*}
  x
  &\leq2U_{k+1}+2U_k\\
  ...
  <10\cdot4^k-2=x
\end{align*}
となり、矛盾する。したがって ${N_{\rm b}}\geq3$ である。しかしこのとき、大きい加数3個だけでも、その和は \eqref{eq:three-g} により
\[
