...
\[
  ...
  \sigma(1)=\sigma(2)=0
\]
によって割り当てる。語 $w=c_0\cdots c_{k-1}\in\{0,1,2,3\}^{k}$ が
\emph{2色Motzkin語}である\footnote{重み $0$ の二つの数字 $1$ と $2$ を、異なる
色とみなしている。}とは、任意の接頭辞の総重みが非負であり\footnote{空列$\lambda$の重みづけは$\sigma(\lambda)=0$とする。}、かつ $w$ 全体の
総重みが $0$ であることをいう。文字列$w$を左から読んで重み付けするとき一般的には何回か$0$になりうる。
...
\[
 ...
 \qquad b_i\in\{0,1\},
\]
は$|B|=2L\geq4$ かつ$B\in\DP$である。その高さ関数を
\[
 h_B(t):=\sum_{i=1}^{t}(2b_i-1)
 \qquad(0\leq t\leq2L)
\]
と定める。条件$B\in\DP$を形式的にかけば、 
...
\end{equation}
を得る。$4p$ を $4$ で割ることは、その4進展開の末尾の数字 $0$ を削除することに
対応するので、
\[
...
\]
したがって、$u$ の任意の接頭辞の重みは非負であり、$u$ 全体の重みは $0$ である。
...
\eqref{eq:reverse-motzkin-total} と \eqref{eq:reverse-height-sigma} より、
\[
