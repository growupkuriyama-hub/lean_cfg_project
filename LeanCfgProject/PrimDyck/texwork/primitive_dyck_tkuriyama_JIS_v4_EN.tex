...
$\lambda$ is not included in $\DP$.  Ordered by length and then
...
total weight and the total weight of $w$ is $0$.  In particular, the running
prefix weight is allowed to return to $0$ more than once.  Let $\mathsf{Motz}$
denote the set of all two-coloured Motzkin words.  Unlike $\DP$,
$\mathsf{Motz}$ contains the empty word $\lambda$.  Write $[w]_4$ for the
value of $w$ as a base-$4$ numeral, and set $[\lambda]_4=0$.  For a positive
integer $n$, let $\operatorname{bin}(n)$ and $\operatorname{quad}(n)$ denote
its standard binary and base-$4$ expansions, respectively.
...
\end{equation}
Since dividing $4p$ by $4$ deletes its final base-$4$ digit $0$, we have
\[
...
\[
 h_B(2m)=2\left(1+\sum_{i=2}^{m}\sigma(q_i)\right)=2.
\]
...
example, $868\in\NpD$, and its binary expansion factors as
\[
...
\]
Let $N_{\rm b}$ be the number of large summands.  If $N_{\rm b}\leq2$, then filling
...
\begin{align*}
  ...
  <10\cdot4^k-2=x,
\end{align*}
a contradiction.  Hence $N_{\rm b}\geq3$.  But then the three large summands
