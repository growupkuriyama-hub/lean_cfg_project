...
\end{gathered}
\]
である。次のPythonプログラムは、ここに表示した項を再現し、この数列の最初の
$50{,}000$ 項を生成する。そこから、いくつかの反復的なパターンも観察できる：
...
\end{aligned}
\]
すなわち、それ以外の正の偶数はすべて $r(N)\leq6$ を満たす。いいかえれば、任意の偶数
$N\geq848$ は、高々6個の原始Dyck数の和である。
\end{theorem}

同値に、$a(n)=r(2n)$ によって定義されるOEIS数列
\seqnum{A395858} \cite{OEIS-A395858} について、
\[
        \max_{n\geq1}a(n)=8,
        \qquad
...
\begin{proof}
式 \eqref{eq:finite-interval-certificate} の第1の包含、すなわち
$[215,254]\subseteq5L$ を示す。式 \eqref{eq:L-def} で定めた集合 $L$ を
\[
 A=\{53,54,57,58,60\},\qquad B=\{3,13,14\}
\]
と分割する。このとき $L=A\sqcup B$ である。逐次加算により、
\[
...
\[
 ...
 4A=[212,238]\cup\{240\}
\]
を得る。したがって $4A+B=[215,254]$ であり、式
\eqref{eq:finite-interval-certificate} の第1の包含が証明された。

次に、式 \eqref{eq:finite-interval-certificate} の第2の包含、すなわち
$[245,486]\subseteq H+4L$ を示す。$L$ を繰り返し加えると、
...
\end{split}
\end{equation}
を得る。式 \eqref{eq:4L-intervals} の右辺に現れる7個の区間のうち、
最初の三つの短い区間に、式 \eqref{eq:H-def} で定めた集合 $H$ を加えると、
\[
\begin{aligned}
...
\end{aligned}
\]
を得る。これらの和集合は $[245,304]$ を含む。さらに、
$\min H=213$, $\max H=248$ であり、$H$ の連続する元の差は高々 $9$ である。
したがって、$I=[a,b]$ が $b-a\geq8$ を満たすなら、
\[
 H+I=[213+a,248+b]
\]
である。これを式 \eqref{eq:4L-intervals} の後半四つの区間に順に適用すると、
\[
 [295,350],\quad[329,396],\quad[375,442],\quad[425,486]
\]
を得る。これらは互いに重なるので、$[245,486]\subseteq H+4L$ が証明された。

式 \eqref{eq:finite-interval-certificate} の第3の包含、すなわち
$[439,674]\subseteq2H+3L$ については、次の包含区間で十分である：
\[
...
\]
である。以上の区間を合わせると、式
\eqref{eq:finite-interval-certificate} の第3の包含が証明された。

式 \eqref{eq:finite-interval-certificate} の第4の包含、すなわち
$[645,862]\subseteq3H+2L$ については、
\[
...
最後に、式 \eqref{eq:finite-interval-certificate} の第5の包含、すなわち
$[855,1046]\subseteq4H+L$ については、
\[
...
\]
である。下端は鋭い。すなわち、整数 $209$, $210$, $211$ のいずれも
$F_5(\Pset)$ に属さない。
\end{proposition}
...
$\rho\equiv n\pmod4$ を満たす一意な $\rho\in\{5,6,7,8\}$ を選ぶ。このとき、
\[
...
\]
と書ける。$5\leq\rho\leq8$ であるから、$\eta_i\in\{1,2\}$ を
\[
...
\]
である。補題 \ref{lem:Pclosure} により、各加数 $4q_i+\eta_i$ は $\Pset$ に属する。
...
したがって、式 \eqref{eq:L-def} の $L$ を用いると、
\[
 \Pset\cap[0,211]=L
\]
である。
...
\]
とおく。$L$ の高々5個の元の和で、$D$ から高々3項を用いるものは、最大でも
\[
 3\cdot60+2\cdot14=208
\]
である。一方、$D$ から少なくとも4項を用いる和は、最小でも
\[
 4\cdot53=212
\]
