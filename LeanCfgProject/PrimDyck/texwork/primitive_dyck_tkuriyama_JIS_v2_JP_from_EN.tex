...
$()(())$のような均衡した括弧列$()(())$は、見やすさのために開き括弧を $1$、閉じ括弧を $0$ とすることにより、$101100$のようにアルファベット $\{1,0\}$ 上の語\footnote{この全体集合を$\{0,1\}^*$とかく。これは正規言語である。}として書くことができ、したがって2進整数として読むことができる。この方法でどの整数が得られるのか、また、それらがなす集合は加法的にどれほど豊かであるのか。本論文では、この問いの自然な変種を扱う。

\emph{Dyck語}とは、$1$ と $0$ を同数含み、かつ任意の接頭辞において $1$ の個数が $0$ の個数以上である語 $w\in\{1,0\}^*$ のことである\footnote{Dyck語の全体集合は正規表現では表せないが、文脈自由文法では表現できる。}。たとえばDyck語$101100$は$10\cdot1100$とDyck語の連結に分割できるが、それ以上はできない。空でないDyck語が\emph{原始的}であるとは、その空でない真の接頭辞のどれも均衡していないことをいう。同値に、$1$をup, $0$をdownと解釈したとき対応する格子路が終点において初めて高さ $0$ に戻ることをいう\footnote{このような路は、\emph{既約}Dyck路または\emph{分解不能}Dyck路とも呼ばれる。}。原始Dyck語の全体集合 $\DP$ と書き、空列$\varepsilon$は含めない。長さ順、同じ長さでは辞書式順に並べると、最初の18語は
\[
\begin{gathered}
...
基数 $m$ の数字からなる語 $w$ に対し、それが基数 $m$ で表す整数を $\val{w}{m}$ と書く。この論文の加法表現の便宜のために$0$ を加えて、
\[
 \NpD=\{0\}\cup\{\val{w}{2}:w\in\DP\}
\]
と定める。$\NpD$に$0$を含める約束は、以下の定理の結論には本質的ではない。最初の18個の正の元は
\[
\begin{gathered}
...
もう半歩進んだ、法$4$における以下の$\NpD$に対する基本的な観察は、繰り返し用られる。
...
\begin{proof}
以降の記述では$|\,\cdot\,|$語列の長さを表す。また$=$を構文的等号も表すことを簡単のために許す。$w\in\DP$を$w\neq\varepsilon$としてとる。$w=w_1a$, $a\in\{0,1\}$とするとき$w_1$は$w$の真の接頭辞であるから$a=0$である。$|w|=2$の場合は$w_1=1$, $a=0$つまり$w=10$, $[w]_2=[10]_2=2$に限る。$|w|\geq4$の場合、$w=w_2b0$, $b\in\{0,1\}$とおくと$b=0$に限る。なぜなら、もし$b\equiv1$ ならば、$w$の接頭辞$w_2$ですでに路が高さ $0$ に戻ってしまうからである。したがって$[w]_4$は$4$である。
\end{proof}
...
また論文のが意味する加法的表現は一意とほど遠いことは指摘しておく。例えば、
\[
...
$S\to\varepsilon\mid 1S0S$ により、空でないDyck語は一意に $1u0v$ と分解される。ただし $u,v$ はDyck語である。したがって、$C_0=1$ および
$C_n=\sum_{k=0}^{n-1}C_kC_{n-1-k}$ が成り立つ。一方、
$B_n=\frac{1}{n+1}\binom{2n}{n}$ とおけば、二項係数の畳み込み恒等式により
$B_0=1$ および $B_n=\sum_{k=0}^{n-1}B_kB_{n-1-k}$ が成り立つ。ゆえに帰納法により $C_n=B_n$ である。}
から、
\[
 \frac{C_{n-1}}{C_n}=\frac{n+1}{2(2n-1)}\longrightarrow\frac14
\]
を得る。しかし、その算術的ふるまいには動的な要素が生まれる。十分大きな偶数はすべて、制限のないDyck数なら3項で足りるのに拘わらず、原始族に制限すると小さいながら非自明な形状の例外集合が発生する。本論文では、高々6個の原始Dyck数の和でない正の偶数族を正確に11個に特定する。そのうち10個は7項を必要とし、$46$ だけが8項を必要とする。これら11個の例外はすべて $848$ 未満にある。

とくに8項を必要とする現象は初期地点で局所的であり、$48$ 未満の正の原始Dyck数が $2$ と $12$ しかないことが原因である。一方、無限遠方でのふるまい、つまり6項上界は構造的であり、二つの自己相似族から生じる。正規部分族が $4$ の倍数を処理し、$4$ で割った完全原始族が残る合同類を処理する。「高々6個必要」というのは無限遠まで分布する、正確に6個の原始Dyck数の和を必要とする後述する数列$N_1, N_2, \cdots$が構成されるためである。
...
\end{gathered}
\]
である。生成のためのPythonプログラムは
\href{https://github.com/growupkuriyama-hub/lean_cfg_project/blob/3525cfb/LeanCfgProject/PrimDyck/A395858.py}{\path{A395858.py}}であり、50000項の実際の出力は\href{https://github.com/growupkuriyama-hub/lean_cfg_project/blob/3525cfb/LeanCfgProject/PrimDyck/b395858.txt} {数列b395858.txt}で見ることができる。
...
軽く走らせた\path{A395858.py}の出力\href{https://github.com/growupkuriyama-hub/lean_cfg_project/blob/3525cfb/LeanCfgProject/PrimDyck/b395858.txt} {数列b395858.txt}を眺めれば$r$は順調に減少してゆき最終的に$r$は$5$以下になるようにも誤解されるが、事実はそうではない。出現頻度が減少するが無限遠まで分布する「引っかかり」$N_k$が存在し、最終的な上界は6が最良である。
...
\]
の形である。同値に $6a+b=23$ である。$6\cdot4>24$ なので $a\leq3$ であり、したがって項数は
\[
        a+b=23-5a\geq8
\]
を満たす。等号は
...
以下で用いる和集合の記法は、加法基底論における標準的なものである \cite[Chap.\ 1]{Nathanson}。非負整数 $k$ に対し、$kX$ を、$X$ の元を重複を許して $k$ 個足して得られる和の集合とし、$0X=\{0\}$ とする。この記法は誤解を生むが、$jX\neq\{jx : x\in X\}$である。また、
\[
 F_k(X)=\bigcup_{j=0}^{k}jX
\]
とおく。したがって、$F_k(X)$ は $X$ の元を高々 $k$ 個足して得られる和の集合である。スカラー $c$ に対しては、$c\cdot X=\{cx:x\in X\}$ と区別して書き、これは $X$ の拡大を表す。
...
$1/4$を出した理由は本理論の４進法との整合性にある。基数4の各数字に重みを
\[
  \sigma(3)=1,\qquad \sigma(0)=-1,\qquad
  \sigma(1)=\sigma(2)=0
\]
によって割り当てる。語 $w=c_0\cdots c_{k-1}\in\{0,1,2,3\}^{k}$ が\emph{2色Motzkin語}である\footnote{この言い方は、重み0の二つの数字 $1$ と $2$ は、異なる色とみなして命名している。}とは、任意の接頭辞の総重みが非負であり、かつ $w$ 全体の総重みが $0$ であることをいう。この全体集合を$\mathsf{Motz}$とかく。$\DP$接頭辞の非空に言及するため$\varepsilon\notin\DP$としたが、今回は$\mathsf{Motz}\ni\varepsilon$とする。$w$ を基数4の数として読んだ値を $[w]_4$ と書き、空列は$[\varepsilon]_4=0$ とする。以降では、正整数 $n$ に対して、その標準的な2進展開および4進展開を、それぞれ $\operatorname{bin}(n)$ および $\operatorname{quad}(n)$ と書く。


```tex
\begin{lemma}[基数4による特徴づけ]\label{lem:base4}
正整数 $p$ が $\Pset$ に属するための必要十分条件は、ある
$w\in\mathsf{Motz}$ が存在して
\[
...
\begin{proof}
まず、$p\in\Pset$ と仮定する。$\Pset$ の定義より、
\[
 4p\in\NpD,\qquad 4p\geq12
\]
である。したがって、$4p$ の2進展開
\[
 B:=\operatorname{bin}(4p)
   =b_1b_2\cdots b_{2L},
 \qquad b_i\in\{0,1\},
\]
は長さ $2L\geq4$ の原始Dyck語である。
...
各4進数字と対応する2ビットブロックは
\[
 3\longleftrightarrow11,\qquad
 2\longleftrightarrow10,\qquad
 1\longleftrightarrow01,\qquad
 0\longleftrightarrow00
\]
である。したがって、第 $j$ ブロックによる高さの変化は
\[
 h_B(2j)-h_B(2j-2)=2\sigma(q_j)
\]
となる。これを $1$ から $j$ まで足し合わせると、
\begin{equation}\label{eq:height-sigma}
 h_B(2j)
 =2\sum_{i=1}^{j}\sigma(q_i)
 \qquad(0\leq j\leq L)
\end{equation}
...
次に、最初と最後のブロックを決定する。$B$ はDyck語なので
$b_1=1$ である。もし $b_2=0$ ならば
\[
 h_B(2)=0
\]
となり、$2<2L$ に反して途中で高さ $0$ に戻る。したがって
$b_2=1$ であり、
\[
 q_1=3
\]
...
同様に、$b_{2L}=0$ である。もし $b_{2L-1}=1$ ならば、最後の
2ビットは $10$ となるので、
\[
 h_B(2L)-h_B(2L-2)=0.
\]
ところが $h_B(2L)=0$ であるから $h_B(2L-2)=0$ となり、
...
また、$1\leq j\leq L-1$ に対して $2j<2L$ であるから、
\eqref{eq:primitive-height} より $h_B(2j)>0$ である。
$h_B(2j)$ は偶数なので、
...
$4p$ を $4$ で割ることは、その4進展開の末尾の数字 $0$ を
一つ削除することに対応する。したがって、
\[
...
ここで、$u$ の長さ $r$ の接頭辞を考える。
$0\leq r\leq L-2$ とする。$r=0$ の場合、その重みは空和 $0$
である。$1\leq r\leq L-2$ の場合には、
...
さらに、\eqref{eq:height-sigma} を $j=L$ に適用すると、
\[
 \sum_{i=1}^{L}\sigma(q_i)
 =\frac{h_B(2L)}{2}
 =0.
\]
$\sigma(q_1)=\sigma(3)=1$ および
$\sigma(q_L)=\sigma(0)=-1$ であるから、
\[
 \sum_{i=2}^{L-1}\sigma(q_i)=0.
\]
したがって、$u$ の任意の接頭辞の重みは非負であり、
$u$ 全体の重みは $0$ である。ゆえに
\[
 u\in\mathsf{Motz}.
\]
...
と書く。ただし、
\[
 q_1=3,\qquad w=q_2q_3\cdots q_m.
\]
$w\in\mathsf{Motz}$ であるから、
...
$q_{m+1}:=0$ とおき、
\[
 U:=\phi(q_1q_2\cdots q_mq_{m+1})
   =b_1b_2\cdots b_{2m+2}
\]
とする。$q_1q_2\cdots q_mq_{m+1}$ は
$\operatorname{quad}(p)$ の末尾に4進数字 $0$ を付けた語なので、
\begin{equation}\label{eq:block-value}
 [U]_2
 =[q_1q_2\cdots q_m0]_4
 =4p
\end{equation}
である。
...
次に、各ブロックの内部、すなわち奇数位置
\[
 2j-1\qquad(1\leq j\leq m+1)
\]
の高さを調べる。第 $j$ ブロックの直前の高さは
$h_U(2j-2)$ である。

まず $q_j\in\{2,3\}$ の場合、対応するブロックは $10$ または
$11$ であり、最初のビットは $1$ である。したがって、
\[
 h_U(2j-1)=h_U(2j-2)+1>0.
\]
$j=1$ の場合にも $q_1=3$ なので、
\[
 h_U(1)=h_U(0)+1=1.
...
次に $q_j=1$ の場合、対応するブロックは $01$ である。
この場合は $2\leq j\leq m$ であり、
\eqref{eq:positive-even-height} より
\[
 h_U(2j-2)\geq2.
\]
よって、
...
次に、$2\leq j\leq m$ かつ $q_j=0$ とする。
\eqref{eq:reverse-motzkin-prefix} より、
\[
 0
 \leq\sum_{i=2}^{j}\sigma(q_i)
...
\]
ゆえに、\eqref{eq:reverse-height-sigma} から
\[
 h_U(2j-2)
 =
 2\left(
   1+\sum_{i=2}^{j-1}\sigma(q_i)
 \right)
 \geq4.
\]
対応するブロックは $00$ であるから、その最初の $0$ を読んだ後も
\[
 h_U(2j-1)
 =h_U(2j-2)-1
 \geq3.
...
最後に、付け加えた終端数字 $q_{m+1}=0$ を考える。
\eqref{eq:reverse-motzkin-total} と
\eqref{eq:reverse-height-sigma} より、
\[
 h_U(2m)
 =
 2\left(
  1+\sum_{i=2}^{m}\sigma(q_i)
 \right)
 =2.
\]
したがって、終端ブロック $00$ の最初のビットを読んだ後には
\[
 h_U(2m+1)=1
\]
であり、二つ目のビットを読んだ後に
...
以上により、任意の $1\leq t<2m+2$ に対して、

\[
 t=
 \begin{cases}
  2j,&1\leq j\leq m,\\
  2j-1,&1\leq j\leq m+1
 \end{cases}
\]
のいずれかであり、前者については
\eqref{eq:positive-even-height}、後者については上の各場合分けから
\[
 h_U(t)>0
\]
が成り立つ。また、\eqref{eq:terminal-height} より
\[
 h_U(2m+2)=0.
\]
したがって、$U$ は最後の位置で初めて高さ $0$ に戻るDyck語、
すなわち
\[
 U\in\DP
\]
...
さらに、$\operatorname{quad}(p)$ の先頭数字は $3$ なので
$p\geq3$、したがって
\[
 4p\geq12.
\]
\eqref{eq:block-value} と $U\in\DP$ より、
\[
 4p\in\NpD,\qquad 4p\geq12.
\]
ゆえに、$\Pset$ の定義から
...
\]
である。数字 $1$ と $2$ はいずれも $\sigma$-重み $0$ をもつので、そのどちらを末尾に付け加えても2色Motzkin条件は保たれる。したがって、補題~\ref{lem:base4} から主張が従う。
\end{proof}
...
\emph{下界。}
$\sigma(0)=-1$ なので、最初の数字は $0$ ではあり得ない。$c_0\in\{1,2\}$ の場合、接尾辞 $w'=c_1\cdots c_{\ell-1}$ も2色Motzkin語である。したがって、
\[
...
\]
は、それぞれ長さ $r-2$ と $\ell-r$ の2色Motzkin語である。したがって $w=3u0v$ である。位取り記数法と帰納法の仮定から、
\begin{align*}
  [w]_4
