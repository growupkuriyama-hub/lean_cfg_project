...
\begin{proof}
以降の証明で$\equiv$は構文的等号をあらわしは$|\,\cdot\,|$語列の長さを表す。$w\in\DP$を$w\neq\varepsilon$としてとる。$w=w_1a$, $a\in\{0,1\}$とするとき$w_1$は$w$の真の接頭辞であるから$a\equiv0$である。$|w|=2$の場合は$w_1\equiv1$, $a\equiv0$つまり$w\equiv10$, $[w]_2=[10]_2=2$に限る。$|w|\geq4$の場合、$w\equiv w_2b0$, $b\in\{0,1\}$とおくと$b\equiv0$に限る。なぜなら、もし$b\equiv1$ ならば、$w$の接頭辞$w_2$ですでに路が高さ $0$ に戻ってしまうからである。したがって$[w]_4$は$4$である。
\end{proof}
...
正整数 $p$ が $\Pset$ に属するための必要十分条件は、 $\operatorname{quad}(p)=3w$かつ$w\in\mathsf{Motz}$であること。
\end{lemma}

\begin{proof}
まず $p\in\Pset$ とする。このとき
\[
 4p\in\NpD,\qquad 4p\geq12
\]
である。$\operatorname{bin}(4p)$を
\[
w= b_1b_2\cdots b_{2L}\qquad(b_i\in\{0,1\})
\]
と書く。$w$を左から読んで$t$番目の高さは
\[
 h_{w}(t):=\sum_{i=1}^{t}(2b_i-1)
 \qquad(0\leq t\leq2L)
\]
で計算できる。$w\in\DP$なので、
\[
 h_{w}(0)=h_{w}(2L)=0,
 \qquad
 h_{w}(t)>0\quad(1\leq t<2L)
\]
が成り立つ。

一方で、$\operatorname{quad}(4p)=q_1\cdots q_L$とすれば、これは$w$を左から２桁づつ４進法にすることに相当し、
\[
q_j=2b_{2j-1}+b_{2j}
 \qquad(1\leq j\leq L)
\]
...
から、
\[
 h_w(2j)-h_w(2j-2)=2\sigma(q_j)
\]
が成り立つ。したがって、
\begin{equation}\label{eq:height-sigma}
 h_w(2j)=2\sum_{i=1}^{j}\sigma(q_i)
 \qquad(0\leq j\leq L)
\end{equation}
を得る。

$4p\geq12$かつ$\operatorname{bin}(4p)=w$より語$w$の長さは少なくとも4であるから、最初の2桁は$q_1=11$、最後の2桁は$q_L=00$ である。実際、そうでなければ、最初の2ビットを読んだ時点、または最後の2ビットを読む前に高さ $0$ に戻ってしまう。したがって、４進法においては、
\[
 q_1=3,\qquad q_L=0
\]
である。$w\in\DP$より$1\leq j\leq L-1$ に対して $h_w(2j)$ は正の偶数なので、\eqref{eq:height-sigma}は
\begin{equation}\label{eq:prefix-height}
 \sum_{i=1}^{j}\sigma(d_i)
 =\frac{h_w(2j)}{2}
 \geq1
\end{equation}
を含意する。

$4p$ を $4$ で割ることは、$\operatorname{quad}(4p)$末尾の$0$ を削除することに対応する。したがって、$p$ の4進展開は $3u$ であり、ここで
\[
 u=q_2\cdots q_{L-1}
\]
である。\eqref{eq:prefix-height}および$\sigma(3)=1$により、
\begin{equation}\label{eq:motzkin-prefix-nonnegative}
 \sum_{i=2}^{r+1}\sigma(q_i)
 =\sum_{i=1}^{r+1}\sigma(q_i)-\sigma(3)
 \geq1-1=0
\end{equation}
である。また、\eqref{eq:height-sigma} より、
\[
 \sum_{i=1}^{L}\sigma(q_i)=\frac{h_w(2L)}2=0
\]
であるが、$\sigma(q_1)=1$ および $\sigma(q_L)=-1$ なので、
\[
 \sum_{i=2}^{L-1}\sigma(q_i)=0
\]
を得る。ゆえに、$u\in\mathsf{Motz}$である。

逆に、
\[
 \operatorname{quad}(p)\equiv q'_1 q'_2\cdots q'_{m}\equiv 3w , \qquad q'_1\equiv3
\]
であり、$w\in\mathsf{Motz}$であるとする。すなわち、
\[
 \sum_{i=2}^{r}\sigma(q'_i)\geq0
 \quad(2\leq r\leq m),
 \qquad
 \sum_{i=2}^{m}\sigma(q'_i)=0
\]
が成り立つ。末尾に4進数字 $0$ を付け、$3w0$ の各数字をブロック写像
\[
 \phi(3)=11,\qquad \phi(2)=10,\qquad
 \phi(1)=01,\qquad \phi(0)=00
\]
によって2進数にする。$\phi(3w0)\equiv u\equiv b_1b_2b_3b_4\cdots b_{2m-1}b_{2m}b_{2m+1}b_{2m+2}$ , $b_1b_2\equiv11$, $b_{2m+1}b_{2m+2}\equiv00$  $b_i\in\{0,1\}$ とし、2進路の高さ関数を $h_u$ とする。$w\in\mathsf{Motz}$なので、$0\leq r\leq m-1$ に対して、
\[
 h_u(2r+2)
 =2\left(1+\sum_{i=2}^{r}\sigma(q'_i)\right)
 \geq2
\]
であり、$u$に付け加えられた$00$を読み終えた後には、
\[
 h_u(2m+2)
 =2\left(1+\sum_{i=2}^{m}\sigma(e_i)-1\right)
 =0
\]
となる。

残るのは、各2ビットブロックの内部の位置を確認することである。数字 $3$ と $2$ に対応するブロックは $1$ で始まるので、高さはまず増加する。数字 $1$ に対応するブロックは $01$ であり、その開始時の高さは少なくとも $2$ なので、最初のビットを読んだ後の高さも少なくとも $1$ である。数字 $0$ が $w$ の内部に現れる場合、Motzkin語の接頭辞条件から、その数字の直前の重みは少なくとも $1$ である。したがって、そのブロックの直前の2進高さは少なくとも $4$ であり、最初の $0$ を読んだ後も少なくとも $3$ である。最後に付け加えた終端ブロック $00$ は、高さ $2$ から始まり、高さ $1$ を経て、高さ $0$ で終わる。

したがって、$u=\phi(3w0)$ は最後のビットより前では高さ $0$ より真に上にあり、最後に高さ $0$ に戻る。ゆえに $u\in\DP$である。$[w]_4=p$と$u=3w0$とあわせれば、$[u]_4=4p$ である。また $p\geq3$ であるから $4p\geq12$ である。したがって $4p\in\NpD$、ゆえに $p\in\Pset$ である。
\end{proof}
