...
\[
  ...
  \sigma(1)=\sigma(2)=0
\]
によって割り当てる。語 $w=c_0\cdots c_{k-1}\in\{0,1,2,3\}^{k}$ が\emph{2色Motzkin語}である\footnote{この言い方は、重み0の二つの数字 $1$ と $2$ は、異なる色とみなして命名している。}とは、任意の接頭辞の総重みが非負であり、かつ $w$ 全体の総重みが $0$ であることをいう。この全体集合を$\mathsf{Motz}$とかく。紛らわしいが、$\DP$と違って$\mathsf{Motz}\ni\varepsilon$とする。$w$ を基数4の数として読んだ値を $[w]_4$ と書き、空列は$[\varepsilon]_4=0$ とする。以降では、正整数 $n$ に対して、その標準的な2進展開および4進展開を、それぞれ $\operatorname{bin}(n)$ および $\operatorname{quad}(n)$ と書く。
...
$w$における数字を左から連続する2桁ずつにまとめ、４進法にすれば、
\[
 d_j:=2b_{2j-1}+b_{2j}\in\{0,1,2,3\}
 \qquad(1\leq j\leq L)
\]
である。同値に,  $[v]_4=d_1\cdots d_L$である。対応
\[
...
\begin{equation}\label{eq:height-sigma}
 h(2j)=2\sum_{i=1}^{j}\sigma(d_i)
 \qquad(0\leq j\leq L)
\end{equation}
を得る。

原始語の長さは少なくとも4であるから、最初の2桁は $11$、最後の2桁は $00$ である。実際、そうでなければ、最初の2ビットを読んだ時点、または最後の2ビットを読む前に高さ $0$ に戻ってしまう。したがって、
\[
 d_1=3,\qquad d_L=0
\]
である。$1\leq j<L$ に対して $h(2j)$ は正の偶数なので、
\[
 \sum_{i=1}^{j}\sigma(d_i)=\frac{h(2j)}2\geq1
\]
が成り立つ。

$v=4p$ を $4$ で割ることは、末尾の4進数字 $0$ を削除することに対応する。したがって、$p$ の4進展開は $3w$ であり、ここで
\[
 w:=d_2d_3\cdots d_{L-1}
\]
である。$w$ の長さ $r$ の任意の接頭辞（$0\leq r\leq L-2$）に対して、
\[
 \sum_{i=2}^{r+1}\sigma(d_i)
 =\sum_{i=1}^{r+1}\sigma(d_i)-\sigma(d_1)
 \geq1-1=0
\]
...
\]
である。$\sigma(d_1)=1$ および $\sigma(d_L)=-1$ なので、
\[
 \sum_{i=2}^{L-1}\sigma(d_i)=0
\]
を得る。ゆえに、$w\in\mathsf{Motz}$である。
...
\[
 \sum_{i=1}^{r}\sigma(e_i)\geq0
 \quad(0\leq r\leq m),
 \qquad
 \sum_{i=1}^{m}\sigma(e_i)=0
\]
...
\]
によって展開する。$B=\phi(3w0)$ とし、その2進路の高さ関数を $h$ とする。各2桁組の境界では、
\[
 h(2)=2
\]
であり、$0\leq r\leq m$ に対して、
\[
 h(2r+2)
 =2\left(1+\sum_{i=1}^{r}\sigma(e_i)\right)
 \geq2
\]
である。付け加えた最後の数字 $0$ を読み終えた後には、
\[
 h(2m+4)
 =2\left(1+\sum_{i=1}^{m}\sigma(e_i)-1\right)
 =0
\]
となる。
...
したがって、$B=\phi(3w0)$ は最後のビットより前では高さ $0$ より真に上にあり、最後に高さ $0$ に戻る。ゆえに $B$ は原始Dyck語である。その4進数字は $3w0$ なので、$B$ が表す整数は $4p$ である。また $p\geq3$ であるから $4p\geq12$ である。したがって $4p\in\NpD$、ゆえに $p\in\Pset$ である。
\end{proof}
