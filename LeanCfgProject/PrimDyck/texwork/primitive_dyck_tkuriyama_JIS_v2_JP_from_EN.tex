...
$C_n=\frac1{n+1}\binom{2n}{n}$\footnote{文法
...
\[
  ...
  \sigma(1)=\sigma(2)=0
\]
によって割り当てる。語 $w=c_0\cdots c_{k-1}\in\{0,1,2,3\}^{k}$ が\emph{2色Motzkin語}であるとは、任意の接頭辞の総重みが非負であり、かつ $w$ 全体の総重みが $0$ であることをいう。重み0の二つの数字 $1$ と $2$ は、異なる色とみなす。$w$ を基数4の数として読んだ値を $[w]_4$ と書き、$[\varepsilon]_4=0$ とする。正整数 $n$ に対して、その標準的な2進展開および4進展開を、それぞれ $\operatorname{bin}(n)$ および $\operatorname{quad}(n)$ と書く。

\begin{lemma}[基数4による特徴づけ]\label{lem:base4}
正整数 $p$ が $\Pset$ に属するための必要十分条件は、その基数4展開が $3w$ の形であり、$w$ が2色Motzkin語であることである。
\end{lemma}
...
2進数字を左から連続する2桁ずつにまとめ、
\[
 d_j:=2b_{2j-1}+b_{2j}\in\{0,1,2,3\}
 \qquad(1\leq j\leq L)
\]
とおく。このとき $d_1\cdots d_L$ は $v$ の4進展開である。対応
\[
...
したがって、$B$ は最後のビットより前では高さ $0$ より真に上にあり、最後に高さ $0$ に戻る。ゆえに $B$ は原始Dyck語である。その4進数字は $3w0$ なので、$B$ が表す整数は $4p$ である。また $p\geq3$ であるから $4p\geq12$ である。したがって $4p\in\NpD$、ゆえに $p\in\Pset$ である。
\end{proof}
