...
\]
と定める。この約束は、以下の定理の結論には本質的ではない。最初の18個の正の元は
\[
\begin{gathered}
...
\begin{proof}
空でないDyck語はすべて末尾が $0$ である。さらに、長さが4以上の原始Dyck語は実際には末尾が $00$ でなければならない。もし末尾が $10$ ならば、最後の2文字の直前で既に路が高さ $0$ に戻ってしまうからである。$\NpD$ の最小数は$[10]_2=2$であり、したがってそれより長い$\DP$の語はすべて $4$ の倍数を表す。
\end{proof}
