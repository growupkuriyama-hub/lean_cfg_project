...
\begin{proof}
$w\in\DP$を$w\neq\epsilon$としてとる。$w=w_1a$, $a\in\{0,1\}$とするとき$w_1$は接頭辞であるから$a=0$である。$|w|=2$の場合は$w_1=1$, $a=0$つまり$w=10$, $[w]_2=[10]_2=2$に限る。$|w|\geq4$の場合、$w=w_2a_1a_2$, $a_i\in\{0,1\}$とおくと$a_1=a_2=0$に限る。なぜなら、もし$a_1a_2=10$ ならば、$w$の接頭辞$w_2$ですでに路が高さ $0$ に戻ってしまうからである。したがって$[w]_4$は$4$である。
\end{proof}
...
以下で用いる和集合の記法は、加法基底論における標準的なものである \cite[Chap.\ 1]{Nathanson}。非負整数 $k$ に対し、$kX$ を、$X$ の元を重複を許して $k$ 個足して得られる和の集合とし、$0X=\{0\}$ とする。$jX\neq\{jx : x\in X\}$である。また、
\[
...
今度は４進数にも視点を広げる。基数4の各数字に重みを
\[
  \sigma(3)=1,\qquad \sigma(0)=-1,\qquad
  \sigma(1)=\sigma(2)=0
\]
によって割り当てる。語 $w=c_0\cdots c_{k-1}\in\{0,1,2,3\}^{k}$ が\emph{2色Motzkin語}である\footnote{この言い方は、重み0の二つの数字 $1$ と $2$ は、異なる色とみなして命名している。}とは、任意の接頭辞の総重みが非負であり、かつ $w$ 全体の総重みが $0$ であることをいう。この全体集合を$\mathsf{Motz}$とかく。$w$ を基数4の数として読んだ値を $[w]_4$ と書き、空列は$[\varepsilon]_4=0$ とする。正整数 $n$ に対して、その標準的な2進展開および4進展開を、それぞれ $\operatorname{bin}(n)$ および $\operatorname{quad}(n)$ と書く。
...
\]
と書く。$v$を左から読んで$t$番目の高さは
\[
 h(t):=\sum_{i=1}^{t}(2b_i-1)
 \qquad(0\leq t\leq2L)
\]
で計算できる。$v$ は原始Dyck語によって表されるので、
\[
 h(0)=h(2L)=0,
 \qquad
