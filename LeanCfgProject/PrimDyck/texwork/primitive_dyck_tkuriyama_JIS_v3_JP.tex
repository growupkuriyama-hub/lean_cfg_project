...
できる。この方法でどの整数が得られるのか。また、それらがなす集合は加法的に
...
原始Dyck語全体の言語を $\DP$ と書く。空語 $\varepsilon$ は $\DP$ に
...
$S\to\varepsilon\mid 1S0S$ により、空でないDyck語は一意に $1u0v$ と
...
$\Pset$ の定義で $4$ による除算を行うのは、この族の基数4構造を明らかにするためである。
基数4の各数字に重みを
\[
...
\emph{2色Motzkin語}であるとは、任意の接頭辞の総重みが非負であり、かつ $w$ 全体の
総重みが $0$ であることをいう。\footnote{重み $0$ の二つの数字 $1$ と $2$ を、異なる
色とみなしている。}2色Motzkin語全体の集合を $\mathsf{Motz}$ と書く。$\DP$ とは異なり、
$\mathsf{Motz}$ は空語 $\varepsilon$ を含む。$w$ を基数4の数として読んだ値を $[w]_4$
と書き、$[\varepsilon]_4=0$ と定める。正整数 $n$ に対し、その標準的な2進展開および
4進展開を、それぞれ $\operatorname{bin}(n)$ および $\operatorname{quad}(n)$ と書く。

\begin{lemma}[基数4による特徴づけ]\label{lem:base4}
正整数 $p$ が $\Pset$ に属するための必要十分条件は、ある $w\in\mathsf{Motz}$ が存在して
\[
...
\[
 ...
 \qquad b_i\in\{0,1\},
\]
は長さ $2L\geq4$ の原始Dyck語である。その高さ関数を
\[
...
$1\leq j\leq L-1$ に対して $2j<2L$ であるから、
\eqref{eq:primitive-height} より $h_B(2j)>0$ である。$h_B(2j)$ は偶数なので、
...
$u$ の長さ $r$ の接頭辞を考え、$0\leq r\leq L-2$ とする。$r=0$ の場合、その
...
\[
 \sum_{i=1}^{L}\sigma(q_i)=\frac{h_B(2L)}{2}=0.
\]
$\sigma(q_1)=\sigma(3)=1$ および $\sigma(q_L)=\sigma(0)=-1$ であるから、
\[
...
\]
とする。各4進数字は対応する2ビットの2進ブロックに置き換えられるので、
...
\begin{equation}\label{eq:positive-even-height}
 ...
 \qquad(1\leq j\leq m)
\end{equation}
である。一方、$\sigma(q_{m+1})=\sigma(0)=-1$ と
...
偶数の場合は \eqref{eq:positive-even-height}、奇数の場合は直前の場合分けによって、
\[
 h_U(t)>0\qquad(1\leq t<2m+2)
\]
が成り立つ。一方、\eqref{eq:terminal-height} より $h_U(2m+2)=0$ である。
したがって $U$ は原始Dyck語であり、$U\in\DP$ である。

さらに、$p$ の4進展開の先頭数字は $3$ なので $p\geq3$、したがって
$4p\geq12$ である。\eqref{eq:block-value} と $U\in\DP$ を合わせると、
\[
 4p\in\NpD,\qquad 4p\geq12.
\]
...
\]
である。数字 $1$ と $2$ はいずれも $\sigma$-重み $0$ をもつので、どちらを末尾に
