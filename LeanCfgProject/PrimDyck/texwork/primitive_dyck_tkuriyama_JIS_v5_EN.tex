...
$101100=10\cdot1100$.\footnote{Here and below, $\cdot$ denotes
concatenation of words.}  A nonempty Dyck word is \emph{primitive} if none of
...
\begin{proof}
Let $p$ be a generation-$d$ element.  By the preceding paragraph,
$p=3\cdot4^{d-1}+[w]_4$ for some $w\in\mathsf{Motz}$ with
$|w|=d-1$.  Applying \eqref{eq:motzkin-extrema} with $\ell=d-1$ gives
\[
 \frac{4^{d-1}-1}{3}
 \leq[w]_4
 \leq4^{d-1}-2^{d-1}.
\]
Adding $3\cdot4^{d-1}$ yields \eqref{eq:generation-interval}.  The lower
