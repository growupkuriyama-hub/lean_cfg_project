...
\begin{proof}
Fix $m\in kA$.  Choose $a_1,\ldots,a_k\in A$ such that
\[
 m=a_1+\cdots+a_k.
\]
For every integer $r$ with $k\leq r\leq2k$, one can choose
$\eta_1,\ldots,\eta_k\in\{1,2\}$ such that
\[
 \eta_1+\cdots+\eta_k=r.
\]
By \eqref{eq:digit-closure}, each $4a_i+\eta_i$ belongs to $A$, and hence
\[
 4m+r
 =\sum_{i=1}^{k}(4a_i+\eta_i)
 \in kA.
\]
As $r$ ranges from $k$ to $2k$, this proves
\eqref{eq:digit-lifting}.
\end{proof}
...
\[
 m:=\frac{n-\rho}{4}.
\]
Then $m\in\mathbb Z$.  Since $n\geq4M+k$ and $\rho\leq k+3$,
\[
 m\geq\frac{4M+k-(k+3)}4=M-\frac34.
\]
Because $m$ is an integer, it follows that $m\geq M$.  On the other hand,
since $\rho\geq k\geq3$,
\[
 m=\frac{n-\rho}{4}
 \leq\frac{n-k}{4}
 <n.
\]
The induction hypothesis therefore gives $m\in kA$.  Combining
Lemma~\ref{lem:digit-lifting} with $k\leq\rho\leq2k$, we obtain
\[
 n=4m+\rho
 \in[4m+k,4m+2k]
 \subseteq kA.
...
$\mathsf{Motz}$.  Hence Lemma~\ref{lem:base4} gives
\[
 \mathcal E^+\subseteq\Pset.
\]
It follows from \eqref{eq:NpD-decomp} that
...
The regular family $\mathcal E$ is sufficient for treating integers
congruent to $0$ modulo $4$.  To treat integers congruent to $2$
modulo $4$, however, we need five-summand representations from the full
halved primitive family $\Pset$, rather than only from the regular
subfamily $\mathcal E^+$.
...
By Lemma~\ref{lem:base4}, the corresponding elements of $\Pset$ are
exactly
