...
\newcommand{\Pset}{\tfrac14\cdot
  \bigl(\mathcal N_{\mathrm{pD}}\bigr)_{\ge 12}}
\newcommand{\seqnum}[1]{\href{https://oeis.org/#1}{\rm \underline{#1}}}

\title{Additive Representations by Primitive Dyck Numbers}
...
\begin{abstract}
Reading a Dyck word as a binary integer, with $1$ as an opening bracket
and $0$ as a closing bracket, gives a totally balanced number.  We study
the subset arising from primitive Dyck words, whose running balance
returns to zero only at the end.  We determine the least number of
primitive Dyck numbers needed to represent every positive even integer.
The integer $46$ uniquely requires eight summands, and the integers
$34$, $44$, $98$, $154$, $198$, $202$, $206$, $838$, $842$, and $846$
require seven.  Every other positive even integer is a sum of at most
six primitive Dyck numbers.  In particular, $848$ is the sharp threshold
beyond which six summands always suffice.  The proof combines a regular
sublanguage with a self-similar closure property of the full primitive
family after division by $4$.
\end{abstract}

\noindent\textbf{Keywords:} additive representation, relative asymptotic
additive basis, exceptional set, Dyck word, primitive Dyck path, regular
language, interval filling.
...
class $0$ modulo $4$, but the class $2$ modulo $4$ is handled by a
self-similar closure property of the full primitive family.  This leads
not only to an eventual bound but to the complete exceptional set, the
optimal uniform bound for all even integers, and the sharp eventual
threshold.

The primitive family still has positive asymptotic proportion $1/4$
within each fixed-length Dyck layer.  Indeed, there are $C_n$ Dyck words
of length $2n$, and every primitive one is uniquely of the form $1u0$
with $u$ a Dyck word of length $2n-2$; so there are $C_{n-1}$ primitive
words of length $2n$.  From the Catalan formula
...
\end{gathered}
\]
The displayed terms and the finite certificates used below are
reproducible from the accompanying Python programs.
...
\]
Every other positive even integer satisfies $r(N)\leq6$.  Consequently,
every even integer $N\geq848$ is a sum of at most six primitive Dyck
numbers.
\end{theorem}
...
The integer $46$ is a sum of eight primitive Dyck numbers, but not of
seven.
\end{proposition}
...
$0X=\{0\}$, and for a scalar $c$ write
$c\cdot X=\{cx:x\in X\}$ for the dilation of $X$.

For $A\subseteq2\mathbb N_0$, we call $A$ a \emph{relative additive
basis of order at most $h$ for $2\mathbb N_0$} if every element of
$2\mathbb N_0$ is a sum of at most $h$ elements of $A$.  Its relative
order is \emph{exactly $h$} if $h$ is the least integer with this
property.  We similarly call $A$ a \emph{relative asymptotic additive
basis of order at most $h$} if every sufficiently large element of
$2\mathbb N_0$ is a sum of at most $h$ elements of $A$.  

\section{A regular family inside the primitive words}

A simple regular pattern runs through the primitive Dyck words.  For
...
of $\mathcal E_m$ is split into its leading digit and its lower digits, the
...
\]
so that $\max \Delta_m=u_m$.  Let $\mathcal E_m$ be the finite subset of
...
\]
The largest element of $\mathcal E_m$ is
\[
 \max\mathcal E_m=[3\underbrace{22\cdots2}_{m-1\text{ digits}}]_4
                =3\cdot4^{m-1}+2u_{m-1},
\]
...
\end{equation}
Moreover $\max\mathcal E_m>u_m$, so $\max S_{t,m}$ decreases with $t$.
...
$[25,e_2]=[25,62]\subseteq6\mathcal E_2$.  The intervals $[26,28]$,
...
To lighten the notation during this step, write
\[
...
\]
Indeed, the new leading digit of an element of $\Delta_m$ is $0$ or $1$,
whereas a new $m$-digit element of $\mathcal E_m$ has leading digit $3$
...
copies of $\mathcal E_m$ use their new $m$-digit part, then
...
It remains to propagate the interval inside $6\mathcal E_m$.  Taking
...
$[25,e_{m-1}]$ by the induction hypothesis.  Since $m\geq3$ we have
$u\geq5$, and
\[
...
Since $e_m\to\infty$ and $\mathcal E=\bigcup_{m\geq1}\mathcal E_m$,
Proposition \ref{prop:E} follows at once.

The preceding proposition will handle the congruence class $0$ modulo
$4$: every multiple of $4$ at least $100$ is a sum of at most six
primitive Dyck numbers.

\section{The full primitive family after division by four}

The regular family $\mathcal E$ is sufficient for multiples of $4$, but
it does not by itself provide the five-summand interval needed below.
For the other congruence class we instead use the full primitive family.
Set
\[
...
\]
For a set $X$ of integers and $k\geq0$, write
\[
...
\begin{proof}
Write the binary expansion of the primitive Dyck number $4p$ as $u00$.
Immediately after reading $u$, the associated path has height $2$.  The
binary expansions of $4(4p+1)$ and $4(4p+2)$ are, respectively,
\[
 u0100\qquad\text{and}\qquad u1000.
\]
The inserted blocks $01$ and $10$ both begin and end at height $2$, and
their intermediate heights are positive.  Thus both new paths remain
strictly above zero until their final symbol and return to zero only at
the end.  The two displayed words are therefore primitive Dyck words,
...
\texttt{verify\_certificates\_for\_paper.py}, archived at commit
...
\end{aligned}
\]
Their union contains $[245,304]$.  Consecutive elements of $H$ differ
by at most $9$.  Therefore, if $I=[a,b]$ with $b-a\geq8$, then
$H+I=[213+a,248+b]$.  Applying this to the four remaining long
intervals gives
\[
...
\[
 496+[159,178]=[655,674].
\]
Together these intervals prove $[439,674]\subseteq2H+3L$.
...
choose the unique
$S\in\{5,6,7,8\}$ with $S\equiv n$ (mod $4$).  Then
\[
...
\]
Because $5\leq S\leq8$, we may choose $e_i\in\{1,2\}$ so that
$e_1+\cdots+e_5=S$.  Therefore
\[
 n=4M+S=\sum_{i=1}^{5}(4q_i+e_i).
\]
Lemma~\ref{lem:Pclosure} shows that every summand $4q_i+e_i$ belongs to
$\Pset$.  Thus $n\in5\Pset$, completing the induction.
...
$213$.  Longer primitive words have still larger values.  Hence
$\Pset\cap[0,211]=L$.

A sum of at most five members of $L$ using at most three terms from
$\{53,54,57,58,60\}$ is at most
\[
...
\section{The exceptional set}\label{sec:exceptions}
...
a multiple of $4$.  Hence a representation of $N\equiv2$ (mod $4$) contains
...
Suppose first that $N\equiv0$ (mod $4$).  If $N\geq100$, write $N=4n$.
...
Now let $N\equiv2$ (mod $4$), say $N=4m+2$.  If $N\geq850$, then $m\geq212$,
...
It remains to classify the numbers $N\leq846$ with $N\equiv2$ (mod $4$).
...
\begin{proof}
Theorem~\ref{thm:main} shows that six summands suffice for every even
...
Theorem~\ref{thm:main}; the integer $0$ is the empty sum.
\end{proof}

Theorem~\ref{thm:main} gives the complete exceptional set for
six-summand representability.  Corollary~\ref{cor:eight} shows that
$\NpD$ is a relative additive basis of exact order eight for
$2\mathbb N_0$.  It is also a relative asymptotic additive basis of
order at most six; equivalently, $\frac12\cdot\NpD$ is an asymptotic additive
basis of $\mathbb N_0$ of order at most six.  Corollary~\ref{cor:threshold}
shows that the associated threshold $848$ is sharp.

Let $h_{\mathrm{asym}}$ denote the least $h$ for which every sufficiently
large even integer is a sum of at most $h$ primitive Dyck numbers.  The
larger family of all totally balanced numbers is not an asymptotic
additive basis of order two \cite[Thm.~14]{BLS}; since the primitive
family is a subset of it, two summands cannot suffice here either.
Together with the present upper bound, this gives
\[
        3\leq h_{\mathrm{asym}}\leq6.
\]
The exact asymptotic order is therefore one of $3,4,5,6$.  In particular,
it remains open whether the bound six can be reduced to five.


\section{Sequences from the OEIS}\label{sec:oeis}

Two existing OEIS sequences are discussed in this paper.  The totally
balanced numbers form \seqnum{A014486} \cite{OEIS-A014486}.  The
primitive Dyck numbers, denoted $\NpD$ here, form \seqnum{A057547}
\cite{OEIS-A057547}.  The central sequence studied here is
$a(n)=r(2n)$.

\section{Disclosure of AI use}\label{sec:AI}

The idea for this study arose while the author was using ChatGPT 5.6
(OpenAI) to survey existing research connecting formal languages with
elementary number theory. The author discussed with ChatGPT 5.6 which
aspects of existing results might be modified to yield a new additive
problem, and decided to investigate additive representations by
primitive Dyck numbers.

The specific sets, finite interval certificates, exceptional set,
induction arguments, and proofs appearing in this paper were derived
and verified by the author using a whiteboard, paper, a ballpoint pen,
and a scientific calculator. Large-scale finite computations were
carried out using Python programs, and Claude Opus 4.8 (Anthropic)
assisted with the development and checking of those programs.

ChatGPT 5.6 was also used to check the consistency and uniformity of
mathematical symbols and notation, to provide advice on the use of
\LaTeX{}, and to point out unnatural expressions in English text
...
% Replace this pending item by the accepted OEIS entry before submission.
\bibitem{RSS}
