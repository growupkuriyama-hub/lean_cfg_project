...
language.} and hence read as a binary integer.  Which integers arise this
way, and how additively rich is the set they form?  We take up a natural
variant of that question.
...
$101100=10\cdot1100$.  A nonempty Dyck word is \emph{primitive} if none of
...
Let $\DP$ denote the language of primitive Dyck words; the empty word
$\varepsilon$ is not included in $\DP$.  Ordered by length and then
...
$S\to\varepsilon\mid 1S0S$ gives the unique decomposition $1u0v$ of every
...
The division by $4$ in the definition of $\Pset$ is chosen to expose the
base-$4$ structure of the family.  Assign weights to the base-$4$ digits by
\[
...
and the total weight of $w$ is $0$.\footnote{The two zero-weight digits $1$
and $2$ are regarded as distinct colours.}  Let $\mathsf{Motz}$ denote the
set of all two-coloured Motzkin words.  Unlike $\DP$, it contains the empty
word $\varepsilon$.  Write $[w]_4$ for the value of $w$ as a base-$4$
numeral, and set $[\varepsilon]_4=0$.  For a positive integer $n$, let
...
\begin{proof}
Suppose first that $p\in\Pset$.  By the definition of $\Pset$,
\[
...
\[
 ...
 \qquad b_i\in\{0,1\},
\]
is a primitive Dyck word of length $2L\geq4$.  Define its height function by
\[
 h_B(t):=\sum_{i=1}^{t}(2b_i-1)
 \qquad(0\leq t\leq2L).
\]
Since $B$ is primitive,
...
\]
show that the change in height across the $j$th block is
\[
...
$2<2L$ and \eqref{eq:primitive-height}.  Hence $b_2=1$ and
\[
 q_1=3.
\]
...
\eqref{eq:primitive-height}.  Thus $b_{2L-1}=0$ and
\[
 q_L=0.
\]

For $1\leq j\leq L-1$, one has $2j<2L$, so
$h_B(2j)>0$ by \eqref{eq:primitive-height}.  Since $h_B(2j)$ is even,
...
\end{equation}
Dividing $4p$ by $4$ deletes the final base-$4$ digit $0$.  Consequently,
\[
...
Consider a prefix of $u$ of length $r$, where $0\leq r\leq L-2$.
For $r=0$ its weight is the empty sum $0$.  For
$1\leq r\leq L-2$, applying \eqref{eq:prefix-height} with $j=r+1$ gives
...
$\sigma(q_L)=\sigma(0)=-1$, it follows that
\[
 \sum_{i=2}^{L-1}\sigma(q_i)=0.
\]
Thus every prefix of $u$ has nonnegative weight and the total weight of $u$
is $0$.  Therefore $u\in\mathsf{Motz}$, proving the forward implication.
...
$j-1$ in place of $j$,
\[
 h_U(2j-1)=h_U(2j-2)+1\geq3.
\]
If $q_j=1$, its block is $01$, and the same shifted application of
\eqref{eq:positive-even-height} gives
\[
 h_U(2j-1)=h_U(2j-2)-1\geq1.
\]
Finally suppose that $q_j=0$.  By
\eqref{eq:reverse-motzkin-prefix},
\[
...
\]
Therefore \eqref{eq:reverse-height-sigma} gives
\[
 h_U(2j-2)
...
For the appended terminal digit $q_{m+1}=0$,
\eqref{eq:reverse-motzkin-total} and
\eqref{eq:reverse-height-sigma} give
\[
 h_U(2m)=2\left(1+\sum_{i=2}^{m}\sigma(q_i)\right)=2.
\]
...
case by the preceding case distinction.  Hence
\[
 h_U(t)>0\qquad(1\leq t<2m+2),
\]
while \eqref{eq:terminal-height} gives $h_U(2m+2)=0$.  Therefore $U$ is a
primitive Dyck word, so $U\in\DP$.

Since the first base-$4$ digit of $p$ is $3$, one has $p\geq3$ and hence
$4p\geq12$.  Together with \eqref{eq:block-value} and $U\in\DP$, this gives
\[
 4p\in\NpD,\qquad 4p\geq12.
\]
By the definition of $\Pset$, $p\in\Pset$.  This proves the reverse
implication.
\end{proof}
...
\]
The digits $1$ and $2$ both have $\sigma$-weight $0$, so appending either one
preserves membership in $\mathsf{Motz}$.  The claim follows from
Lemma~\ref{lem:base4}.
\end{proof}
...
\emph{Lower bound.}
Since $\sigma(0)=-1$, the first digit cannot be $0$.  If
$c_0\in\{1,2\}$, then the suffix $w'=c_1\cdots c_{\ell-1}$ is again a
two-coloured Motzkin word.  Hence
\[
...
\emph{Upper bound.}
If $c_0\in\{1,2\}$, then $w'$ is again a two-coloured Motzkin word, so
\begin{align*}
  [w]_4
...
Now suppose $c_0=3$, and let $r$ be the length of the shortest nonempty prefix
of $w$ having total weight $0$.  Then $2\leq r\leq\ell$,
$c_{r-1}=0$, and
\[
  u=c_1\cdots c_{r-2},
  \qquad
...
Thus $w=3u0v$.  By place value and the induction hypothesis,
\begin{align*}
  [w]_4
...
\href{https://github.com/growupkuriyama-hub/lean_cfg_project/blob/3525cfb/LeanCfgProject/PrimDyck/verify_certificates_for_paper.py}{fixed repository copy at commit \texttt{3525cfb}}.
The archived version passed the repository's Python CI check at that commit
(Python CI run \#1).  The same program is included as a supplementary file,
together with a README giving the execution command and the expected output.
...
assume that every integer in $[215,n-1]$ lies in $5\Pset$.  Choose the
unique $\rho\in\{5,6,7,8\}$ such that
...
$jL=(j-1)L+L$, successive addition of the eight elements of $L$ gives the
following table, truncated at $211$:
\[
\begin{array}{c|l}
j & F_j(L)\cap[0,211]\\
\hline
...
Each row of the table is obtained from the preceding one by translating
by the eight elements of $L$ and taking the union with the preceding row.
\end{proof}
...
The program
\href{https://github.com/growupkuriyama-hub/lean_cfg_project/blob/3525cfb/LeanCfgProject/PrimDyck/A395858.py}{\path{A395858.py} at commit \texttt{3525cfb}}
reproduces the displayed initial terms and generates the first $50{,}000$ terms
of A395858.  The supplementary verification script at the same fixed commit
checks the five finite interval inclusions, the sharp lower endpoint for
Proposition~\ref{prop:fiveP}, and the finite exceptional-set certificate.  Its
independent dynamic-programming computation checks $r(N)$ through the selected
finite bound (by default $50{,}000$).  The default run also checks the recurring
obstruction for $2\leq k\leq5$, the six-summand family
\[
 r(10\cdot4^{k+1}-6)=6
\]
for $2\leq k\leq5$, and the five-summand family
\[
 r(10\cdot4^{k+1}-8)=5
\]
for $3\leq k\leq5$.  This version passed the repository's Python CI check at
commit \texttt{3525cfb} (Python CI run \#1).  These computations provide
reproducible finite cross-checks; the assertions for all $k$ and all sufficiently
large integers are proved theoretically above.
