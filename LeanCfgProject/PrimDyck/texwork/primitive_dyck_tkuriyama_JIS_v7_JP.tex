...
第~\ref{sec:base4}節で得た基数4構造は、有限区間の被覆を無限の尾部へ
伝播させる。まず、その一般的な機構を取り出す。

\begin{lemma}[桁持ち上げ]\label{lem:digit-lifting}
$k\geq1$ とし、$A\subseteq\mathbb Z_{>0}$ が
...
\begin{proof}
$m\in kA$ を固定する。ある $a_1,\ldots,a_k\in A$ を選んで
\[
 m=a_1+\cdots+a_k
\]
と書く。任意の整数 $r$（$k\leq r\leq2k$）に対し、
\[
 \eta_1+\cdots+\eta_k=r,
 \qquad \eta_i\in\{1,2\}
\]
となる $\eta_1,\ldots,\eta_k$ を選ぶことができる。
式~\eqref{eq:digit-closure} より各 $4a_i+\eta_i$ は $A$ に属するので、
\[
 4m+r
 =\sum_{i=1}^{k}(4a_i+\eta_i)
 \in kA
\]
である。$r$ を $k$ から $2k$ まで動かせば、
式~\eqref{eq:digit-lifting} が従う。
\end{proof}

\begin{corollary}[有限区間から尾部への伝播]
\label{cor:tail-propagation}
$k\geq3$ とし、$A\subseteq\mathbb Z_{>0}$ が
式~\eqref{eq:digit-closure} を満たすとする。ある整数 $M\geq1$ に対して
...
\begin{proof}
$n\geq M$ に関する強い帰納法により、$n\in kA$ を示す。
\[
 M\leq n\leq4M+k-1
\]
の場合は、式~\eqref{eq:tail-seed} による。

次に $n\geq4M+k$ とし、$M\leq j<n$ を満たすすべての整数 $j$ に対して
$j\in kA$ が成り立つと仮定する。$n$ と法4で合同になる一意な整数
\[
...
\[
 ...
 <n
\]
である。したがって、帰納法の仮定から $m\in kA$ である。
補題~\ref{lem:digit-lifting} と $k\leq\rho\leq2k$ を合わせると、
\[
 n=4m+\rho
 \in[4m+k,4m+2k]
 \subseteq kA
...
\]
と定め、$\mathcal E^+=\mathcal E\setminus\{0\}$ とおく。$j=0$ のときは、
接尾辞を空語と解釈し、対応する元は $[3]_4=3$ である。
各語 $\varepsilon_1\cdots\varepsilon_j$ は $\mathsf{Motz}$ に属するので、
補題~\ref{lem:base4} より
\[
 \mathcal E^+\subseteq\Pset
\]
である。したがって、式~\eqref{eq:NpD-decomp} より
\begin{equation}\label{eq:4E}
 4\cdot\mathcal E\subseteq\NpD
\end{equation}
を得る。

また、$e\in\mathcal E^+$ とすると、その4進展開は
\[
...
\]
であり、どちらも再び $\mathcal E^+$ に属する。したがって、
\begin{equation}\label{eq:Eclosure}
 4\cdot\mathcal E^++\{1,2\}\subseteq\mathcal E^+
\end{equation}
を得る。

以下では、次の有限包含だけを計算証明書として用いる。

\begin{lemma}[正規族の有限証明書]
...
\end{aligned}
\]
このとき、$A_0\subseteq\mathcal E$ および
$A,B,C\subseteq\mathcal E^+$ である。さらに、
...
\begin{proof}
各主張は、有限集合の和集合に関する完全な直接計算である。
有限集合 $X$ に対し、$S_0(X)=\{0\}$ とおき、
\[
 S_{i+1}(X)=S_i(X)+X
\]
と再帰的に計算する。補助検証プログラムは、表示した各整数区間の
すべての整数が対応する和集合に属することを検査する。
\end{proof}
...
\]
を与える。式~\eqref{eq:E-tail-certificate} に現れる和集合は、いずれも
$A,C\subseteq\mathcal E^+$ だけから構成されており、$0$ を加数として
用いていない。したがって、その5区間の重なりは
\[
...
ここで、系~\ref{cor:tail-propagation} を $A=\mathcal E$ ではなく
$A=\mathcal E^+$ に適用する。実際、$\mathcal E$ は $0$ を含むため、
式~\eqref{eq:digit-closure} を満たすならば $1,2\in\mathcal E$ でなければ
ならないが、これは成り立たない。一方、$\mathcal E^+$ は
式~\eqref{eq:Eclosure} により式~\eqref{eq:digit-closure} を満たす。
また、
\[
 877=4\cdot218+6-1
\]
...
\[
 [218,877]\subseteq6\mathcal E^+
\]
は $k=6$、$M=218$ に対する尾部伝播系の初期区間条件そのものである。
したがって、系~\ref{cor:tail-propagation} より
\[
...
命題~\ref{prop:E} と式~\eqref{eq:4E} より、$100$ 以上の任意の
4の倍数は、高々6個の原始Dyck数の和である。

\subsection{完全な半分化族による5項被覆}

正規族 $\mathcal E$ は、法4で $0$ に合同な整数を処理するには十分である。
一方、法4で $2$ に合同な整数を扱うためには、正規部分族
$\mathcal E^+$ だけではなく、半分化原始族全体 $\Pset$ の5項表示を
用いる必要がある。
...
\[
 ...
 \bigr)
\]
と書く。また、$\mathbf h(\lambda)=()$ とする。ここで
\[
 \sigma(3)=1,\qquad
 \sigma(0)=-1,\qquad
 \sigma(1)=\sigma(2)=0
\]
である。この記法では、$w\in\mathsf{Motz}$ であることは、
$\mathbf h(w)$ のすべての成分が非負であり、その最後の成分が $0$ で
あることと同値である。
...
この表が完全であることも直接確認できる。長さ1の語が総重み0を
もつためには、その唯一の文字が1または2でなければならない。
長さ2の場合、最初の文字が1または2ならば、二文字目も1または2で
なければならず、
\[
 11,\ 12,\ 21,\ 22
\]
を得る。最初の文字が3ならば、高さ1から総重み0に戻るために
二文字目は0でなければならず、語 $30$ を得る。最初の文字が0ならば、
最初の接頭辞の重みが負になるので許されない。したがって、長さ
$0,1,2$ の2色Motzkin語は、表に挙げた
\[
 1+2+5=8
\]
個で全部である。

補題~\ref{lem:base4} より、対応する $\Pset$ の元は正確に
...
この列挙も完全である。最初の文字が1の場合、残りの長さ2の接尾辞は
\[
 11,\ 12,\ 21,\ 22,\ 30
\]
のいずれかであるから、
...
最初の文字が3の場合、最初の接頭辞の重みは1である。したがって、
残り二文字の重みの和は $-1$ でなければならない。接頭辞重みの
非負性も満たす可能性は
\[
 01,\ 02,\ 10,\ 20
\]
...
\[
 301,\ 302,\ 310,\ 320
\]
を得る。最初の文字が0ならば、最初の接頭辞の重みが負になるため
許されない。したがって、長さ3の2色Motzkin語は、表に挙げた
\[
 5+5+4=14
\]
個で全部である。

よって、対応する $\Pset$ の世代4の元は正確に
...
\end{split}
\end{equation}
である。実際、$H$ は単なる部分集合ではなく、$\Pset$ の世代4全体である。

最後に、系~\ref{cor:gen} によれば、世代4の最小元は
\[
\begin{aligned}
 g_4
...
\end{aligned}
\]
である。したがって、$213$ 未満の $\Pset$ の元は世代1、2、3に属する
ものに限られ、それらは最初の表に列挙した $L$ の8元にほかならない。
ゆえに、より強く
\[
 \Pset\cap[0,212]=L
\]
...
$4\cdot\Pset+\{1,2\}\subseteq\Pset$ である。また、
\eqref{eq:fiveP-seed} は特に
$[215,864]\subseteq5\Pset$ を含む。したがって、
系~\ref{cor:tail-propagation} を $k=5$、$M=215$ に適用すると、
\[
...
$m\in F_5(\Pset)$ である。系~\ref{cor:sixcriterion} から、$N$ は
...
補題~\ref{lem:finiteexception}、および系~\ref{cor:sixcriterion} から、
...
\begin{corollary}[5項判定条件]\label{cor:fivecrit}
...
\[
 ...
 m-1\notin F_2(\Pset)
\]
である。したがって、系~\ref{cor:fivecrit} から $r(N_k)\geq6$ を得る。
