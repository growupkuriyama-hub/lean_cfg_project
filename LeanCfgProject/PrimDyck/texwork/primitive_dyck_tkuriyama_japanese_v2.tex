...
\begin{proof}
\eqref{eq:finite-interval-certificate}の第1行$[215,254]\subseteq5L$については、$L$ : \eqref{eq:L-def} を
\[
 A=\{53,54,57,58,60\},\qquad B=\{3,13,14\}
\]
と$L=A\sqcup B$に分割する。逐次加算により、
\[
...
\[
 ...
 4A=[212,238]\cup\{240\}
\]
を得る。したがって $4A+B=[215,254]$ であり、\eqref{eq:finite-interval-certificate}の第1行が証明される。

\eqref{eq:finite-interval-certificate}の第2行$[245,486]\subseteq H+4L$については、$L$ を繰り返し加えると、
...
\end{split}
\end{equation}
を得る。前７ツの区間のうち、最初の三つの短い区間に$H$ : \eqref{eq:H-def}を加えると、
\[
\begin{aligned}
...
\end{aligned}
\]
を得る。これらの和集合は $[245,304]$ を含む。一方で、$H$ の連続する元の差は高々 $9$かつ$H\subseteq[213,248]$ であるので、$I=[a,b]$ が $b-a\geq8$ を満たすなら、
$H+I=[213+a,248+b]$ である。これを\eqref{eq:4L-intervals}の後半四つの区間に適用すると、それぞれ
\[
 [295,350],\quad[329,396],\quad[375,442],\quad[425,486]
\]
を得る。これらは互いに重なるので、$[245,486]\subseteq H+4L$ の証明が完了する。

\eqref{eq:finite-interval-certificate}の第3行$[439,674]\subseteq2H+3L$については、次の包含区間で十分である：
\[
...
\[
 496+[159,178]=[655,674]
\]
である。以上の区間を合わせると、\eqref{eq:finite-interval-certificate}の第3行が証明される。

\eqref{eq:finite-interval-certificate}の第4行$[645,862]\subseteq3H+2L$については、については、
\[
...
\]
は $[645,862]$ を覆う。\eqref{eq:finite-interval-certificate}の第4行$[855,1046]\subseteq4H+L$については、については、
\[
...
\]
と書ける。$5\leq \rho\leq8$ であるから、
$r_1+\cdots+r_5=\rho$ となる $r_i\in\{1,2\}$ を選ぶことができる。したがって、
\[
 n=4M+\rho=\sum_{i=1}^{5}(4q_i+r_i)
\]
である。補題 \ref{lem:Pclosure} により、各加数 $4q_i+r_i$ は $\Pset$ に属する。
