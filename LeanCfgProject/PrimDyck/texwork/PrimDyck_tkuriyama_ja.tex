...
\end{gathered}
\]
である。ここに表示した項は、以下のPythonプログラムから
50000項まで再現でき、ある種の規則性を見ることができる：
...
同値な形で、 OEIS数列 \seqnum{A395858} \cite{OEIS-A395858} $a(n)=r(2n)$ を用いれば、
\[
        \max_{n\geq1}a(n)=8,
        \qquad
...
この段階での記法を簡潔にするため、$ q_m=[3\underbrace{11\cdots1}_{m-1\text{ 桁}}]_4
      =3\cdot4^{m-1}+u$とし、
\[
...
\]
を得る。実際、$\Delta_m$ の元の新しい$m$ 桁の元は先頭桁は $1$ である。一方、
$\mathcal E_m$ の新しい $m$ 桁の元は、先頭桁が $3$ で、その後に
$\Delta_{m-1}$ から来る下位 $m-1$ 桁の補正項が続く。

$t$ 個の $\Delta_m$ のコピーのうち $s$ 個が平行移動された部分を用い、
$6-t$ 個の $\mathcal E_m$ のコピーのうち $v$ 個が新しい $m$ 桁部分を用いるなら、\eqref{eq:S-t-m}は、
...
$ L=\{3,13,14,53,54,57,58,60\}$の部分集合を
...
\end{split}
\]
を得る。$ H=\{13,214,217,218,220,229,230,233,234,236,241,242,244,248\}$を加えると、最初の三つの短い区間から、
\[
\begin{aligned}
...
\end{aligned}
\]
を得る。これらの和集合は $[245,304]$ を含む。一方で、$H$ の連続する元の差は高々 $9$ であるので、$I=[a,b]$ が $b-a\geq8$ を満たすなら、
