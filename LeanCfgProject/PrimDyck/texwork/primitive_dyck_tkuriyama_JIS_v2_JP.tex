...
基数 $m$ の数字からなる語 $w$ に対し、それが基数 $m$ で表す整数を $\val{w}{m}$ と書く。便宜上 $0$ を加えて、
\[
...
\[
  ...
  \sigma(1)=\sigma(2)=0
\]
によって割り当てる。語 $w=c_1\cdots c_\ell\in\{0,1,2,3\}^{\ell}$ が\emph{2色Motzkin語}であるとは、任意の接頭辞の総重みが非負であり、かつ $w$ 全体の総重みが $0$ であることをいう。重み0の二つの数字 $1$ と $2$ は、異なる色とみなす。$w$ を基数4の数として読んだ値を $[w]_4$ と書き、$[\varepsilon]_4=0$ とする。
...
\begin{proof}
$v=4p\in\NpD$、$v\geq12$ とし、$v$ の2進数字を左から連続する2桁ずつにまとめる。各組を基数4の数字として読むと、語 $d_1\cdots d_L$ が得られる。各組の境界における2進路の高さは、累積 $\sigma$ 重みの2倍である。実際、
\[
  11\leftrightarrow3,\quad 10\leftrightarrow2,\quad
  01\leftrightarrow1,\quad 00\leftrightarrow0
\]
だからである。$v$ は長さ4以上の原始Dyck数なので、先頭が $11$、末尾が $00$ である。したがって $d_1=3$、$d_L=0$ である。最後以外の各組境界では2進路の高さは少なくとも $2$ であり、最後の境界で $0$ となる。

これらの境界条件は、各2桁組の内部の高さも制御する。型 $01$ の組は、高さを一時的に1だけ下げてから元に戻す。最後でない型 $00$ の組は、終了時の高さが少なくとも $2$ なので、開始時の高さは少なくとも $4$ である。また、残る二つの型はいずれも $1$ で始まる。したがって、最後のビットより前で高さが正であることは、最後の境界で $0$ に戻ることと併せれば、すべての真の組境界で高さが正であることと同値である。

$v$ を $4$ で割ることは、末尾の基数4数字 $0$ を削除することに対応する。したがって、$p$ の基数4展開は $3w$ であり、$3w$ の任意の接頭辞の $\sigma$ 重みは少なくとも $1$、また $3w$ 全体の重みは $1$ である。先頭の数字 $3$ を取り除けば、$w$ の任意の接頭辞の重みが非負であり、$w$ 全体の重みが $0$ であることが分かる。

逆に、$p$ の基数4展開が $3w$ で、$w$ が2色Motzkin語であるとする。末尾に数字 $0$ を付け加え、各基数4数字を対応する2進の2桁組に展開する。得られる2進語は $1$ と $0$ を同数含み、上の組内部の確認により最後のビットより前では高さ $0$ より真に上にあり、最後に高さ $0$ に戻る。したがって原始Dyck語であり、$4p\in\NpD$、ゆえに $p\in\Pset$ である。
\end{proof}
...
\begin{proof}
$p$ の基数4展開を $3w$ と書く。ただし $w$ は2色Motzkin語である。このとき、$4p+1$ と $4p+2$ の基数4展開は、それぞれ $3w1$ と $3w2$ である。重み0のいずれの色を末尾に付け加えても2色Motzkin条件は保たれるので、補題~\ref{lem:base4} から主張が従う。
\end{proof}
...
\emph{下界。}
最初の数字は $0$ ではあり得ない。$c_1\in\{1,2\}$ の場合、接尾辞 $w'=c_2\cdots c_\ell$ も2色Motzkin語なので、
\[
...
\emph{上界。}
$c_1\in\{1,2\}$ の場合、$w'$ も2色Motzkin語なので、
\[
  [w]_4
...
次に $c_1=3$ とし、接頭辞の重みが初めて $0$ に戻る位置を $r$ とする。このとき $2\leq r\leq\ell$、$c_r=0$ であり、
\[
