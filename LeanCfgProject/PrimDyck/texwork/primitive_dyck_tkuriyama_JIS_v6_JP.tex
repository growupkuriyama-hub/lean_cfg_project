...
$k\geq1$ とし、$A\subseteq\mathbb N$ が
...
\begin{proof}
$m=a_1+\cdots+a_k$（$a_i\in A$）と書く。任意の
$k\leq r\leq2k$ に対して、
\[
 \eta_1+\cdots+\eta_k=r,
 \qquad \eta_i\in\{1,2\}
\]
となる $\eta_1,\ldots,\eta_k$ を選べる。条件\eqref{eq:digit-closure}により、
\[
...
$k\geq3$ とし、$A\subseteq\mathbb N$ が
\eqref{eq:digit-closure} を満たすとする。ある整数 $M\geq1$ に対して
...
\begin{proof}
$M\leq n\leq4M+k-1$ の場合は、仮定
\eqref{eq:tail-seed} より従う。

$M\leq j<n$ を満たすすべての整数 $j$ に対して
$j\in kA$ が成り立つと仮定する。以下ではとくに$4M+k\leq n$の場合を考える。
$\rho\equiv n\pmod4$ を満たす一意な$ \rho\in\{k,k+1,k+2,k+3\}$
を選び、$ m:=\frac{n-\rho}{4}$
とおく。$\rho\equiv n\pmod4$ であるから、$m$ は整数である。

$n\geq4M+k$ および $\rho\leq k+3$ より、
\[
 m=\frac{n-\rho}{4}
 \geq\frac{4M+k-(k+3)}{4}
 =M-\frac34.
\]
$m$ は整数なので、$m\geq M$ である。一方、$\rho\geq k\geq3$ より、
\[
 m=\frac{n-\rho}{4}
 \leq\frac{n-k}{4}
 <n.
...
\]
ゆえに補題~\ref{lem:digit-lifting} を適用して、
\[
 n=4m+\rho\in kA
\]
を得る。以上により、強い帰納法が完了する。
\end{proof}
...
$\mathsf{Motz}$ に属するから、補題~\ref{lem:base4} より
\[
...
\end{equation}
である。また、4進展開の末尾に $1$ または $2$ を付け加えることにより、
...
\]
を与える。式~\eqref{eq:E-tail-certificate} の5区間も互いに重なり、
\[
 [218,877]\subseteq6\mathcal E^+
\]
を与える。ここで $\mathcal E^+$ は \eqref{eq:Eclosure} を満たす。
系~\ref{cor:tail-propagation} を $h=6$、$M=218$ に適用すると、
$[218,\infty)\subseteq6\mathcal E^+$ を得る。二つの区間を合わせれば
主張が従う。
\end{proof}
...
正規族 $\mathcal E$ は法 $4$ で $0$ の合同類を処理するには十分であるが、
もう一方の合同類には完全な族 $\Pset$ が必要になる。有限な基底部分として
...
\end{split}
\end{equation}
を用いる。これらの所属は、各元の4進展開が $3w$（$w\in\mathsf{Motz}$）
の形であることから、補題~\ref{lem:base4} により直ちに確認できる。
また、長さ $0,1,2$ の2色Motzkin語を列挙すると、それらから得られる
$\Pset$ の元は正確に $L$ の8元である。一方、系~\ref{cor:gen} より
世代4の最小元は $g_4=213$ である。したがって、
