...
$h\geq1$ とし、$A\subseteq\mathbb N$ が
\begin{equation}\label{eq:digit-closure}
 4A+\{1,2\}\subseteq A
\end{equation}
を満たすとする。このとき、$m\in hA$ ならば
\begin{equation}\label{eq:digit-lifting}
 [4m+h,4m+2h]\subseteq hA
\end{equation}
である。
\end{lemma}

\begin{proof}
$m=a_1+\cdots+a_h$（$a_i\in A$）と書く。任意の
$h\leq r\leq2h$ に対して、
\[
 \eta_1+\cdots+\eta_h=r,
 \qquad \eta_i\in\{1,2\}
\]
となる $\eta_1,\ldots,\eta_h$ を選べる。したがって、
\[
 4m+r=\sum_{i=1}^{h}(4a_i+\eta_i)\in hA
\]
であり、主張が従う。
\end{proof}

\begin{corollary}[有限区間から尾部への伝播]\label{cor:tail-propagation}
$h\geq3$ とし、$A\subseteq\mathbb N$ が
\eqref{eq:digit-closure} を満たすとする。ある整数 $M\geq1$ に対して
\begin{equation}\label{eq:tail-seed}
 [M,4M+h-1]\subseteq hA
\end{equation}
ならば、
\[
 [M,\infty)\subseteq hA
...
$M\leq n\leq4M+h-1$ は仮定による。$n\geq4M+h$ とし、
$\rho\equiv n\pmod4$ を満たす一意な
$\rho\in\{h,h+1,h+2,h+3\}$ を選ぶ。$h\geq3$ より
$h\leq\rho\leq2h$ である。また、
\[
 m=\frac{n-\rho}{4}
\]
とおけば $M\leq m<n$ である。帰納法の仮定より $m\in hA$ なので、
補題~\ref{lem:digit-lifting} から $n=4m+\rho\in hA$ を得る。
\end{proof}
