...
\end{gathered}
\]
this is OEIS sequence \seqnum{A057547} \cite{OEIS}.  Every positive member of
...
\seqnum{A014486} \cite{OEIS}).  Using regular underapproximations and automata,
...
Enumeratively, the restriction is asymptotically mild.  There are $C_n$
Dyck words of length $2n$, and every primitive one is uniquely of the
form $1u0$ with $u$ a Dyck word of length $2n-2$; so there are
$C_{n-1}$ primitive words of length $2n$.  From the Catalan formula
...
positive primitive Dyck numbers whose sum is $N$.  The sequence
$(r(2n))_{n\geq1}$ is OEIS sequence \seqnum{A395858} \cite{OEIS}.
Its terms can be computed using the accompanying Python program
\texttt{compute\_A395858.py}.  The program is available at
...
\end{split}
\]
Membership is checked directly from the binary expansions of the
corresponding primitive Dyck numbers in $4L$ and $4H$.

The finite sumset computations from Lemma~\ref{lem:finitefive} through
Lemma~\ref{lem:finiteexception} can be reproduced using the accompanying
Python program \path{verify_certificates_for_paper.py}.
The program is publicly available at
...
$\NpD$ is an additive basis of exact order eight relative to the even
nonnegative integers.  It is also an asymptotic additive basis of order
at most six relative to those integers; equivalently, $\frac12\NpD$ is
an asymptotic additive basis of $\mathbb N_0$ of order at most six.
Corollary~\ref{cor:threshold} shows that the associated threshold $848$
is sharp.  Whether five summands suffice for all sufficiently large even
integers remains open.


\section{Sequences from the OEIS}\label{sec:oeis}

Three OEIS sequences are discussed in this paper.  The totally
balanced numbers form \seqnum{A014486}.  The primitive Dyck numbers,
denoted $\NpD$ here, form \seqnum{A057547}.  Finally, the minimum number
$r(2n)$ of positive primitive Dyck numbers whose sum is $2n$ forms
\seqnum{A395858}.
