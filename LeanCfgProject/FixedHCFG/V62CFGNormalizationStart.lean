...
theorem v62_fresh_start_separated
  ...
  classical
  intro r hr
  change Symbol.nonterminal (none : Option g.NT) ∉ r.output
...
theorem v62_fresh_start_produces
  ...
  · simp [v62FreshStartGrammar]
  · simpa [v62FreshStartRule] using
      (ContextFreeRule.Rewrites.input_output
        (r := v62FreshStartRule g))
...
theorem v62_fresh_start_preserves_old_word_derivation
  ...
  simpa [v62LiftCFGSymbol, List.map_map, Function.comp_def] using hLift

/-- Hence every old language word is generated by the fresh-start grammar. -/
theorem v62_language_subset_fresh_start
    {T : Type*} (g : ContextFreeGrammar T) :
    g.language ⊆ (v62FreshStartGrammar g).language := by
  intro w hw
  ...
  simpa [v62FreshStartGrammar] using hAll
