...
Current clean-restart checkpoint: workflow run `MCFG2 Confirmed-Success Regression v4 (233 roots) #1`, commit `f4619eafd68b2b96a52a4eb02584d36da0f12a51`, manually run by `growupkuriyama-hub`.
...
The authoritative clean-restart status is now the fixed successful-root regression.  The workflow `MCFG2 Confirmed-Success Regression v4 (233 roots)` contains an exact manifest of `233` MCFG2 modules and checks each listed module as a root target.  At the start of every shard it removes the compiled MCFG2 products, and before each target it removes that target's own build products, so every listed root is forced to elaborate again.  Workflow run `MCFG2 Confirmed-Success Regression v4 (233 roots) #1` passed at commit `f4619eafd68b2b96a52a4eb02584d36da0f12a51`.

The current MCFG2 snapshot contains `303` Lean modules.  Therefore `233/303` modules (76.9%) are directly CI-confirmed by the present regression workflow, while `70` modules remain outside the confirmed-success manifest.  This is stronger evidence than the former single-target dependency-closure checkpoint, but it is still not an all-files MCFG2 verification.
...
```text
.github/workflows/MCFG2_successful_experiments_regression_v4_233.yml
```

Its essential checking procedure is:

```text
fixed successful-root manifest: 233 modules
shards: 12
...
collect all 233 results before the final regression decision
```
...
| Directly confirmed-success root modules | 233 |
| Modules outside the confirmed-success manifest | 70 |
| Direct-root coverage | 233/303 = 76.9% |
| Regression result | PASS (233/233) |
| Workflow run | `MCFG2 Confirmed-Success Regression v4 (233 roots) #1` |
| Commit | `f4619eafd68b2b96a52a4eb02584d36da0f12a51` |
| Pusher | `growupkuriyama-hub` |

Interpretation:

```text
fixed 233-root regression manifest: confirmed PASS
all 233 listed roots: forced to elaborate as root targets
all MCFG2 modules: not yet confirmed
70 modules outside the manifest: still pending
```
...
the authoritative current manifest contains exactly 233 directly passing roots;
the remaining 70 MCFG2 modules must still be checked and repaired separately.
```
...
The authoritative current count is `233` directly confirmed root modules in the
passing `MCFG2 Confirmed-Success Regression v4 (233 roots)` manifest, with `70` MCFG2
...
```text
Workflow run: MCFG2 Confirmed-Success Regression v4 (233 roots) #1
Run name: MCFG2 confirmed 233 regression — master @ f4619eafd68b2b96a52a4eb02584d36da0f12a51
Commit: f4619eafd68b2b96a52a4eb02584d36da0f12a51
Pusher: growupkuriyama-hub
Trigger: manual workflow dispatch
Manifest: 233 fixed MCFG2 root modules
...
uses a fixed list of 233 root modules.  It clears compiled MCFG2 products at the
...
directly confirmed-success roots:       233
outside confirmed-success manifest:      70
direct-root coverage:                    76.9%
```

A module counted as “directly confirmed” was present in the fixed v4 manifest
...
The 70 pending modules are not all known to be false or broken.  They are simply
not members of the authoritative passing manifest.  In particular, changes to
a pending file are not certified by the 233-root guard until that file is added
deliberately to a later successful-root manifest.

### 10.3 Next experiment required

The next verification phase should work only on the 70 pending modules while
keeping the 233-root regression workflow enabled as a non-regression guard.
The recommended order is:

```text
1. continue the deduplicated frontier diagnostic on the 70 pending roots;
...
### 10.4 Directly confirmed-success modules (233)

<details>
<summary>Show the 233 directly confirmed files</summary>
...
### 10.5 Modules still outside the confirmed-success manifest (70)

<details>
<summary>Show the 70 pending files</summary>
...
Current fixed 233-root regression: PASS at commit f4619eafd68b2b96a52a4eb02584d36da0f12a51
Current directly confirmed coverage: 233/303 = 76.9%
Current pending set: 70 modules
