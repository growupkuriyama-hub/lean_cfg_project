...
/-! ##########################################################################
    PART 4 : INTEGRATION.  Feed the exact-once constructor directly into the
    existing local rule-splicing and filling-witness interfaces.
    ...
    The definitions below implement the corrected version: take the exact-once
    constructor plus the working-grammar exact-once hypothesis, and pass
    `hexact ρ hρ : ρ.ExactlyOnce` (definitionally
    `TemplateTuple.ExactlyOnce ρ.body`) to `splice`.
    ########################################################################## -/
...
/-! ## Exact-once integration with the reachable proof chain -/

section ExactRuleIntegration
...
namespace ExactNamedContextSplicingConstructor

/-- Exact-once splicing gives local evidence for every listed binary rule. -/
...
/-- Exact-once splicing gives a rule-indexed splicing provider. -/
def toRuleSplicingProvider
  ...
  evidence := by
    intro ρ hρ
    exact U.toRuleSplicingEvidence (G := G) (C := C) hexact ρ hρ

/-- Exact-once splicing gives the filling witnesses used by the reachable theorem. -/
def toFillingWitnessFamily
  ...
  (U.toRuleSplicingProvider hexact G C).toFillingWitnessFamily

end ExactNamedContextSplicingConstructor

end ExactRuleIntegration
