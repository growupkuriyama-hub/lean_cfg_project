...
/--
Shortlex can only improve terminal length: the canonical `omega(X)` is no
longer than any other terminal yield generated by the same retained state.
-/
theorem canonicalOmega_length_le_of_derives
    ...
    {z : Word Sigma}
    (hz : TypedDerives Obs terminal binary X.1 z) :
    (canonicalOmega X).length ≤ z.length := by
  by_contra h
  have hlt : z.length < (canonicalOmega X).length := Nat.lt_of_not_ge h
  exact (canonicalOmega_minimal X hz) (List.Shortlex.of_length_lt hlt)

/--
The v44 context order minimizes total context length before its shortlex
tie-breaker, so the canonical `chi(X)` is no longer than any constructed
reaching context.  This is the bridge used for the wrapper case.
-/
theorem canonicalChi_total_length_le_of_occurs
    {N : Type v} {Sigma : Type u}
    [LinearOrder Sigma] [WellFoundedLT Sigma]
    {Obs : Observer Sigma}
    {terminal : TerminalRules N Sigma} {binary : BinaryRules N}
    ...
    (X : KeptState Obs terminal binary start)
    {l r : Word Sigma}
    (hOcc : TypedOccurs Obs terminal binary start X.1 l r) :
    (canonicalLeftCtx X).length + (canonicalRightCtx X).length ≤
      l.length + r.length := by
  by_contra h
  have hlt : l.length + r.length <
      (canonicalLeftCtx X).length + (canonicalRightCtx X).length :=
    Nat.lt_of_not_ge h
  apply canonicalChi_minimal X hOcc
  change Prod.Lex (fun a b : Nat => a < b)
    (ContextShortlex (Sigma := Sigma))
    (contextCanonicalKey (l, r))
    (contextCanonicalKey (canonicalChi X))
  apply Prod.Lex.left
  simpa [contextCanonicalKey, canonicalLeftCtx, canonicalRightCtx] using hlt

/-- Strict upper bounds on a constructed context transfer to the canonical one. -/
theorem canonicalChi_total_length_lt_of_occurs
    {N : Type v} {Sigma : Type u}
    [LinearOrder Sigma] [WellFoundedLT Sigma]
    {Obs : Observer Sigma}
    {terminal : TerminalRules N Sigma} {binary : BinaryRules N}
    {start : StartRules N}
    (X : KeptState Obs terminal binary start)
    {l r : Word Sigma} {k : Nat}
    (hOcc : TypedOccurs Obs terminal binary start X.1 l r)
    (hBound : l.length + r.length < k) :
    (canonicalLeftCtx X).length + (canonicalRightCtx X).length < k := by
  exact lt_of_le_of_lt (canonicalChi_total_length_le_of_occurs X hOcc) hBound
