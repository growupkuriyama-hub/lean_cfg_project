...
fixed.  The wrappers are marked noncomputable because they use classical
propositional decidability.
-/

variable {Q : Type u} [Mul Q]

noncomputable def sameObservedSyntactic_has_decision
    (S : Set Q) (x y : Q) :
    Decidable (SameObservedSyntactic S x y) := by
  classical
  exact inferInstance

noncomputable def frameResidual_membership_has_decision
    (S : Set Q) (a b x : Q) :
    Decidable (x ∈ FrameResidual S a b) := by
  classical
  exact inferInstance

noncomputable def singleObservedBlock_has_decision
    (S : Set Q) (a b : Q) :
