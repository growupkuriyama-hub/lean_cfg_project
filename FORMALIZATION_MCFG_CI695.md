...
Current confirmed CI point: Lean CI #695, commit `01c31b5`, pushed by `growupkuriyama-hub`.
...
reduced to the binary length of one maximum serialized field-value bound.  The
remaining encoded-size task is now sharply localized: bound that maximum
serialized field value, and the natural-field count, by sample length, fan-out,
presentation size, and template-size parameters.
...
`ConcreteCanonicalLearnerWorkingGrammarNaturalFieldMaximum.lean` is now
confirmed passed at Lean CI #695.  The verified chain listed above contains
205 CI-confirmed files.
...
grammar bit length: reduced to natural-field count and one maximum natural-field value
...
current strongest top-level item-count endpoint:
  correctedConcreteWorkingGrammarLearner_presentationSize_semantic_package
current strongest complete codec endpoint:
...
Here `naturalFieldValueBound` is the maximum of the complete natural-field
count and the largest natural value appearing in the checked grammar
serialization.  A sample-length/fan-out bound on this last value remains open.
...
checked natural-list and prefix-free bit serialization of actual output grammars: complete
encoded symbol/bit description-size infrastructure: about 82--88%
sample-parametric closed bit-size bound: about 60--65%
executable polynomial learner implementation: about 60--70%
whole positive-learning and quantitative half: about 84--88%
whole paper including observation design and boundary/non-identifiability results: about 72--77%
```

The principal remaining work is now sharply divided into two tracks.  On the
quantitative/executable side: classify every natural serialization field,
bound the maximum field value and natural-field count by sample length, fan-out,
and presentation size, replace brute-force bounded-word enumeration by sample
factorization, and remove remaining `noncomputable` definitions.  On the
mathematical boundary side: strict output normalization if desired, optional
start normalization, bounded observation products, observation-design
complexity, no-advice non-identifiability, and copy/member-kernel exclusions.
...
What remains after the CI #695 encoding progress:
...
classification and sample-parametric bounds for every serialized natural field
sample-length/fan-out bound for naturalFieldCount and naturalFieldValueBound
...
and finally the complete checked natural/bit serialization chain through the
maximum-field reduction.
...
CI #695 completes the qualitative positive-learning route, actual finite
grammar compilation, top-level item-count analysis, exact representation
hierarchies, minimum sample-length ranks, semantic mind-change stabilization,
and a complete checked natural/bit serialization pipeline for the actual
learner output.  The remaining limitations are executable construction,
sample-parametric closure of the bit-size bounds, normal-form,
sharper-complexity, and boundary issues.
...
function-valued Finsets, and noncomputable filters/images.  The new
serialization files provide checked finite codecs and round-trip theorems, but
the encoders themselves still inherit noncomputability from the underlying
finite objects and dense first-index choices.  Extracted executable learner
code is therefore still absent.
...
The previous open-ended encoding problem is now substantially closed
structurally.  Verified components include:

```text
abstract per-entry cost and total description-size bounds
natural, unary, binary, dense, and globally tagged dense entry codes
checked top-level dense decoding
dependent template-atom and component-framed tuple serialization
complete binary-rule natural serialization
finite terminal-alphabet closure for all actual compiled rules
complete tagged grammar serialization as one List Nat
complete prefix-free unary and logarithmic-structure List Bool codecs
automatic least positive common natural-field width
reduction to naturalFieldCount and maximumNaturalFieldValue.
```

The remaining gap is no longer “construct a grammar encoding.”  It is the
quantitative closure theorem:

```text
bound every natural serialization field,
naturalFieldCount,
and naturalFieldValueBound
by sampleLengthBudget K, fan-out f, and existing presentation/rule bounds.
```

Once those inequalities are proved, the current checked codec immediately gives
a fully expanded sample-parametric bit-size bound.
...
sample-parametric bound on naturalFieldCount and naturalFieldValueBound
fully expanded encoded hypothesis bit size
...
learner, the actual finite-object learner, the actual grammar-valued learner,
and several successive encoding interfaces.  A final public import should
identify the CI #695 semantic and codec endpoints as the canonical route and
move superseded interfaces to legacy/internal namespaces.

## 6. Immediate next files

The current CI #695 endpoint is:

```lean
ConcreteCanonicalLearnerWorkingGrammarNaturalFieldMaximum.lean
```

The next file should classify and bound the concrete natural fields already
produced by the verified whole-grammar serializer.

### 6.1 `ConcreteCanonicalLearnerWorkingGrammarNaturalFieldClassification.lean`

Recommended next.

Prove that every member of

```lean
H.encodeCompiledWorkingGrammarNaturalList dummy
```

comes from one of the finite structural field classes:

```text
complete entry count
entry payload length
top-level entry tag
global nonterminal dense code
terminal dense code
...
left/right variable index.
```

Package elimination lemmas that reduce a universal field bound to one bound for
each class.

### 6.2 `ConcreteCanonicalLearnerWorkingGrammarNaturalFieldValueBounds.lean`

Use the classification to prove a common structural bound from:

```text
presentationItemCount
compiledTerminalAlphabet.card
maximum lhs/child fan-out
maximum template-component length
maximum binary payload length
sampleLengthBudget K.
```

Conclude explicit bounds for:

```lean
compiledWorkingGrammarMaximumNaturalFieldValue
compiledWorkingGrammarNaturalFieldValueBound
compiledWorkingGrammarAutomaticNaturalFieldBitWidth.
```

### 6.3 `ConcreteCanonicalLearnerWorkingGrammarNaturalFieldCountBounds.lean`

Bound the complete number of natural serialization fields by:

```text
presentation entry count
plus total serialized rule/body size.
```

Then substitute existing source-rule and sample-length bounds.

### 6.4 `ConcreteCanonicalLearnerWorkingGrammarBitDescriptionSize.lean`

Combine the previous two files with

```lean
compiledWorkingGrammarLogarithmicBitCount_le_binaryNatCodeLength_valueBound
```

to obtain the fully expanded bit-size theorem in sample and fan-out
parameters.
...
and prove semantic equivalence with the current learner and serializer.
...
### 6.7 `ConcreteCanonicalLearnerComplexity.lean`

After executable factorization enumeration exists, prove fixed-parameter
construction-time and encoded hypothesis-size bounds.

### 6.8 Strict output normalization and boundary theorem files

If required by the final paper statement:
...
Status: substantially complete at the semantic, item-count, and structural
codec levels.
...
abstract description-cost interface
natural/unary/binary/dense/tagged-dense top-level encodings
checked dependent template and binary-rule serialization
finite terminal closure of actual compiled rules
complete checked List Nat grammar serialization
complete prefix-free unary and logarithmic-structure List Bool codecs
automatic least common natural-field bit width
one-value reduction of total bit size.
```

Still open:

```text
classification of all concrete natural serialization fields
sample/fan-out bounds on naturalFieldCount and naturalFieldValueBound
fully expanded sample-parametric bit-size theorem
executable list-based learner implementation
...
abstract, dense, and globally tagged top-level encoding bounds
checked tagged-dense decoder
dependent template and complete binary-rule structural serialization
finite augmented terminal-alphabet closure for actual compiled rules
complete checked grammar serialization as List Nat
complete checked prefix-free unary and logarithmic-structure List Bool codecs
automatic least positive common natural-field width
reduction of total grammar bit size to field count and one maximum field value.
```
...
checked serialization and decoding functions that remain noncomputable because
  their source finite objects and dense-code selections are noncomputable.
```

### Not yet verified

```text
fully executable decidable construction equivalent to the current learner
polynomial sample-factorization learner
classification of every concrete natural serialization field
sample-parametric bounds on naturalFieldCount and naturalFieldValueBound
fully expanded sample/fan-out bit-description-size bound
...
```bash
lake build LeanCfgProject.MCFG.ConcreteCanonicalLearnerWorkingGrammarNaturalFieldMaximum
```
...
Current strongest actual-grammar semantic and size endpoints:
...
Current strongest checked serialization endpoints:
...
⇒ complete checked natural-list serialization of every output grammar
⇒ complete checked prefix-free bit serialization
⇒ total bit length reduced to naturalFieldCount and naturalFieldValueBound.
```
...
CI #695 completes the actual codec construction and reduces the remaining bit
description-size problem to two natural quantities.  The next proof should
classify every field occurring in

```lean
H.encodeCompiledWorkingGrammarNaturalList dummy
```

and reduce a universal field-value bound to the finite structural cases:
entry counts and lengths, tags, dense nonterminal/terminal codes, binary payload
lengths, component lengths, and variable indices.  This is the shortest route
to a fully expanded sample-parametric bit-size theorem.
