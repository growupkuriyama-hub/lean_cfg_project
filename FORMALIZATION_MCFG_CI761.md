...
Current confirmed CI point: Lean CI #761, commit `1e4048d`, pushed by `growupkuriyama-hub`.
...
`ConcreteCanonicalLearnerWorkingGrammarObservationSelectionCanonicalMinimumCertificateSearch.lean`
is now confirmed passed at Lean CI #761.  The verified chain listed above
contains 269 CI-confirmed files.
...
fully encoded construction of the feasibility table and formal NP complexity: still open
...
finite decision-table, bounded-certificate, minimum-budget, and canonical-certificate search layer: complete relative to a correct table
...
observation-selection finite decision/certificate/search layer: about 75--85%
formal observation-selection complexity classes and hardness: about 25--35%
...
whole intended paper formalization: about 91--94%
```
...
profile/gap theory, and the table-relative decision/certificate/search layer are
now essentially closed.  The remaining work is to construct concrete
strict-gain witnesses, build the finite feasibility table from a fully encoded
selection instance rather than assume its semantic correctness, give an
intrinsic injective certificate encoding, prove verifier/runtime bounds and a
formal NP-membership statement, and then establish NP-hardness or
NP-completeness if valid.  On the boundary side: prove no-advice
...
least-code canonical certificates and singleton finite search under injective coding.
```
...
construction of the feasibility table from a fully encoded grammar/target instance
an intrinsic dense injective code for observation-subset certificates
formal polynomial verifier bounds and complexity-class membership
...
import should expose the CI #761 endpoints and place historical facades and
intermediate decomposition files behind internal/legacy imports.

## 6. Immediate next files

The current CI #761 endpoint is:

```lean
ConcreteCanonicalLearnerWorkingGrammarObservationSelectionCanonicalMinimumCertificateSearch.lean
```

The finite decision/certificate/search layer is now complete relative to a
correct feasibility table and an externally supplied injective certificate
code.  The cleanest next step is to internalize that code before attempting a
complexity-class theorem.

### 6.1 `ConcreteCanonicalLearnerWorkingGrammarObservationSelectionDenseCertificateEncoding.lean`

Construct a canonical dense code for certificates `certificate : Finset ι`
with `certificate ⊆ U`, for example by their bit vectors/positions in the finite
universe `U`.  Prove:

```text
code injectivity on subsets of U
code < 2 ^ U.card
checked decode/encode round trip
membership/subset preservation
replacement of the external certificateCode hypothesis in the canonical search.
```

### 6.2 `ConcreteCanonicalLearnerWorkingGrammarObservationSelectionExecutableVerifier.lean`

Package the finite decision table, dense certificate codec, and Boolean
verifier as an encoded finite instance.  Prove exact verifier correctness and
explicit operation/size bounds without consulting semantic target membership
during verification.

### 6.3 `ConcreteCanonicalLearnerWorkingGrammarObservationSelectionNP.lean`

Once the encoded verifier and size model are stable, formalize polynomially
bounded certificates and verification and state the appropriate NP-membership
theorem.  Do not claim NP-hardness until a concrete reduction is separately
constructed and checked.

### 6.4 `ConcreteCanonicalLearnerWorkingGrammarObservationStrictGainWitness.lean`
...
Replace classical/function-valued finite learner objects by decidable list-based
codes and prove semantic equivalence with the present certified learner and
checked serializer.
...
Status: semantic framework complete at the present abstraction level; finite
decision/certificate/search theory complete relative to a correct encoded table.
...
least-code canonical certificate and singleton search under injective coding.
```
...
fully encoded construction of the correct feasibility table
intrinsic dense certificate code and checked decoder
formal polynomial verifier/NP-membership theorem
...
least-code canonical certificates
singleton canonical minimum-certificate search under injective coding.
```
...
decision/search executability relative to an externally supplied correct feasibility table
canonical-certificate uniqueness relative to an externally supplied injective certificate code.
```
...
intrinsic checked dense encoding of observation-subset certificates
formal polynomial observation-selection verification and NP membership
NP-hardness or NP-completeness of observation design
no-advice non-identifiability
...
```bash
lake build LeanCfgProject.MCFG.ConcreteCanonicalLearnerWorkingGrammarObservationSelectionCanonicalMinimumCertificateSearch
```
...
minimum budgets and realizing subset certificates are selected
least-code minimum certificates are canonical
finite canonical search is a singleton under injective certificate coding.
```
...
CI #761 closes the finite decision, certificate, bounded-search,
minimum-selector, and least-code canonical-search layers relative to a correct
feasibility table.  The remaining artificial input in the final theorem is the
external function `certificateCode : Finset ι → Nat` together with an
injectivity assumption.  The next file should construct a dense checked code
intrinsically from the finite coordinate universe `U`, prove injectivity and a
`2 ^ U.card` bound, and instantiate the canonical singleton-search theorem
without any external coding hypothesis.  After that, the verifier/runtime and
formal NP-membership layer can be stated on genuinely encoded instances.
