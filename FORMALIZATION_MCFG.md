...
enumeration, sample-generated nonterminals, learner grammar skeletons, rule
skeletons, and Gold-style identification wrappers.
...
Latest checked CI: Lean CI #429
Latest commit reported by user: 0cf10b2
Status: succeeded
Top checked module: LeanCfgProject.MCFG.FI_v2_1_SampleGeneratedRuleSkeletonGold
Aggregate import: LeanCfgProject.MCFG.Basic
```

The current CI chain checks all MCFG companion files through Lake.  The top
currently checked block now reaches a **sample-generated rule skeleton**:
finite sample words are decomposed into subword contexts, same-context/same-type
subword pairs generate unit-edge witnesses, those witnesses generate canonical
learner nonterminals, and the resulting sample-generated grammar skeleton is
extended with start, terminal, binary-concatenation, and unit-rule candidates.

A typical local check is:

```bash
lake build LeanCfgProject.MCFG.FI_v2_1_SampleGeneratedRuleSkeletonGold
lake build LeanCfgProject.MCFG.Basic
lake build LeanCfgProject
```

Warnings about unused section variables, unused simp arguments, or GitHub
Actions runtime notices may remain.  They are not failed proof obligations.
...
- a canonical learner nonterminal type generated from sample support;
- a sample-generated learner grammar skeleton with arity and finite
  nonterminal-list membership facts;
- a sample-generated rule skeleton containing start, terminal,
  binary-concatenation, and unit-rule candidates.
...
> actual-refined-rule-list, sample-extraction, subword-decomposition,
> unit-edge-enumeration, canonical-nonterminal, learner-skeleton, rule-skeleton,
> and main-theorem-interface infrastructure for the MCFG learning construction,
...
interface or wrapper layers.  The post-#417 progress is more constructive and
vertical: actual refined rule lists, raw/subword decomposition enumeration,
unit-edge enumeration, canonical nonterminals, learner grammar skeletons, and
rule skeletons are now checked.
...
| 95 | `FI_v2_1_RawSampleDecompositionGeneration.lean` | Turns raw decomposition witnesses into generated support, sample-distribution facts, and unit reachability facts | checked |
| 96 | `FI_v2_1_RawSampleDecompositionGenerationExact.lean` | Exactness wrapper for raw sample decomposition generation | checked |
| 97 | `FI_v2_1_RawSampleDecompositionGenerationGold.lean` | Gold wrapper for raw sample decomposition generation | checked |
| 98 | `FI_v2_1_RawSampleDecompositionEnumeration.lean` | Enumerates identity one-hole decompositions for every sample word | checked |
| 99 | `FI_v2_1_RawSampleDecompositionEnumerationExact.lean` | Exactness wrapper for raw sample decomposition enumeration | checked |
| 100 | `FI_v2_1_RawSampleDecompositionEnumerationGold.lean` | Gold wrapper for raw sample decomposition enumeration | checked |
| 101 | `FI_v2_1_SubwordContextDecomposition.lean` | Two-sided `left ++ middle ++ right` subword/context decomposition witnesses | checked |
| 102 | `FI_v2_1_SubwordContextDecompositionExact.lean` | Exactness wrapper for subword/context decomposition | checked |
| 103 | `FI_v2_1_SubwordContextDecompositionGold.lean` | Gold wrapper for subword/context decomposition | checked |
| 104 | `FI_v2_1_SubwordContextEnumeration.lean` | Finite prefix/suffix and subword-cut enumeration for sample words | checked |
| 105 | `FI_v2_1_SubwordContextEnumerationExact.lean` | Exactness wrapper for subword/context enumeration | checked |
| 106 | `FI_v2_1_SubwordContextEnumerationGold.lean` | Gold wrapper for subword/context enumeration | checked |
| 107 | `FI_v2_1_SubwordUnitEdgeEnumeration.lean` | Enumerates same-context/same-type subword pairs as unit-edge witnesses | checked |
| 108 | `FI_v2_1_SubwordUnitEdgeEnumerationExact.lean` | Exactness wrapper for subword unit-edge enumeration | checked |
| 109 | `FI_v2_1_SubwordUnitEdgeEnumerationGold.lean` | Gold wrapper for subword unit-edge enumeration | checked |
| 110 | `FI_v2_1_CanonicalLearnerNonterminal.lean` | Canonical learner nonterminal type generated from sample support | checked |
| 111 | `FI_v2_1_CanonicalLearnerNonterminalExact.lean` | Exactness wrapper for canonical learner nonterminals | checked |
| 112 | `FI_v2_1_CanonicalLearnerNonterminalGold.lean` | Gold wrapper for canonical learner nonterminals | checked |
| 113 | `FI_v2_1_SampleGeneratedLearnerGrammarSkeleton.lean` | Sample-generated learner grammar skeleton with arity and nonterminal membership facts | checked |
| 114 | `FI_v2_1_SampleGeneratedLearnerGrammarSkeletonExact.lean` | Exactness wrapper for sample-generated learner grammar skeletons | checked |
| 115 | `FI_v2_1_SampleGeneratedLearnerGrammarSkeletonGold.lean` | Gold wrapper for sample-generated learner grammar skeletons | checked |
| 116 | `FI_v2_1_SampleGeneratedRuleSkeleton.lean` | Start, terminal, concatenation, and unit rule candidate skeletons generated from sample data | checked |
| 117 | `FI_v2_1_SampleGeneratedRuleSkeletonExact.lean` | Exactness wrapper for sample-generated rule skeletons | checked |
| 118 | `FI_v2_1_SampleGeneratedRuleSkeletonGold.lean` | Gold wrapper for sample-generated rule skeletons | checked |
...
### 4.8 Canonical learner nonterminals and grammar/rule skeletons

Current top constructive layers: `CanonicalLearnerNonterminal`,
`SampleGeneratedLearnerGrammarSkeleton`, `SampleGeneratedRuleSkeleton`, and
their exactness/Gold wrappers.

Representative declarations include:

```lean
CanonicalLearnerNonterminal
supportTupleNonterminals
supportContextNonterminals
supportUnitEdgeEndpointNonterminals
supportTypedTupleNonterminals
supportCanonicalNonterminals

SubwordUnitEdgeEnumerationData.learnerNonterminals
SubwordUnitEdgeEnumerationData.enumerated_sample_word_nonterminals

SampleGeneratedLearnerGrammarSkeleton
SampleGeneratedLearnerGrammarSkeleton.support
SampleGeneratedLearnerGrammarSkeleton.nonterminals
SampleGeneratedLearnerGrammarSkeleton.root
SampleGeneratedLearnerGrammarSkeleton.arity
SampleGeneratedDecompositionNode
SampleGeneratedUnitCandidate
SampleGeneratedUnitCandidate.unitReach
enumeratedSampleGeneratedLearnerGrammarSkeleton
enumeratedSkeleton_sample_word_node

SampleGeneratedStartCandidate
SampleGeneratedTerminalCandidate
SampleGeneratedConcatCandidate
SampleGeneratedUnitRuleCandidate
SampleGeneratedRuleSkeleton
enumeratedRuleSkeleton_sample_word_start_candidate
```

This block is the newest concrete progress.  The development defines a
canonical learner nonterminal type with root, tuple, context, and typed nodes.
It then builds a sample-generated learner grammar skeleton with arity and
finite nonterminal membership facts.  On top of that, it defines a rule
skeleton containing candidate start rules, terminal rules, binary-concatenation
rules, and unit-rule candidates.

This is still not yet a `WorkingMCFG`.  It is the immediate pre-grammar layer:
nonterminals and rule candidates are present, but they have not yet been
assembled into the final concrete grammar object with a proved string language.
...
| canonical learner nonterminal type | `CanonicalLearnerNonterminal` | formalized |
| finite learner nonterminal list from support | `supportCanonicalNonterminals` | formalized |
| sample-generated learner grammar skeleton | `SampleGeneratedLearnerGrammarSkeleton` | formalized |
| sample-generated rule skeleton | `SampleGeneratedRuleSkeleton` | formalized |
...
distributions, distribution-level reconstruction certificates, finite-hypothesis
certificates, output-type refined rule and grammar skeletons, finite refined-
grammar certificates, finite rule-enumeration plans derived from a finite
observation monoid, actual finite lists of refined terminal/binary/start rules
generated from such plans, finite support generated from observed sample atoms,
raw and subword sample decomposition witnesses, finite subword-cut enumeration,
unit-edge enumeration from same-context/same-type subword pairs, canonical
learner nonterminals, and sample-generated learner/rule skeletons.  The
formalization is not yet a machine-checked proof of the full concrete
reconstruction theorem: the concrete canonical learner grammar, final
sample-extracted grammar-rule construction, presentation-relative
characteristic-sample construction, hybrid filling lemma, no-advice boundary,
and bounded-spine polynomial-data theorem remain outside the current Lean
development.
```
...
support and unit-edge extraction, canonical learner nonterminals, and
sample-generated learner/rule skeletons.  The full concrete canonical-grammar
...
construction.  It currently reaches sample-generated learner nonterminals and
rule skeletons from finite subword-context enumeration, while the full concrete
...
1. **Sample-generated `WorkingMCFG` construction.**  Assemble the current
...
2. **Rule-candidate-to-rule-list conversion.**  Convert `SampleGeneratedStartCandidate`,
   `SampleGeneratedTerminalCandidate`, and `SampleGeneratedConcatCandidate` into
   concrete `StartRule`, `TerminalRule`, and `BinaryRule` lists.

3. **Sample consistency for the concrete learner grammar.**  Prove that every
...
4. **Occurrence witnesses in derivation trees.**  Strengthen `ExposedWithContext`
   into a derivation-tree occurrence notion.

5. **Hybrid filling lemma.**  Prove that replacing a tuple by a
   distribution-equivalent tuple inside a derivation context preserves target
   membership.

6. **Presentation-relative characteristic sample construction.**  Build the
   finite characteristic sample from a witnessing presentation rather than
   assuming it as a certificate input.

7. **Exact reconstruction theorem.**  Combine output-type refinement,
...
8. **No-advice boundary and bounded-spine results.**  Formalize the no-advice
...
sample-to-grammar path: convert the rule skeleton into actual `WorkingMCFG`
rule lists, prove sample generation, and then attack occurrence/hybrid filling.
...
> actual-refined-rule-list, sample-extraction, subword-decomposition,
> unit-edge-enumeration, canonical-nonterminal, learner-skeleton, rule-skeleton,
> and main-theorem-interface infrastructure of the MCFG learning construction
> through CI #429 / commit `0cf10b2`, while leaving the full concrete
